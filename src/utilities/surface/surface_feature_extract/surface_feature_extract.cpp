...
)
{
  ...
  if (index == -1)
  {
    FATAL_ERROR_IN("calcVertexNormals()")
...
)
{
  ...
  FOR_ALL(surf, fI)
  {
    ...
    FOR_ALL(fEdges, feI)
    {
      ...
      normalDifferences[feI] =
       pointNormals[meshPointMap[e[0]]] - pointNormals[meshPointMap[e[1]]];
    }
  ...
  FOR_ALL(curvaturePointField, pI)
  {
...
)
{
  ...
  FOR_ALL(edgeFaces, edgeI)
  {
    ...
    if (eFaces.size() > 2)
    {
      ...
      const mousse::vector& n0 = surf.faceNormals()[i0];
      //Pout<< "edge:" << edgeI << " n0:" << n0 << endl;
      bool same = true;
      for (label i = 1; i < eFaces.size(); i++)
      {
        //const labelledTri& f = surf[i];
        const mousse::vector& n = surf.faceNormals()[eFaces[i]];
...
)
{
  ...
  DynamicList<mousse::vector> normals{2};
  DynamicList<labelList> bins{2};
  FOR_ALL(eFaces, eFaceI)
  {
    const mousse::vector& n = surf.faceNormals()[eFaces[eFaceI]];
    // Find the normal in normals
    label index = -1;
    FOR_ALL(normals, normalI)
    {
  ...
  else
  {
    ...
    if (includedAngle >= 0)
    {
      ...
      FOR_ALL(eFaces, i)
      {
        const mousse::vector& ni = surf.faceNormals()[eFaces[i]];
        for (label j=i+1; j<eFaces.size(); j++)
        {
          const mousse::vector& nj = surf.faceNormals()[eFaces[j]];
          if (mag(ni & nj) < minCos)
          {
...
int main(int argc, char *argv[])
{
  ...
  #include "set_system_run_time_dictionary_io.inc"
  ...
  FOR_ALL_CONST_ITER(dictionary, dict, iter)
  {
    ...
    if (surfaceDict.isDict("subsetFeatures"))
    {
      ...
      if (!openEdges)
      {
        Info << "Removing all open edges" << " (edges with 1 connected face)"
          << endl;
        FOR_ALL(edgeStat, edgeI)
        {
    ...
    Info<< nl << "Writing featureEdgeMesh to " << bfeMesh.objectPath() << endl;
