...
public:
  ...
  :
    ...
    offset_(offset)
  {}
  inline Point toLocal(const mousse::point& pt) const
  {
    mousse::point p = cmptMultiply(scale_, (pt + offset_));
    return Point(p.x(), p.y(), p.z());
  }
  inline mousse::point toGlobal(const Point& pt) const
  {
  ...
  public ::data_access
{
  ...
  scalar signedDistance(const mousse::point& pt) const
  {
...
public:
  ...
  // Member Functions
    ...
    virtual bool need_refine( const Cube &c )
    {
      ...
      //}
      //mousse::List<mousse::List<pointIndexHit> > hitInfo;
      ...
      //{
      //    const mousse::List<pointIndexHit>& hits = hitInfo[edgeI];
      //    forAll(hits, i)
      //    {
      ...
      //
      //mousse::List<pointIndexHit> nearestInfo;
...
int main(int argc, char *argv[])
{
  ...
  if (mc.ntrigs() > 0)
  {
    ...
    mousse::DynamicList<labelledTri> tris(mc.ntrigs());
    ...
    {
      mousse::List<pointIndexHit> hitInfo;
