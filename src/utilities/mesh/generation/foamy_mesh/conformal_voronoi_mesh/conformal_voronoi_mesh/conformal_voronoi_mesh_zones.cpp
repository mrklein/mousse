...
) const
{
  ...
  FOR_ALL(faceToSurface, faceI) {
    blockedFace[faceI] = faceToSurface[faceI] != -1;
  }
  ...
  FOR_ALL(closedNamedSurfaces, i) {
    ...
    FOR_ALL(volType, pointI) {
      if (cellToSurface[pointI] != -1)
        continue;
      if ((volType[pointI] == volumeType::INSIDE && selectInside)
          || (volType[pointI] == volumeType::OUTSIDE && !selectInside)) {
...
) const
{
  ...
  FOR_ALL(faces, faceI) {
    ...
    if (mesh.isInternalFace(faceI)) {
      ...
      if ((ownerSurfaceI >= 0 || neiSurfaceI >= 0)
  ...
  calcNeighbourCellCentres(mesh, cellCentres, neiCc);
