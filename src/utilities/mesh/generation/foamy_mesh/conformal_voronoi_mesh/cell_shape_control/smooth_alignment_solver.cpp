...
)
{
  ...
  labelListList pointPoints;
  autoPtr<mapDistribute> meshDistributor =
    buildMap
    (
      mesh_,
      pointPoints
  ...
  for (auto vit = mesh_.finite_vertices_begin();
  ...
  for (label iter = 0; iter < maxSmoothingIterations; iter++) {
    ...
    FOR_ALL(alignments, pI) {
      ...
      for (direction dir = 0; dir < 3; ++dir) {
        if (newTriad.set(dir) && oldTriad.set(dir) && !fixedAlignment.set(dir)) {
    ...
    Info << ", Residual = "
      << residual/returnReduce(points.size(), sumOp<label>()) << endl;
  ...
  autoPtr<mapDistribute> referredDistributor =
    buildReferredMap
    (
      mesh_,
      referredPoints
