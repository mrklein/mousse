...
void mousse::CV2D::fast_restore_Delaunay(Vertex_handle vh)
{
  ...
  do {
    i = f->index(vh);
    if (!is_infinite(f)) {
      if (!internal_flip(f, cw(i)))
        external_flip(f, i);
      if (f->neighbor(i) == start)
        start = f;
    }
    f = f->neighbor(cw(i));
  } while (f != start);
...
void mousse::CV2D::external_flip(Face_handle& f, int i)
{
  ...
  const auto sooc = side_of_oriented_circle(n, f->vertex(i)->point());
  if (CGAL::ON_POSITIVE_SIDE != sooc) {
    return;
  }
  flip(f, i);
...
:
  ...
  relaxationModel_
  {
    relaxationModel::New(cvMeshDict.subDict("motionControl"), runTime)
  },
  z_
  {
    point
...
void mousse::CV2D::boundaryConform()
{
  ...
  for (auto fit = finite_faces_begin();
       fit != finite_faces_end();
       fit++) {
    fit->faceIndex() = Fb::SAVE_CHANGED;
  }
  for (label iter=1;
       iter <= meshControls().maxBoundaryConformingIter();
       iter++) {
    label nIntersections =
...
void mousse::CV2D::newPoints()
{
  ...
  for (auto eit = finite_edges_begin(); eit != finite_edges_end(); eit++) {
    ...
    alignmentDirsB[1] = vector2D{-alignmentDirsB[0].y(), alignmentDirsB[0].x()};
    ...
    FOR_ALL(alignmentDirs, aD) {
      ...
      if (alignmentDotProd <= cosAlignmentAcceptanceAngle)
        continue;
      scalar targetFaceSize = 0.5*(sizes[vA->index()] + sizes[vB->index()]);
      ...
      } else if ((vA->internalPoint() || vB->internalPoint())
