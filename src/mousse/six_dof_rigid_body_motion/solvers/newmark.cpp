...
DEFINE_TYPE_NAME_AND_DEBUG(Newmark, 0);
ADD_TO_RUN_TIME_SELECTION_TABLE(sixDoFSolver, Newmark, dictionary);
...
)
{
  ...
  // Correct position
  centreOfRotation() = centreOfRotation0()
    + (tConstraints() & (deltaT*v0()
                         + sqr(deltaT)*beta_*a()
                         + sqr(deltaT)*(0.5 - beta_)*a0()));
  // Correct orientation
  vector piDeltaT =
    rConstraints() & (deltaT*pi0()
                      + sqr(deltaT)*beta_*tau()
                      + sqr(deltaT)*(0.5 - beta_)*tau0());
