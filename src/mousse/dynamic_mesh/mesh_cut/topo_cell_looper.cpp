...
) const
{
  ...
  walkFace(features, faceI, startEdgeI, startVertI, nFeats, oppositeEdgeI,
           oppositeVertI);
  // Loop over super edge to find internal points if there are any.
  labelList superEdge =
    getSuperEdge(features, faceI, oppositeEdgeI, oppositeVertI);
...
) const
{
  ...
  do {
    ...
    label startLoop = -1;
    if ((vertI != -1)
        && ((startLoop = findIndex(loop, vertToEVert(vertI))) != -1)) {
    ...
    if ((edgeI != -1)
        && ((startLoop = findIndex(loop, edgeToEVert(edgeI))) != -1)) {
    ...
    if (vertI == -1) {
      ...
      walkAcrossFace(features, faceI, edgeI, vertI, 2, nextEdgeI, nextVertI);
      edgeI = nextEdgeI;
      vertI = nextVertI;
    } else {
      // On vertex.
      ...
      if (edgeI == -1) {
        ...
        labelList nextEdges = getVertEdgesNonFace(cellI, faceI, vertI);
        if (nextEdges.empty()) {
        ...
        labelList nextFaces{getVertFacesNonEdge(cellI, edgeI, vertI)};
        if (nextFaces.size() == 1) {
          ...
          faceI = -1;
          edgeI =
            meshTools::getSharedEdge(mesh(), nextFaces[0], nextFaces[1]);
          vertI = mesh().edges()[edgeI].otherVertex(vertI);
        } else {
...
) const
{
  if (mesh().cellShapes()[cellI].model() == hex_) {
    ...
    return
      hexCellLooper::cut(refDir, cellI, vertIsCut, edgeIsCut, edgeWeight,
                         loop, loopWeights);
  } else {
    cellFeatures superCell(mesh(), featureCos, cellI);
    if (hexMatcher().isA(superCell.faces())) {
      label edgeI =
        getAlignedNonFeatureEdge(refDir, cellI, superCell);
      label vertI = -1;
      label faceI = -1;
      if (edgeI != -1) {
      ...
      walkSplitHex(cellI, superCell, faceI, edgeI, vertI, localLoop,
                   localLoopWeights);
      if (localLoop.size() <=2) {
        return false;
      } else {
...
) const
{
  ...
  return
    hexCellLooper::cut(cutPlane, cellI, vertIsCut, edgeIsCut, edgeWeight, loop,
                       loopWeights);
}
