...
mousse::label mousse::cellCuts::findPartIndex
(
...
mousse::boolList mousse::cellCuts::expand
(
  const label size,
...
mousse::scalarField mousse::cellCuts::expand
(
  const label size,
  const labelList& labels,
...
mousse::label mousse::cellCuts::firstUnique
(
  const labelList& lst,
  const Map<label>& map
)
...
void mousse::cellCuts::writeUncutOBJ
(
  const fileName& dir,
  const label cellI
) const
...
mousse::label mousse::cellCuts::edgeEdgeToFace
(
  const label cellI,
  const label edgeA,
  const label edgeB
) const
{
  ...
  FOR_ALL(cFaces, cFaceI) {
    ...
    if (findIndex(fEdges, edgeA) != -1 && findIndex(fEdges, edgeB) != -1) {
     return faceI;
    }
  }
...
mousse::label mousse::cellCuts::edgeVertexToFace
(
  const label cellI,
  const label edgeI,
  const label vertI
) const
{
  ...
  FOR_ALL(cFaces, cFaceI) {
    ...
    const labelList& fEdges = mesh().faceEdges()[faceI];
    if (findIndex(fEdges, edgeI) != -1 && findIndex(f, vertI) != -1) {
     return faceI;
    }
  }
  WARNING_IN
...
mousse::label mousse::cellCuts::vertexVertexToFace
(
  const label cellI,
  const label vertA,
  const label vertB
) const
{
  ...
  FOR_ALL(cFaces, cFaceI) {
    label faceI = cFaces[cFaceI];
    const face& f = mesh().faces()[faceI];
    if (findIndex(f, vertA) != -1 && findIndex(f, vertB) != -1) {
     return faceI;
    }
  }
...
void mousse::cellCuts::calcFaceCuts() const
{
  if (faceCutsPtr_) {
  ...
  for (label faceI = 0; faceI < mesh().nFaces(); faceI++) {
    ...
    FOR_ALL(f, fp) {
      ...
      if (pointIsCut_[v0]) {
        ...
        if (!pointIsCut_[vMin1] && !edgeIsCut_[findEdge(faceI, v0, vMin1)]) {
...
mousse::label mousse::cellCuts::findEdge
(
  const label faceI,
  const label v0,
  const label v1
...
mousse::label mousse::cellCuts::loopFace
(
  const label cellI,
...
bool mousse::cellCuts::walkPoint
(
  const label cellI,
...
) const
{
  ...
  FOR_ALL(pFaces, pFaceI) {
    ...
    if (otherFaceI != exclude0 && otherFaceI != exclude1
        && meshTools::faceOnCell(mesh(), cellI, otherFaceI)) {
      label oldNVisited = nVisited;
      bool foundLoop = walkCell(cellI, startCut, otherFaceI, otherCut,
                                nVisited, visited);
      if (foundLoop) {
        return true;
      }
...
bool mousse::cellCuts::crossEdge
(
  const label cellI,
  const label startCut,
  const label faceI,
...
) const
{
  ...
  label oldNVisited = nVisited;
  // Recurse to otherCut
  bool foundLoop = walkCell(cellI, startCut, otherFaceI, otherCut, nVisited,
                            visited);
  if (foundLoop) {
    return true;
  } else {
...
) const
{
  ...
  bool validWalk = walkFace(cellI, startCut, faceI, cut, lastCut,
                            beforeLastCut, nVisited, visited);
  if (!validWalk) {
    return false;
  }
  if (debug & 2) {
  ...
  if (isEdge(beforeLastCut)) {
    if (isEdge(lastCut)) {
      ...
      return crossEdge(cellI, startCut, faceI, lastCut, nVisited, visited);
    } else {
      // beforeLastCut=edge, lastCut=vertex.
      // Go from lastCut to all connected faces (except faceI)
      return walkPoint(cellI, startCut, faceI, -1, lastCut, nVisited, visited);
    }
  } else {
    if (isEdge(lastCut)) {
      ...
      // edge.
      label edgeI =
        findEdge(faceI, getVertex(beforeLastCut), getVertex(lastCut));
      if (edgeI != -1) {
        ...
        return walkPoint(cellI, startCut, f0, f1, lastCut, nVisited, visited);
      } else {
...
// one found.
void mousse::cellCuts::calcCellLoops(const labelList& cutCells)
{
    ...
    if (nCutFaces[cellI] >= 3) {
      ...
      FOR_ALL(cFaces, cFaceI) {
        ...
        if (fCuts.size() >= 2) {
          ...
          validLoop =
            walkCell(cellI, fCuts[0], faceI, fCuts[0], nVisited, visited);
          if (validLoop) {
            break;
          }
...
void mousse::cellCuts::walkEdges
(
  const label cellI,
  const label pointI,
  const label status,
...
) const
{
  ...
  FOR_ALL(pEdges, pEdgeI) {
    ...
    if (meshTools::edgeOnCell(mesh(), cellI, edgeI)
...
mousse::labelList mousse::cellCuts::nonAnchorPoints
(
...
) const
{
  ...
  FOR_ALL(cellPoints, i) {
    ...
    if (findIndex(anchorPoints, pointI) == -1
...
bool mousse::cellCuts::calcAnchors
(
  const label cellI,
  const labelList& loop,
  const pointField& loopPts,
...
mousse::pointField mousse::cellCuts::loopPoints
(
  const labelList& loop,
...
bool mousse::cellCuts::validEdgeLoop
(
  const labelList& loop,
  const scalarField& loopWeights
) const
  ...
  FOR_ALL(loop, fp) {
    ...
    if (isEdge(cut)) {
      ...
      if (edgeIsCut_[edgeI]) {
        ...
        if (mag(loopWeights[fp] - edgeWeight_[edgeI])
...
mousse::label mousse::cellCuts::countFaceCuts
(
  const label faceI,
  const labelList& loop
) const
{
  ...
  FOR_ALL(f, fp) {
    ...
    if (pointIsCut_[vertI] || (findIndex(loop, vertToEVert(vertI)) != -1)) {
      nCuts++;
    }
  }
  // Count cut edges.
  ...
  FOR_ALL(fEdges, fEdgeI) {
    ...
    if (edgeIsCut_[edgeI] || (findIndex(loop, edgeToEVert(edgeI)) != -1)) {
      nCuts++;
    }
  }
  return nCuts;
...
bool mousse::cellCuts::validLoop
(
  const label cellI,
  const labelList& loop,
  const scalarField& loopWeights,
...
) const
{
  ...
  return calcAnchors(cellI, loop, loopPoints(loop, loopWeights), anchorPoints);
}
...
void mousse::cellCuts::setFromCellLoops()
{
  ...
  FOR_ALL(cellLoops_, cellI) {
    ...
    if (loop.size()) {
      ...
      labelList anchorPoints;
      if (!validLoop(cellI, loop, loopWeights(loop), faceSplitCuts,
                     anchorPoints)) {
...
// was valid.
bool mousse::cellCuts::setFromCellLoop
(
  const label cellI,
  const labelList& loop,
  const scalarField& loopWeights
...
void mousse::cellCuts::setFromCellCutter
(
...
void mousse::cellCuts::check() const
{
  ...
  FOR_ALL(edgeIsCut_, edgeI) {
    if (edgeIsCut_[edgeI]) {
      if (edgeWeight_[edgeI] < geomCellLooper::snapTol()
  ...
  FOR_ALL(cellLoops_, cellI) {
    ...
    FOR_ALL(loop, i) {
      label cut = loop[i];
      if ((isEdge(cut) && !edgeIsCut_[getEdge(cut)])
  ...
  FOR_ALL(cellLoops_, cellI) {
    ...
    FOR_ALL(loop, i) {
      label cut = loop[i];
      if (!isEdge(cut) && findIndex(anchors, getVertex(cut)) != -1) {
        FATAL_ERROR_IN("cellCuts::check()")
          << "cell:" << cellI << " loop:" << loop
  ...
  cellAnchorPoints_{mesh.nCells()}
{
  ...
  clearOut();
  if (debug) {
    Pout << "cellCuts : leaving constructor from cut verts and edges" << endl;
  }
}
...
// Member Functions 
mousse::pointField mousse::cellCuts::loopPoints(const label cellI) const
{
  const labelList& loop = cellLoops_[cellI];
  pointField loopPts{loop.size()};
  FOR_ALL(loop, fp) {
...
// Flip loop for cell
void mousse::cellCuts::flip(const label cellI)
{
  labelList& loop = cellLoops_[cellI];
  reverse(loop);
  // Reverse anchor point set.
  cellAnchorPoints_[cellI] =
    nonAnchorPoints(mesh().cellPoints()[cellI], cellAnchorPoints_[cellI], loop);
}


// Flip loop only
void mousse::cellCuts::flipLoopOnly(const label cellI)
{
  labelList& loop = cellLoops_[cellI];
  reverse(loop);
}
...
void mousse::cellCuts::writeCellOBJ(const fileName& dir, const label cellI) const
{
  const labelList& anchors = cellAnchorPoints_[cellI];
