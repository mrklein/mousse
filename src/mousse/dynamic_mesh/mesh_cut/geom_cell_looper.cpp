...
// edge endpoints
const mousse::scalar mousse::geomCellLooper::pointEqualTol_ = 1e-3;
// Snap cuts through edges onto edge endpoints. Fraction of edge length.
mousse::scalar mousse::geomCellLooper::snapTol_ = 0.1;
...
bool mousse::geomCellLooper::cutEdge
(
...
mousse::label mousse::geomCellLooper::snapToVert
(
  const scalar tol,
  const label edgeI,
  const scalar weight
...
void mousse::geomCellLooper::getBase(const vector& n, vector& e0, vector& e1)
const
{
...
// the edge end points.
bool mousse::geomCellLooper::edgeEndsCut
(
...
) const
{
  ...
  if (!isEdge(prevCut) && !isEdge(nextCut)) {
    ...
    if ((v0 == e[0] && v1 == e[1]) || (v0 == e[1] && v1 == e[0])) {
      return true;
    }
  }
  return false;
...
mousse::geomCellLooper::geomCellLooper(const polyMesh& mesh)
:
  cellLooper(mesh)
{}
...
// Destructor 
mousse::geomCellLooper::~geomCellLooper()
{}


// Member Functions 
bool mousse::geomCellLooper::cut
(
  const vector& refDir,
  const label cellI,
...
) const
{
  ...
  return cut(plane(mesh().cellCentres()[cellI], refDir), cellI, vertIsCut,
             edgeIsCut, edgeWeight, loop, loopWeights);
}
...
) const
{
  ...
  if (debug&2) {
    ...
    FOR_ALL(loop, i) {
      Pout << "At angle:" << sortedAngles[i] << endl << "    cut:";
      writeCut(Pout, loop[i], loopWeights[i]);
      Pout << "  coord:" << coord(loop[i], loopWeights[i]) << endl;
    }
  }
  return true;
}
