...
) const
{
  ...
  if (nSplitEdges == 0 && nModPoints == 0) {
    ...
    FOR_ALL(extendedFace, i) {
      ...
      if (newFace.size() > 2
          && (pointI >= mesh_.nPoints() || pointToPos.found(pointI))) {
...
void mousse::boundaryCutter::updateMesh(const mapPolyMesh& morphMap)
{
  ...
  {
    typedef HashTable<labelList, edge, Hash<edge>> HlLeHe;
    // Create copy since we're deleting entries
    HlLeHe newEdgeAddedPoints{edgeAddedPoints_.size()};
    for (HlLeHe::const_iterator iter = edgeAddedPoints_.begin();
      ...
      if (newStart >= 0 && newEnd >= 0) {
        ...
        label newI = 0;
        FOR_ALL(addedPoints, i) {
          label newAddedPointI = morphMap.reversePointMap()[addedPoints[i]];
          if (newAddedPointI >= 0) {
