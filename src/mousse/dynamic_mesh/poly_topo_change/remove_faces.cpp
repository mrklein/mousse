...
void mousse::removeFaces::changeCellRegion
(
  const label cellI,
...
mousse::label mousse::removeFaces::changeFaceRegion
(
    ...
    FOR_ALL(fEdges, i) {
      ...
      if (nFacesPerEdge[edgeI] >= 0 && nFacesPerEdge[edgeI] <= 2) {
        ...
        FOR_ALL(eFaces, j) {
          ...
          nChanged +=
            changeFaceRegion(cellRegion, removedFace, nFacesPerEdge,
                             nbrFaceI, newRegion, fEdges1, faceRegion);
        }
      }
    }
  }
...
// - removal of edges
// - removal of points
mousse::boolList mousse::removeFaces::getFacesAffected
(
  const labelList& cellRegion,
...
void mousse::removeFaces::writeOBJ
(
...
void mousse::removeFaces::mergeFaces
(
  const labelList& cellRegion,
  const labelList& cellRegionMaster,
  const labelHashSet& pointsToRemove,
...
void mousse::removeFaces::getFaceInfo
(
  const label faceI,
  label& patchID,
  label& zoneID,
...
mousse::face mousse::removeFaces::filterFace
(
  const labelHashSet& pointsToRemove,
  const label faceI
) const
...
void mousse::removeFaces::modFace
(
  const face& f,
...
// Constructors 
// Construct from mesh
mousse::removeFaces::removeFaces
(
  const polyMesh& mesh,
  const scalar minCos
)
...
mousse::label mousse::removeFaces::compatibleRemoves
(
  const labelList& facesToRemove,
  labelList& cellRegion,
...
void mousse::removeFaces::setRefinement
(
  const labelList& faceLabels,
  const labelList& cellRegion,
  const labelList& cellRegionMaster,
...
) const
{
...

  {
      ...
      if (nFacesPerEdge[edgeI] == 2) {
        ...
        if (f0 != -1 && f1 != -1) {
          ...
          if (patch0 != patch1) {
            ...
            if (mag(n0[f0 - pp0.start()] & n0[f1 - pp0.start()]) < minCos_) {
              WARNING_IN("removeFaces::setRefinement")
                << "not merging faces " << f0 << " and "
    ...
    syncTools::syncEdgeList(mesh_, nFacesPerEdge, maxEqOp<label>(), labelMin);
  ...
  // are only used by 2 unremoved edges.
  {
    ...
    syncTools::syncPointList(mesh_, nEdgesPerPoint, maxEqOp<label>(), labelMin);
    FOR_ALL(nEdgesPerPoint, pointI) {
  ...
  {
    getFacesAffected(cellRegion, cellRegionMaster, faceLabels, edgesToRemove,
                     pointsToRemove)
  };
  //
  // Now we know
