...
void mousse::polyTopoChange::renumberReverseMap
(
  const labelList& map,
...
void mousse::polyTopoChange::renumber
(
  const labelList& map,
  labelHashSet& elems
)
...
void mousse::polyTopoChange::renumberCompact
(
  const labelList& map,
  labelList& elems
)
...
void mousse::polyTopoChange::countMap
(
  const labelList& map,
...
mousse::labelHashSet mousse::polyTopoChange::getSetIndices
(
...
void mousse::polyTopoChange::writeMeshStats(const polyMesh& mesh, Ostream& os)
{
...
void mousse::polyTopoChange::getMergeSets
(
...
void mousse::polyTopoChange::checkFace
(
  const face& f,
  const label faceI,
  const label own,
...
void mousse::polyTopoChange::makeCells
(
...
// of faces
void mousse::polyTopoChange::makeCellCells
(
  const label nActiveFaces,
...
mousse::label mousse::polyTopoChange::getCellOrder
(
...
void mousse::polyTopoChange::getFaceOrder
(
  const label nActiveFaces,
...
void mousse::polyTopoChange::reorderCompactFaces
(
  const label newSize,
...
void mousse::polyTopoChange::compact
(
...
)
{
        ...
        if (own >= 0) {
          ...
          if (nei >= 0) {
            ...
            if (faceNeighbour_[faceI] >= 0
              ...
              flipFaceFlux_[faceI] = flipFaceFlux_[faceI] ? 0 : 1;
              faceZoneFlip_[faceI] = faceZoneFlip_[faceI] ? 0 : 1;
            }
          }
        } else if (nei >= 0) {
          // Update neighbour.
  ...
  // Reorder faces into upper-triangular and patch ordering
  {
    ...
    getFaceOrder(nActiveFaces_, cellFaces, cellFaceOffsets, localFaceMap,
...
mousse::labelList mousse::polyTopoChange::selectFaces
(
...
// label)
void mousse::polyTopoChange::calcPatchPointMap
(
    ...
    FOR_ALL(meshPoints, i) {
      if (meshPoints[i] < pointMap_.size()) {
        ...
        Map<label>::const_iterator ozmpmIter =
          oldMeshPointMap.find(pointMap_[meshPoints[i]]);
...
void mousse::polyTopoChange::calcFaceInflationMaps
(
  const polyMesh& mesh,
...
) const
{
  ...
  if (faceFromPoint_.size()) {
    ...
    FOR_ALL_CONST_ITER(Map<label>, faceFromPoint_, iter) {
      ...
      if (region_[newFaceI] == -1) {
        ...
        facesFromPoints[nFacesFromPoints++] =
          objectMap
          (
            newFaceI,
            selectFaces
  ...
  if (faceFromEdge_.size()) {
    ...
    FOR_ALL_CONST_ITER(Map<label>, faceFromEdge_, iter) {
      ...
      if (region_[newFaceI] == -1) {
        ...
        facesFromEdges[nFacesFromEdges++] =
          objectMap
          (
            newFaceI,
            selectFaces
  ...
  getMergeSets(reverseFaceMap_, faceMap_, facesFromFaces);
}


void mousse::polyTopoChange::calcCellInflationMaps
(
  const polyMesh& mesh,
...
) const
{
  ...
  if (cellFromPoint_.size()) {
    ...
    FOR_ALL_CONST_ITER(Map<label>, cellFromPoint_, iter) {
      cellsFromPoints[nCellsFromPoints++] =
        objectMap
        (
          iter.key(),
  ...
  if (cellFromEdge_.size()) {
    ...
    FOR_ALL_CONST_ITER(Map<label>, cellFromEdge_, iter) {
      cellsFromEdges[nCellsFromEdges++] =
        objectMap(iter.key(), mesh.edgeCells()[iter()]);
    }
  }
  ...
  if (cellFromFace_.size()) {
    ...
    FOR_ALL_CONST_ITER(Map<label>, cellFromFace_, iter) {
      ...
      if (mesh.isInternalFace(oldFaceI)) {
        ...
        cellsFromFaces[nCellsFromFaces++] = objectMap(iter.key(), twoCells);
      } else {
        cellsFromFaces[nCellsFromFaces++] =
          objectMap(iter.key(), labelList{1, mesh.faceOwner()[oldFaceI]});
      }
    }
  }
  // Cells from cell merging
  getMergeSets(reverseCellMap_, cellMap_, cellsFromCells);
}


void mousse::polyTopoChange::resetZones
(
  const polyMesh& mesh,
  polyMesh& newMesh,
...
) const
{
...

  {
    ...
    FOR_ALL(addressing, zoneI) {
      ...
      FOR_ALL(newZoneAddr, i) {
        if (newZoneAddr[i] < faceMap_.size()) {
          curFzFaceRnb[i] = oldZone.whichFace(faceMap_[newZoneAddr[i]]);
        } else {
          curFzFaceRnb[i] = -1;
        }
      }
    ...
    FOR_ALL(newMesh.faceZones(), zoneI) {
      ...
      newMesh.faceZones()[zoneI].resetAddressing(addressing[zoneI],
                                                 flipMode[zoneI]);
    }
  }
  // cellZones
    ...
    FOR_ALL(addressing, zoneI) {
      ...
      FOR_ALL(newZoneAddr, i) {
        if (newZoneAddr[i] < cellMap_.size()) {
          curCellRnb[i] = oldZone.whichCell(cellMap_[newZoneAddr[i]]);
        } else {
          curCellRnb[i] = -1;
        }
      }
...
void mousse::polyTopoChange::calcFaceZonePointMap
(
  const polyMesh& mesh,
  ...
  FOR_ALL(faceZones, zoneI) {
    ...
    FOR_ALL(newZoneMeshPoints, pointI) {
      if (newZoneMeshPoints[pointI] < pointMap_.size()) {
        Map<label>::const_iterator ozmpmIter =
          oldZoneMeshPointMap.find(pointMap_[newZoneMeshPoints[pointI]]);
...
void mousse::polyTopoChange::reorderCoupledFaces
(
...
void mousse::polyTopoChange::compactAndReorder
(
  const polyMesh& mesh,
  const bool syncParallel,
  const bool orderCells,
...
)
{
  ...
  getMergeSets(reversePointMap_, pointMap_, pointsFromPoints);
  calcFaceInflationMaps(mesh, facesFromPoints, facesFromEdges, facesFromFaces);
  calcCellInflationMaps(mesh, cellsFromPoints, cellsFromEdges, cellsFromFaces,
                        cellsFromCells);
...
mousse::polyTopoChange::polyTopoChange(const label nPatches, const bool strict)
:
  strict_{strict},
  nPatches_{nPatches},
  points_{0},
...
// Construct from components
mousse::polyTopoChange::polyTopoChange
(
  const polyMesh& mesh,
  const bool strict
)
...
// Member Functions 
void mousse::polyTopoChange::clear()
{
...
void mousse::polyTopoChange::addMesh
(
  const polyMesh& mesh,
...
)
{
  ...
  // Add points
  {
    ...
    for (label pointI = 0; pointI < mesh.nPoints(); pointI++) {
      addPoint(points[pointI], pointI, newZoneID[pointI], true);
    }
  }
  // Add cells
  {
...
void mousse::polyTopoChange::setCapacity
(
  const label nPoints,
  const label nFaces,
  const label nCells
...
mousse::label mousse::polyTopoChange::setAction(const topoAction& action)
{
...
mousse::label mousse::polyTopoChange::addPoint
(
  const point& pt,
...
void mousse::polyTopoChange::modifyPoint
(
  const label pointI,
  const point& pt,
...
void mousse::polyTopoChange::movePoints(const pointField& newPoints)
{
...
void mousse::polyTopoChange::removePoint
(
  const label pointI,
...
)
{
  ...
  if (strict_ && (pointRemoved(pointI) || pointMap_[pointI] == -1)) {
    FATAL_ERROR_IN("polyTopoChange::removePoint(const label, const label)")
...
mousse::label mousse::polyTopoChange::addFace
(
  const face& f,
  const label own,
  const label nei,
...
void mousse::polyTopoChange::modifyFace
(
  const face& f,
  const label faceI,
  const label own,
...
void mousse::polyTopoChange::removeFace(const label faceI, const label mergeFaceI)
{
  ...
  if (strict_ && (faceRemoved(faceI) || faceMap_[faceI] == -1)) {
    FATAL_ERROR_IN("polyTopoChange::removeFace(const label, const label)")
      << "face " << faceI
...
mousse::label mousse::polyTopoChange::addCell
(
  const label masterPointID,
  const label masterEdgeID,
  const label masterFaceID,
...
void mousse::polyTopoChange::modifyCell
(
  const label cellI,
  const label zoneID
)
...
void mousse::polyTopoChange::removeCell(const label cellI, const label mergeCellI)
{
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::polyTopoChange::changeMesh
(
  polyMesh& mesh,
  const bool inflate,
  const bool syncParallel,
...
)
{
  ...
  calcPatchPointMap(oldPatchMeshPointMaps, mesh.boundaryMesh(), patchPointMap);
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::polyTopoChange::makeMesh
(
...
)
{
...

  {
    FOR_ALL(oldPointZones, i) {
      pZonePtrs[i] = new pointZone{oldPointZones[i].name(), labelList(0), i,
                                   newMesh.pointZones()};
    }
  }
  ...
  calcPatchPointMap(oldPatchMeshPointMaps, newMesh.boundaryMesh(),
                    patchPointMap);
  // Create the face zone mesh point renumbering
