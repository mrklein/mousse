...
void mousse::refinementHistory::writeDebug
(
...
// Constructors 
//- Construct null
mousse::refinementHistory::splitCell8::splitCell8()
:
  parent_{-1},
  addedCellsPtr_{NULL}
{}
...
mousse::refinementHistory::splitCell8::splitCell8(const label parent)
:
  parent_{parent},
  addedCellsPtr_{NULL}
{}
...
mousse::refinementHistory::splitCell8::splitCell8(Istream& is)
{
  is >> *this;
}
...
mousse::refinementHistory::splitCell8::splitCell8(const splitCell8& sc)
:
  parent_{sc.parent_},
  addedCellsPtr_
  {
...
// Friend Operators 
mousse::Istream& mousse::operator>>(Istream& is, refinementHistory::splitCell8& sc)
{
...
)
{
  ...
  if (sc.addedCellsPtr_.valid()) {
    return os << sc.parent_ << token::SPACE << labelList(sc.addedCellsPtr_());
  } else {
...
mousse::label mousse::refinementHistory::allocateSplitCell
(
  const label parent,
  const label i
)
...
void mousse::refinementHistory::freeSplitCell(const label index)
{
...
void mousse::refinementHistory::markSplit
(
  const label index,
  labelList& oldToNew,
...
// Constructors 
mousse::refinementHistory::refinementHistory(const IOobject& io)
:
  regIOobject{io}
{
  // Temporary warning
...
//- Read or construct
mousse::refinementHistory::refinementHistory
(
  const IOobject& io,
  const List<splitCell8>& splitCells,
  const labelList& visibleCells
...
// Construct from Istream
mousse::refinementHistory::refinementHistory(const IOobject& io, Istream& is)
:
  regIOobject{io},
  splitCells_{is},
  freeSplitCells_{0},
...
// Member Functions 
void mousse::refinementHistory::resize(const label size)
{
...
void mousse::refinementHistory::updateMesh(const mapPolyMesh& map)
{
  if (!active())
    return;
...
void mousse::refinementHistory::countProc
(
  const label index,
...
void mousse::refinementHistory::distribute(const mapDistributePolyMesh& map)
{
  if (!active()) {
    FATAL_ERROR_IN
    (
  ...
  FOR_ALL(visibleCells_, cellI) {
    label index = visibleCells_[cellI];
    if (index >= 0) {
      countProc(splitCells_[index].parent_, destination[cellI], splitCellProc,
                splitCellNum);
    }
  }
  ...
  for (label procI = 0; procI < Pstream::nProcs(); procI++) {
    ...
    // Send to neighbours
    OPstream toNbr{Pstream::blocking, static_cast<int>(procI)};
  ...
  visibleCells_ = -1;
  for (label procI = 0; procI < Pstream::nProcs(); procI++) {
    IPstream fromNbr{Pstream::blocking, static_cast<int>(procI)};
...
void mousse::refinementHistory::compact()
{
  if (debug) {
  ...
  FOR_ALL(visibleCells_, cellI) {
    ...
    if (index >= 0) {
      ...
      // parent or subsplits.
      if (splitCells_[index].parent_ != -1
  ...
  FOR_ALL(splitCells_, index) {
    if (splitCells_[index].parent_ == -2) {
      // freed cell.
    } else if (splitCells_[index].parent_ == -1
...
void mousse::refinementHistory::writeDebug() const
{
  writeDebug(visibleCells_, splitCells_);
}


void mousse::refinementHistory::storeSplit
(
  const label cellI,
...
void mousse::refinementHistory::combineCells
(
...
bool mousse::refinementHistory::readData(Istream& is)
{
  is >> *this;
  return !is.bad();
}
...
bool mousse::refinementHistory::writeData(Ostream& os) const
{
  os << *this;
  return os.good();
}
...
// Friend Operators 
mousse::Istream& mousse::operator>>(Istream& is, refinementHistory& rh)
{
...
mousse::Ostream& mousse::operator<<(Ostream& os, const refinementHistory& rh)
{
  const_cast<refinementHistory&>(rh).compact();
  return os
    << "// splitCells" << nl << rh.splitCells_ << nl
    << "// visibleCells" << nl << rh.visibleCells_;
}
