...
void mousse::tetDecomposer::modifyFace
(
...
void mousse::tetDecomposer::addFace
(
  polyTopoChange& meshMod,
  const face& f,
  const label own,
...
mousse::label mousse::tetDecomposer::triIndex(const label faceI, const label fp)
const
{
...
// Constructors 
mousse::tetDecomposer::tetDecomposer(const polyMesh& mesh)
:
  mesh_{mesh}
{}
...
// Member Functions 
void mousse::tetDecomposer::setRefinement
(
...
)
{
  ...
  FOR_ALL(mesh_.cellCentres(), cellI) {
    ...
    cellToPoint_[cellI] =
      meshMod.addPoint(mesh_.cellCentres()[cellI], masterPointI, -1, true);
  }
  ...
  if (decomposeType == FACE_CENTRE_TRIS) {
    ...
    FOR_ALL(mesh_.faceCentres(), faceI) {
      ...
      faceToPoint_[faceI] =
        meshMod.addPoint(mesh_.faceCentres()[faceI], masterPointI, -1, true);
    }
  }
        ...
        if (edgeFnd == edgeToFace.end()) {
          ...
          if (decomposeType == FACE_CENTRE_TRIS) {
            ...
            if (mesh_.faceOwner()[otherFaceI] == cellI) {
              ...
              otherTet = faceNeighbourCells_[otherFaceI][otherTriI];
            }
          }
          addFace
            (
...
void mousse::tetDecomposer::updateMesh(const mapPolyMesh& map)
{
