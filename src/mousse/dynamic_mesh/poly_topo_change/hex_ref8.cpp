...
void mousse::hexRef8::getFaceInfo
(
  const label faceI,
  label& patchID,
  label& zoneID,
...
mousse::label mousse::hexRef8::addFace
(
...
// for the face)
mousse::label mousse::hexRef8::addInternalFace
(
  polyTopoChange& meshMod,
...
mousse::scalar mousse::hexRef8::getLevel0EdgeLength() const
{
  ...
  // 1. Look only at edges surrounded by cellLevel cells only.
  {
    ...
    // edges.
    syncTools::syncEdgeList(mesh_, edgeLevel, ifEqEqOp<labelMax>(), labelMin);
  ...
  FOR_ALL(typEdgeLenSqr, levelI) {
    ...
    if (lenSqr < GREAT2) {
      level0Size = mousse::sqrt(lenSqr)*(1<<levelI);
      if (debug) {
        Pout << "hexRef8::getLevel0EdgeLength() :"
          << " For level:" << levelI
          << " have edgeLen:" << mousse::sqrt(lenSqr)
...
mousse::label mousse::hexRef8::getAnchorCell
(
...
void mousse::hexRef8::getFaceNeighbours
(
  const labelListList& cellAnchorPoints,
  const labelListList& cellAddedCells,
  const label faceI,
...
{
  // Is owner split?
  own = getAnchorCell(cellAnchorPoints, cellAddedCells,
                      mesh_.faceOwner()[faceI], faceI, pointI);
  if (mesh_.isInternalFace(faceI)) {
    nei = getAnchorCell(cellAnchorPoints, cellAddedCells,
                        mesh_.faceNeighbour()[faceI], faceI, pointI);
  } else {
    nei = -1;
  }
}
...
mousse::label mousse::hexRef8::findMinLevel(const labelList& f) const
{
...
mousse::label mousse::hexRef8::findMaxLevel(const labelList& f) const
{
...
mousse::label mousse::hexRef8::countAnchors
(
  const labelList& f,
...
void mousse::hexRef8::dumpCell(const label cellI) const
{
  OFstream str{mesh_.time().path()/"cell_" + mousse::name(cellI) + ".obj"};
...
mousse::label mousse::hexRef8::findLevel
(
  const label faceI,
  const face& f,
  const label startFp,
...
mousse::label mousse::hexRef8::getAnchorLevel(const label faceI) const
{
  const face& f = mesh_.faces()[faceI];
  if (f.size() <= 4) {
...
void mousse::hexRef8::checkInternalOrientation
(
  polyTopoChange& meshMod,
  const label cellI,
  const label faceI,
...
void mousse::hexRef8::checkBoundaryOrientation
(
  polyTopoChange& meshMod,
  const label cellI,
  const label faceI,
...
// splitPoint.
void mousse::hexRef8::insertEdgeSplit
(
...
mousse::label mousse::hexRef8::storeMidPointInfo
(
  const labelListList& cellAnchorPoints,
  const labelListList& cellAddedCells,
...
) const
{
  ...
  if (changed && haveTwoAnchors && haveTwoFaceMids) {
    ...
    label anchorCell0 = getAnchorCell(cellAnchorPoints, cellAddedCells, cellI,
                                      faceI, anchorPointI);
    label anchorCell1 = getAnchorCell(cellAnchorPoints, cellAddedCells, cellI,
                                      faceI,
...
void mousse::hexRef8::createInternalFaces
(
  const labelListList& cellAnchorPoints,
  const labelListList& cellAddedCells,
  const labelList& cellMidPoint,
...
) const
{
  ...
  FOR_ALL(cFaces, i) {
    ...
    if (nAnchors == 1) {
      ...
      label edgeMid = findLevel(faceI, f, f.fcIndex(anchorFp), true, cLevel+1);
      label faceMid = findLevel(faceI, f, f.fcIndex(edgeMid), true, cLevel+1);
    ...
    FOR_ALL(f, fp0) {
      ...
      if (pointLevel_[point0] <= cLevel) {
        ...
        if (pointLevel_[f[fpMin1]] <= cLevel) {
          ...
          label edgeMid = findLevel(faceI, f, fpMin1, false, cLevel+1);
          edgeMidPointI = f[edgeMid];
        }
        newFaceI =
          storeMidPointInfo
...
void mousse::hexRef8::walkFaceToMid
(
  const labelList& edgeMidPoint,
  const label cLevel,
  const label faceI,
...
void mousse::hexRef8::walkFaceFromMid
(
  const labelList& edgeMidPoint,
  const label cLevel,
  const label faceI,
...
mousse::label mousse::hexRef8::faceConsistentRefinement
(
  const bool maxSet,
...
void mousse::hexRef8::checkWantedRefinementLevels
(
...
void mousse::hexRef8::setInstance(const fileName& inst)
{
  if (debug) {
...
void mousse::hexRef8::collectLevelPoints
(
  const labelList& f,
  const label level,
...
bool mousse::hexRef8::matchHexShape
(
  const label cellI,
  ...
  if (quads.size() < 6) {
      ...
      if (verts.size() == 1) {
        ...
        FOR_ALL(f, fp) {
          ...
          if (pointLevel_[pointI] == cellLevel+1) {
            Map<labelList>::iterator iter = pointFaces.find(pointI);
            if (iter != pointFaces.end()) {
              ...
              pointFaces.insert(pointI, labelList{1, faceI});
            }
          }
        }
      }
    ...
    FOR_ALL_CONST_ITER(Map<labelList>, pointFaces, iter) {
      ...
      if (pFaces.size() == 4) {
        ...
        primitivePatch bigFace(SubList<face>(fourFaces, fourFaces.size()),
                               mesh_.points());
...
mousse::hexRef8::hexRef8(const polyMesh& mesh, const bool readHistory)
:
  mesh_{mesh},
  cellLevel_
  {
...
:
  ...
  level0Edge_
  {
    ...
    {"level0Edge", dimLength, (level0Edge >= 0 ? level0Edge : getLevel0EdgeLength())}
  },
  history_
  {
    {
...
// Member Functions 
mousse::labelList mousse::hexRef8::consistentRefinement
(
  const labelList& cellsToRefine,
  const bool maxSet
) const
...
mousse::labelList mousse::hexRef8::consistentSlowRefinement
(
...
) const
{
  ...
  FOR_ALL(facesToCheck, i) {
    ...
    if (mesh_.isInternalFace(faceI)) {
      ...
      (
        refinementData(faceRefineCount, faceCount)
      );
  ...
  FaceCellWave<refinementData, int>
    levelCalc(mesh_, allFaceInfo, allCellInfo, dummyTrackData);
  while (true) {
    if (debug) {
    ...
    syncTools::syncPointList(mesh_, maxPointCount, maxEqOp<label>(), labelMin);
    ...
    FOR_ALL(pointsToCheck, i) {
      ...
      FOR_ALL(pCells, pCellI) {
        ...
        if (!cellInfo.isRefined()
            && (maxPointCount[pointI] > cellInfo.count() + maxFaceDiff*maxPointDiff)) {
  ...
  if (debug) {
    for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++) {
      label own = mesh_.faceOwner()[faceI];
      label ownLevel =
        cellLevel_[own] + (allCellInfo[own].isRefined() ? 1 : 0);
      label nei = mesh_.faceNeighbour()[faceI];
      label neiLevel =
        cellLevel_[nei] + (allCellInfo[nei].isRefined() ? 1 : 0);
      if (mag(ownLevel-neiLevel) > 1) {
        dumpCell(own);
        dumpCell(nei);
        FATAL_ERROR_IN
    ...
    FOR_ALL(neiLevel, i) {
      ...
      label ownLevel =
        cellLevel_[own] + (allCellInfo[own].isRefined() ? 1 : 0);
      label nbrLevel =
        neiLevel[i] + ((neiCount[i] >= neiRefCount[i]) ? 1 : 0);
...
mousse::labelList mousse::hexRef8::consistentSlowRefinement2
(
  const label maxFaceDiff,
  const labelList& cellsToRefine,
  const labelList& facesToCheck
) const
{
  ...
  FOR_ALL(facesToCheck, i) {
    ...
    if (!mesh_.isInternalFace(faceI)) {
      ...
      refinementDistanceData neiData(level0Size, 2*fc - cc[own], ownLevel+1);
      allFaceInfo[faceI].updateFace
        (
          mesh_,
          faceI,
  ...
  if (debug) {
    ...
    {
      cellSet cellsOut2{mesh_, "cellsToRefineOut2", newCellsToRefine.size()};
      FOR_ALL(refineCell, cellI) {
        if (refineCell.get(cellI)) {
...
mousse::labelListList mousse::hexRef8::setRefinement
(
...
)
{
  ...
  FOR_ALL(cellMidPoint, cellI) {
    if (cellMidPoint[cellI] >= 0) {
      ...
      FOR_ALL(cEdges, i) {
        label edgeI = cEdges[i];
        const edge& e = mesh_.edges()[edgeI];
        if (pointLevel_[e[0]] <= cellLevel_[cellI]
  ...
  syncTools::syncEdgeList(mesh_, edgeMidPoint, maxEqOp<label>(), labelMin);
  // Introduce edge points
  {
    ...
    syncTools::syncEdgePositions(mesh_, edgeMids, maxEqOp<vector>(),
  ...
  // Calculate face level (after splitting)
  if (debug) {
    Pout << "hexRef8::setRefinement :"
  ...
  for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++) {
    if (faceAnchorLevel[faceI] >= 0) {
      ...
      if (newOwnLevel > faceAnchorLevel[faceI]
  ...
  syncTools::syncFaceList(mesh_, faceMidPoint, maxEqOp<label>());
  // Introduce face points
  {
    // Phase 1: determine mid points and sync. See comment for edgeMids
    // above
    pointField bFaceMids(mesh_.nFaces()-mesh_.nInternalFaces(),
                         point(-GREAT, -GREAT, -GREAT));
    FOR_ALL(bFaceMids, i) {
      label faceI = i + mesh_.nInternalFaces();
    ...
    syncTools::syncBoundaryFacePositions(mesh_, bFaceMids, maxEqOp<vector>());
    ...
    if (faceMidPoint[faceI] >= 0 && affectedFace.get(faceI)) {
      ...
      FOR_ALL(f, fp) {
        ...
        if (pointLevel_[pointI] <= anchorLevel) {
          ...
          walkFaceToMid(edgeMidPoint, anchorLevel, faceI, fp, faceVerts);
          faceVerts.append(faceMidPoint[faceI]);
          walkFaceFromMid(edgeMidPoint, anchorLevel, faceI, fp, faceVerts);
          // Convert dynamiclist to face.
          newFace.transfer(faceVerts);
          // Get new owner/neighbour
          label own, nei;
          getFaceNeighbours(cellAnchorPoints, cellAddedCells, faceI, pointI,
                            own, nei);
          if (debug) {
            if (mesh_.isInternalFace(faceI)) {
              label oldOwn = mesh_.faceOwner()[faceI];
              label oldNei = mesh_.faceNeighbour()[faceI];
              checkInternalOrientation(meshMod, oldOwn, faceI,
                                       mesh_.cellCentres()[oldOwn],
                                       mesh_.cellCentres()[oldNei], newFace);
            } else {
              label oldOwn = mesh_.faceOwner()[faceI];
              checkBoundaryOrientation(meshMod, oldOwn, faceI,
                                       mesh_.cellCentres()[oldOwn],
                                       mesh_.faceCentres()[faceI], newFace);
            }
          }
          if (!modifiedFace) {
            modifiedFace = true;
...
void mousse::hexRef8::updateMesh(const mapPolyMesh& map)
{
...
// sure the pointLevel_ and cellLevel_ are the size of the new mesh so we
// only need to account for reordering.
void mousse::hexRef8::updateMesh
(
...
void mousse::hexRef8::distribute(const mapDistributePolyMesh& map)
{
  if (debug) {
...
void mousse::hexRef8::checkMesh() const
{
  ...
  // cells
  {
    ...
    FOR_ALL(patches, patchI) {
      ...
      if (pp.coupled()) {
        ...
        // be only one.
        HashTable<label, labelPair, labelPair::Hash<>> cellToFace{2*pp.size()};
...
void mousse::hexRef8::checkRefinementLevels
(
  const label maxPointDiff,
  const labelList& pointsToCheck
) const
{
  ...
  // Check pointLevel is synchronized
  {
    ...
    // Get min level
    syncTools::syncPointList(mesh_, syncPointLevel, minEqOp<label>(), labelMax);
  ...
  if (maxPointDiff != -1) {
    ...
    syncTools::syncPointList(mesh_, maxPointLevel, maxEqOp<label>(), labelMin);
    ...
    FOR_ALL(pointsToCheck, i) {
      ...
      FOR_ALL(pCells, i) {
        ...
        if(mag(cellLevel_[cellI]-maxPointLevel[pointI]) > maxPointDiff) {
          dumpCell(cellI);
          FATAL_ERROR_IN
          (
            "hexRef8::checkRefinementLevels(const label)"
...
const mousse::cellShapeList& mousse::hexRef8::cellShapes() const
{
  if (cellShapesPtr_.empty()) {
    ...
    FOR_ALL(cellLevel_, cellI) {
      if (meshShapes[cellI].model().index() == 0) {
        ...
        DynamicList<face> quads;
        bool haveQuads = matchHexShape(cellI, level, quads);
        if (haveQuads) {
...
// Unrefinement
mousse::labelList mousse::hexRef8::getSplitPoints() const
{
  if (debug) {
    checkRefinementLevels(-1, labelList(0));
  }
  ...
  FOR_ALL(visibleCells, cellI) {
    ...
    if (visibleCells[cellI] != -1 && history_.parentIndex(cellI) >= 0) {
      ...
      FOR_ALL(cPoints, i) {
        ...
        } else if ((masterCellI != parentIndex)
                   || (splitMasterLevel[pointI] != cellLevel_[cellI] - 1)) {
...
mousse::labelList mousse::hexRef8::consistentUnrefinement
(
...
void mousse::hexRef8::setUnrefinement
(
...
bool mousse::hexRef8::write() const
{
