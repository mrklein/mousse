...
mousse::autoPtr<mousse::fvMesh> mousse::polyMeshFilter::copyMesh(const fvMesh& mesh)
{
  ...
  autoPtr<mapPolyMesh> mapPtr =
    originalMeshToNewMesh.makeMesh
    (
      meshCopy,
      {
        mesh.name(),
...
mousse::label mousse::polyMeshFilter::filterFacesLoop(const label /*nOriginalBadFaces*/)
{
  ...
  edgeCollapser::checkMeshQuality(mesh_, meshQualityCoeffDict(), newErrorPoint);
  ...
  while (nOuterIterations < maxIterations() && newBadFaces) {
...

    {
      ...
      while (true) {
        ...
        label nLocalCollapse =
          filterFaces(newMesh, newMeshFaceFilterFactor, origToCurrentPointMap);
        Info << decrIndent;
        if (nLocalCollapse >= nPrevLocalCollapse || nLocalCollapse == 0) {
          Info << decrIndent;
          break;
        } else {
...

    {
      ...
      while (true) {
        Info << nl << indent << "Inner iteration = "
          << nInnerIterations++ << nl << incrIndent << endl;
        label nLocalCollapse =
          filterEdges(newMesh, newMeshMinEdgeLen, origToCurrentPointMap);
        Info << decrIndent;
        if (nLocalCollapse >= nPrevLocalCollapse || nLocalCollapse == 0) {
          Info << decrIndent;
          break;
    ...
    if (controlMeshQuality()) {
      ...
      nBadFaces =
        edgeCollapser::checkMeshQuality(newMesh, meshQualityCoeffDict(),
                                        isErrorPoint);
      Info << nl << "    Number of bad faces     : " << nBadFaces << nl
        << "    Number of marked points : "
        << returnReduce(isErrorPoint.count(), sumOp<unsigned int>())
        << endl;
      updatePointErrorCount(isErrorPoint, origToCurrentPointMap,
                            pointErrorCount);
      checkMeshEdgesAndRelaxEdges(newMesh, origToCurrentPointMap,
                                  isErrorPoint, pointErrorCount);
      checkMeshFacesAndRelaxEdges(newMesh, origToCurrentPointMap,
                                  isErrorPoint, pointErrorCount);
      newBadFaces = false;
      FOR_ALL(mesh_.points(), pI) {
        if (origToCurrentPointMap[pI] >= 0
...
)
{
  ...
  {
    // Collapse faces
    labelPair nCollapsedPtEdge =
      collapser.markSmallSliverFaces(newMeshFaceFilterFactor, pointPriority_(),
                                     collapseEdge, collapsePointToLocation);
    label nCollapsed = 0;
    ...
    Info << indent << "Collapsing " << nCollapsed << " faces "
  ...
  collapser.consistentCollapse(globalPoints, pointPriority_(),
                               collapsePointToLocation, collapseEdge,
                               allPointInfo);
...

  {
    ...
    autoPtr<mapPolyMesh> newMapPtr =
      newMeshMod.changeMesh(newMesh, false);
    ...
    mapOldMeshFaceFieldToNewMesh(newMesh, newMap.faceMap(),
                                 newMeshFaceFilterFactor);
    updateOldToNewPointMap(newMap.reversePointMap(), origToCurrentPointMap);
  }
...
)
{
  ...
  label nSmallCollapsed =
    collapser.markSmallEdges(newMeshMinEdgeLen, pointPriority_(),
                             collapseEdge, collapsePointToLocation);
  reduce(nSmallCollapsed, sumOp<label>());
  Info << indent << "Collapsing " << nSmallCollapsed << " small edges" << endl;
  // Merge inline edges
  label nMerged =
    collapser.markMergeEdges(maxCos(), pointPriority_(), collapseEdge,
  ...
  Info << indent << "Apply changes to the current mesh" << endl;
  // Apply changes to current mesh
  autoPtr<mapPolyMesh> newMapPtr = newMeshMod.changeMesh(newMesh, false);
  const mapPolyMesh& newMap = newMapPtr();
  // Update fields
  newMesh.updateMesh(newMap);
  if (newMap.hasMotionPoints()) {
  ...
  mapOldMeshEdgeFieldToNewMesh(newMesh, newMap.pointMap(), newMeshMinEdgeLen);
  updateOldToNewPointMap(newMap.reversePointMap(), origToCurrentPointMap);
  updatePointPriorities(newMesh, newMap.pointMap());
  return nLocalCollapse;
}
...
)
{
  ...
  FOR_ALL(edges, edgeI) {
    ...
    if (pointErrorCount[e[0]] >= maxPointErrorCount()
        || pointErrorCount[e[1]] >= maxPointErrorCount()) {
      minEdgeLen_[edgeI] = -1;
    }
    if ((newStart >= 0 && isErrorPoint[newStart])
  ...
  for (label smoothIter = 0; smoothIter < maxSmoothIters(); ++smoothIter) {
    ...
    FOR_ALL(mesh_.edges(), edgeI) {
      ...
      minEdgeLen_[edgeI] = min(minEdgeLen_[edgeI], sumMinEdgeLen/nEdges);
    }
    syncTools::syncEdgeList(mesh_, minEdgeLen_, minEqOp<scalar>(), scalar(0.0));
  }
}
...
)
{
  ...
  for (label smoothIter = 0; smoothIter < maxSmoothIters(); ++smoothIter) {
    ...
    FOR_ALL(faces, faceI) {
      ...
      faceFilterFactor_[faceI] = min(faceFilterFactor_[faceI],
...
)
{
  ...
  syncTools::syncPointList(newMesh, newPointPriority, maxEqOp<label>(),
                           labelMin);
...
) const
{
  ...
  FOR_ALL(newEdges, newEdgeI) {
    ...
    tmp[newEdgeI] = min(newMeshMinEdgeLen[pointMap[pStart]],
                        newMeshMinEdgeLen[pointMap[pEnd]]);
  }
  newMeshMinEdgeLen.transfer(tmp);
  syncTools::syncEdgeList(newMesh, newMeshMinEdgeLen, maxEqOp<scalar>(),
                          scalar(0.0));
}
...
) const
{
  ...
  syncTools::syncFaceList(newMesh, newMeshFaceFilterFactor, maxEqOp<scalar>());
}
...
)
{
  ...
  // This will iterate ultimately to the situation where every edge is
  // frozen and nothing gets collapsed.
  while (nOuterIterations < maxIterations()
    ...
    label nPrevLocalCollapse = labelMax;
    while (true) {
      Info << nl << indent << "Inner iteration = " << nInnerIterations++ << nl
        << incrIndent << endl;
      label nLocalCollapse = filterEdges(newMesh, newMeshMinEdgeLen,
    ...
    if (controlMeshQuality()) {
      PackedBoolList isErrorPoint{newMesh.nPoints()};
      nBadFaces = edgeCollapser::checkMeshQuality(newMesh,
      ...
      updatePointErrorCount(isErrorPoint, origToCurrentPointMap,
                            pointErrorCount);
      checkMeshEdgesAndRelaxEdges(newMesh, origToCurrentPointMap, isErrorPoint,
                                  pointErrorCount);
    } else {
      return -1;
    }
