...
const mousse::scalar mousse::perfectInterface::tol_ = 1e-3;


// Private Member Functions 
mousse::pointField mousse::perfectInterface::calcFaceCentres
(
...
mousse::perfectInterface::perfectInterface
(
  const word& name,
  const label index,
...
void mousse::perfectInterface::setRefinement
(
...
) const
{
  ...
  if (debug) {
    Pout
      << "typDim:" << typDim << " edges0:" << edges0.size()
...
void mousse::perfectInterface::setRefinement(polyTopoChange& ref) const
{
  if (debug) {
  ...
  if (masterPatchID_.active() && slavePatchID_.active()
      && faceZoneID_.active()) {
    const polyMesh& mesh = topoChanger().mesh();
    const polyBoundaryMesh& patches = mesh.boundaryMesh();
...
void mousse::perfectInterface::modifyMotionPoints
(
...
void mousse::perfectInterface::write(Ostream& os) const
{
  os << nl << type() << nl
    << name()<< nl
    << faceZoneID_.name() << nl
...
void mousse::perfectInterface::writeDict(Ostream& os) const
{
