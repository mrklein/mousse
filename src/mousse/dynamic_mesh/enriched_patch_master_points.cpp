...
void mousse::enrichedPatch::calcMasterPointFaces() const
{
  ...
  FOR_ALL(masterPatch_, faceI) {
    ...
    FOR_ALL(curFace, pointI) {
      Map<DynamicList<label>>::iterator mpfIter = mpf.find(curFace[pointI]);
      if (mpfIter == mpf.end()) {
  ...
  FOR_ALL(slavePointFaceHits_, pointI) {
    if (slavePointPointHits_[pointI] < 0
        && slavePointEdgeHits_[pointI] < 0
        && slavePointFaceHits_[pointI].hit()) {
      // Get the index of projected point corresponding to this slave
      // point
      const label mergedSmp = pointMergeMap().find(slaveMeshPoints[pointI])();
      Map<DynamicList<label>>::iterator mpfIter = mpf.find(mergedSmp);
      if (mpfIter == mpf.end()) {
        // Not found, add new dynamic list
        mpf.insert
          (
        ...
        // Iterator is invalidated - have to find again
        mpf.find(mergedSmp)().append(slavePointFaceHits_[pointI].hitObject());
      } else {
