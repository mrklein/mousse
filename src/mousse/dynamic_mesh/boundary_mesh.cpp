...
mousse::label mousse::boundaryMesh::nFeatureEdges(label pointI) const
{
  label nFeats = 0;
...
mousse::label mousse::boundaryMesh::nextFeatureEdge
(
  const label edgeI,
  const label vertI
) const
...
// featVisited.
mousse::labelList mousse::boundaryMesh::collectSegment
(
...
void mousse::boundaryMesh::markEdges
(
...
) const
{
  if (distance < maxDistance) {
    ...
    FOR_ALL(startEdges, pEdgeI) {
      markEdges(maxDistance, startEdges[pEdgeI], distance+1, minDistance,
                visited);
    }
    // Do edges connected to e.end
    const labelList& endEdges = mesh().pointEdges()[e.end()];
    FOR_ALL(endEdges, pEdgeI) {
      markEdges(maxDistance, endEdges[pEdgeI], distance+1, minDistance,
                visited);
    }
  }
}
...
mousse::label mousse::boundaryMesh::findPatchID
(
...
mousse::label mousse::boundaryMesh::whichPatch
(
  const polyPatchList& patches,
  const label faceI
) const
...
mousse::labelList mousse::boundaryMesh::faceToEdge
(
...
mousse::labelList mousse::boundaryMesh::edgeToFace
(
  const label region,
...
void mousse::boundaryMesh::markZone
(
...
) const
{
  ...
  while (true) {
    changedEdges = faceToEdge(borderEdge, currentZone, changedFaces, edgeZone);
    if (debug) {
...
// Constructors 
// Null constructor
mousse::boundaryMesh::boundaryMesh()
:
  meshPtr_{NULL},
  patches_{},
  meshFace_{},
...
void mousse::boundaryMesh::clearOut()
{
  if (meshPtr_) {
    delete meshPtr_;
    meshPtr_ = NULL;
...
// Member Functions 
void mousse::boundaryMesh::read(const polyMesh& mesh)
{
  patches_.clear();
  ...
  FOR_ALL(mesh.boundaryMesh(), patchI) {
    ...
    (
      patchI,
      new boundaryPatch{pp.name(), patchI, pp.size(), bFaceI, pp.type()}
    );
  ...
  PrimitivePatch<face, List, const pointField&>
    globalPatch{bFaces, mesh.points()};
...
void mousse::boundaryMesh::readTriSurface(const fileName& fName)
{
  ...
  if (surfPatches.size() == regionToBoundaryPatch.size()) {
    ...
    FOR_ALL(surfPatches, patchI) {
      ...
      (
        patchI,
        new boundaryPatch{surfPatch.name(), patchI, 0, 0,
    ...
    FOR_ALL(patches_, patchI) {
      ...
      (
        patchI,
        new boundaryPatch{"patch" + name(patchI), patchI, 0, 0, "empty"}
      );
    }
  }
  //
...
void mousse::boundaryMesh::writeTriSurface(const fileName& fName) const
{
  ...
  FOR_ALL(patches_, patchI) {
    const boundaryPatch& bp = patches_[patchI];
    surfPatches[patchI] =
      geometricSurfacePatch{bp.physicalType(), bp.name(), patchI};
  }
  //
...
mousse::labelList mousse::boundaryMesh::getNearest
(
...
) const
{
  ...
  uindirectPrimitivePatch leftPatch{UIndirectList<face>{mesh(), leftFaces},
                                    mesh().points()};
  uindirectPrimitivePatch rightPatch{UIndirectList<face>{mesh(), rightFaces},
                                     mesh().points()};
  // Overall bb
  ...
  const scalar planarTol =
    indexedOctree<treeDataPrimitivePatch<uindirectPrimitivePatch>>::perturbTol();
  // Create the octrees
  indexedOctree
  <
  ...
  FOR_ALL(nearestBFaceI, patchFaceI) {
    ...
    if (rightInfo.hit()) {
      if (leftInfo.hit()) {
        ...
        // if ((rightSign > 0 && leftSign > 0) || (rightSign < 0 && leftSign < 0)) {
        if (rightSign*leftSign > 0) {
          // Both same sign. Choose nearest.
          nearestBFaceI[patchFaceI] =
            (rightDist < leftDist) ? rightFaceI : leftFaceI;
        } else {
          // Differing sign.
          ...
          if (rightDist < typDim && leftDist < typDim) {
            ...
            nearestBFaceI[patchFaceI] =
              (rightSign > 0) ? rightFaceI : leftFaceI;
          } else {
...
void mousse::boundaryMesh::patchify
(
...
) const
{
  ...
  if (newPatchPtrList.size()) {
    ...
    label nAvgFaces =
      (newMesh.nFaces() - newMesh.nInternalFaces())/nNewPatches;
...
void mousse::boundaryMesh::setFeatureEdges(const scalar minCos)
{
  ...
  do {
    ...
    segments.append
    (
      collectSegment(isFeaturePoint, featureToEdge_[startFeatI], featVisited)
    );
  } while (true);
  //
  // Store in *this
...
void mousse::boundaryMesh::setExtraEdges(const label edgeI)
{
...
void mousse::boundaryMesh::addPatch(const word& patchName)
{
  patches_.setSize(patches_.size() + 1);
  // Add empty patch at end of patch list.
  label patchI = patches_.size()-1;
  boundaryPatch* bpPtr =
    new boundaryPatch{patchName, patchI, 0, mesh().size(), "empty"};
...
void mousse::boundaryMesh::deletePatch(const word& patchName)
{
...
void mousse::boundaryMesh::changePatchType
(
  const word& patchName,
...
)
{
  ...
  FOR_ALL(patches_, patchI) {
    if (patchI == changeI) {
      ...
      boundaryPatch* bpPtr =
        new boundaryPatch{oldBp.name(), oldBp.index(), oldBp.size(),
                          oldBp.start(), patchType};
...
void mousse::boundaryMesh::changeFaces
(
...
)
{
  ...
  FOR_ALL(patches_, patchI) {
    ...
    (
      patchI,
      new boundaryPatch{bp.name(), patchI, nFaces[patchI], startFace[patchI],
                        bp.physicalType()}
    );
  }
  patches_ = newPatches;
...
mousse::label mousse::boundaryMesh::getNTris(const label faceI) const
{
...
mousse::label mousse::boundaryMesh::getNTris
(
...
void mousse::boundaryMesh::triangulate
(
  const label startFaceI,
  const label nFaces,
...
mousse::label mousse::boundaryMesh::getNPoints
(
  const label startFaceI,
  const label nFaces
) const
...
void mousse::boundaryMesh::triangulateLocal
(
  const label startFaceI,
  const label nFaces,
  const label totalNTris,
...
void mousse::boundaryMesh::markFaces
(
...
) const
{
  ...
  FOR_ALL(currentZone, faceI) {
    visited[faceI] = currentZone[faceI] == 0;
    /*
