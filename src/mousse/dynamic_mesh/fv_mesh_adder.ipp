...
)
{
...

  {
    ...
    for (label newPatchI = nUsedPatches;
         newPatchI < bfld.size();
         newPatchI++) {
...
{
  typedef GeometricField<Type, fvPatchField, volMesh> GF;
  HashTable<const GF*> fields
  {
    mesh.objectRegistry::lookupClass<GF>()
  };
  HashTable<const GF*> fieldsToAdd
  {
    meshToAdd.objectRegistry::lookupClass<GF>()
  };
  // It is necessary to enforce that all old-time fields are stored
  // before the mapping is performed.  Otherwise, if the
  // old-time-level field is mapped before the field itself, sizes
  // will not match.
  for (typename HashTable<const GF*>:: iterator fieldIter = fields.begin();
       fieldIter != fields.end();
       ++fieldIter) {
    const_cast<GF*>(fieldIter())->storeOldTimes();
  }
  for (typename HashTable<const GF*>::iterator fieldIter = fields.begin();
       fieldIter != fields.end();
       ++fieldIter) {
    GF& fld = const_cast<GF&>(*fieldIter());
    if (fieldsToAdd.found(fld.name())) {
      const GF& fldToAdd = *fieldsToAdd[fld.name()];
...
)
{
  ...
  // Patch fields from old mesh
  {
    ...
    FOR_ALL(oldPatchMap, patchI) {
      label newPatchI = oldPatchMap[patchI];
      oldToNew[patchI] = (newPatchI != -1) ? newPatchI : unusedPatchI++;
#if 0
      if (newPatchI != -1) {
        oldToNew[patchI] = newPatchI;
      } else {
    ...
    bfld.setSize(mesh.boundaryMesh().size());
    // Delete unused patches
    for (label newPatchI = nUsedPatches; newPatchI < bfld.size(); newPatchI++) {
      bfld.set(newPatchI, NULL);
    }
    // Map old values
    FOR_ALL(oldPatchMap, patchI) {
  ...
  // Patch fields from added mesh
  {
    ...
    FOR_ALL(addedPatchMap, patchI) {
      ...
      if (newPatchI != -1) {
        const polyPatch& newPatch = mesh.boundaryMesh()[newPatchI];
        const polyPatch& oldPatch = fldToAdd.mesh().boundaryMesh()[patchI];
        if (!bfld(newPatchI)) {
          // First occurrence of newPatchI. Map from existing
          // patchField
          // From new patch faces to patch faces on added mesh.
...
)
{
  ...
  // old-time-level field is mapped before the field itself, sizes
  // will not match.
  for (typename HashTable<const fldType*>::iterator fieldIter = fields.begin();
       fieldIter != fields.end();
       ++fieldIter) {
