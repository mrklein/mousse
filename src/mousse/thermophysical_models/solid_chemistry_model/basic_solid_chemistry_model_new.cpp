...
)
{
  ...
  if (cstrIter == fvMeshConstructorTablePtr_->end()) {
    ...
    wordList validChemistryTypeNames{fvMeshConstructorTablePtr_->sortedToc()};
    Info << validChemistryTypeNames << endl;
    // Build a table of the thermo packages constituent parts
    // Note: row-0 contains the names of constituent parts
    List<wordList>
      validChemistryTypeNameCmpts{validChemistryTypeNames.size() + 1};
    ...
    FOR_ALL(validChemistryTypeNames, i) {
      validChemistryTypeNameCmpts[i+1] =
        basicThermo::splitThermoName(validChemistryTypeNames[i], nCmpt);
    }
