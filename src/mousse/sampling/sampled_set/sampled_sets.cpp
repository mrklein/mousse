...
)
{
  ...
  FOR_ALL(sampledSets, setI) {
    ...
    List<point> allPts
    {
      ListListOps::combine<List<point>>(gatheredPts, accessOp<List<point>>())
    };
    labelList allSegments
    {
      ListListOps::combine<labelList>(gatheredSegments, accessOp<labelList>())
    };
    scalarList allCurveDist
    {
      ListListOps::combine<scalarList>(gatheredDist, accessOp<scalarList>())
    };
...
void mousse::sampledSets::write()
{
  ...
  if (Pstream::master()) {
    if (debug) {
      Pout
        << "timeName = " << mesh_.time().timeName() << nl
    ...
    if (nFields) {
      if (debug) {
        Pout << "Creating directory " << outputPath_/mesh_.time().timeName()
            << nl << endl;
      }
