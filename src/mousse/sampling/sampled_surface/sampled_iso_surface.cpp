...
void mousse::sampledIsoSurface::getIsoFields() const
{
  ...
  if (!subMeshPtr_.valid()) {
    ...
    if (subFvm.foundObject<volScalarField>(isoField_)) {
      ...
      (
        subMeshPtr_().interpolate(*volFieldPtr_).ptr()
      );
    ...
    word pointFldName =
      "volPointInterpolate(" + volSubFieldPtr_->name() + ')';
    if (subFvm.foundObject<pointScalarField>(pointFldName)) {
      ...
      (
        volPointInterpolation::New(subFvm).interpolate(*volSubFieldPtr_).ptr()
      );
