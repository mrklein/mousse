...
mousse::graph mousse::noiseFFT::pt() const
{
  ...
  return graph{"p(t)", "t [s]", "p(t) [Pa]", t, *this};
}
...
) const
{
  tmp<scalarField> tPn2
  {
    mag
      (
        fft::reverseTransform(ReComplexField(tpn), labelList{1, tpn().size()})
      )
  };
  tpn.clear();
  tmp<scalarField> tPn
  {
    new scalarField{scalarField::subField(tPn2(), tPn2().size()/2)}
  };
  scalarField& Pn = tPn();
...
) const
{
  ...
  return graph{"P(f)", "f [Hz]", "P(f) [Pa]", f, MeanPf};
}
...
) const
{
  ...
  return graph{"P(f)", "f [Hz]", "P(f) [Pa]", f, RMSMeanPf};
}


mousse::graph mousse::noiseFFT::Lf(const graph& gPf) const
{
  return graph{"L(f)", "f [Hz]", "L(f) [dB]", gPf.x(), 20*log10(gPf.y()/p0)};
}
...
) const
{
  ...
  fm.setSize(j);
  ldelta.setSize(j);
  return graph{"Ldelta", "fm [Hz]", "Ldelta [dB]", fm, ldelta};
}
...
) const
{
  ...
  scalar fl = f1/sqrt(fratio);
  scalar fu = fratio*fl;
  label istart = label{static_cast<label>(fl/deltaf + 1.0 - SMALL)};
  label j = 0;
  ...
  fm.setSize(j);
  pdelta.setSize(j);
  return graph{"Pdelta", "fm [Hz]", "Pdelta [dB]", fm, pdelta};
}
