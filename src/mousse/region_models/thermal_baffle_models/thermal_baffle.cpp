...
void thermalBaffle::solveEnergy()
{
  ...
  if (oneD_ && !constantThickness_) {
    ...
    FOR_ALL(ppCoupled, localFaceI) {
      ...
      FOR_ALL(cells, i) {
        ...
        Q[cellId] =
          Qs_.boundaryField()[patchI][localFaceI]/thickness_[localFaceI];
  ...
  if (moveMesh_) {
    surfaceScalarField phiMesh{fvc::interpolate(rho*h_)*regionMesh().phi()};
...
:
  ...
  Qs_
  {
    ...
    regionMesh(),
    {"zero", dimEnergy/dimArea/dimTime, pTraits<scalar>::zero}
  },
  Q_
  {
    {
    ...
    regionMesh(),
    {"zero", dimEnergy/dimVolume/dimTime, pTraits<scalar>::zero}
  },
  radiation_
  {
    radiation::radiationModel::New(dict.subDict("radiation"), thermo_->T())
  }
{
  init();
  thermo_->correct();
...
:
  ...
  radiation_
  {
    radiation::radiationModel::New(thermo_->T())
  }
{
  init();
  thermo_->correct();
