...
DEFINE_TYPE_NAME_AND_DEBUG(vtkUnstructuredReader, 1);
...
void mousse::vtkUnstructuredReader::readField
(
  ISstream& inFile,
  objectRegistry& obj,
...
// read fields
mousse::wordList mousse::vtkUnstructuredReader::readFieldArray
(
  ISstream& inFile,
  objectRegistry& obj,
...
) const
{
  ...
  for (label i = 0; i < numArrays; i++) {
    ...
    if (debug) {
      Info << "Reading field " << arrayName << " of " << numTuples
        << " tuples of rank " << numComp << endl;
    }
...
mousse::objectRegistry& mousse::vtkUnstructuredReader::selectRegistry
(
...
void mousse::vtkUnstructuredReader::read(ISstream& inFile)
{
  ...
  while (inFile.good()) {
    ...
    if (debug) {
      Info << "line:" << inFile.lineNumber() << " tag:" << tag << endl;
    }
    if (tag == "DATASET") {
