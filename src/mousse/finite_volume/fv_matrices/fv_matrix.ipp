...
void mousse::fvMatrix<Type>::addToInternalField
(
...
template<class Type>
template<class Type2>
void mousse::fvMatrix<Type>::subtractFromInternalField
(
  const labelUList& addr,
  const Field<Type2>& pf,
  Field<Type2>& intf
...
template<class Type>
void mousse::fvMatrix<Type>::addBoundaryDiag
(
  scalarField& diag,
  const direction solveCmpt
) const
{
  FOR_ALL(internalCoeffs_, patchI) {
    addToInternalField
    (
...
template<class Type>
void mousse::fvMatrix<Type>::addCmptAvBoundaryDiag(scalarField& diag) const
{
  FOR_ALL(internalCoeffs_, patchI) {
    addToInternalField
    (
...
template<class Type>
void mousse::fvMatrix<Type>::addBoundarySource
(
  Field<Type>& source,
  const bool couples
) const
{
  FOR_ALL(psi_.boundaryField(), patchI) {
...
void mousse::fvMatrix<Type>::setValuesFromList
(
      ...
      FOR_ALL(c, j) {
        ...
        if (mesh.isInternalFace(facei)) {
          ...
          if (internalCoeffs_[patchi].size()) {
            label patchFacei = mesh.boundaryMesh()[patchi].whichFace(facei);
            internalCoeffs_[patchi][patchFacei] = pTraits<Type>::zero;
            boundaryCoeffs_[patchi][patchFacei] = pTraits<Type>::zero;
          }
        }
      }
    }
  ...
  faceFluxCorrectionPtr_{NULL}
{
  ...
  FOR_ALL(psi.mesh().boundary(), patchI) {
    internalCoeffs_.set
    (
      patchI,
      new Field<Type>
...
template<class Type>
mousse::fvMatrix<Type>::fvMatrix(const fvMatrix<Type>& fvm)
:
  refCount{},
  lduMatrix{fvm},
  psi_{fvm.psi_},
...
template<class Type>
mousse::fvMatrix<Type>::fvMatrix(const tmp<fvMatrix<Type>>& tfvm)
:
  refCount{},
  lduMatrix
  {
...
// Member Functions 
template<class Type>
void mousse::fvMatrix<Type>::setValues
(
  const labelUList& cellLabels,
...
template<class Type>
void mousse::fvMatrix<Type>::setReference
(
  const label celli,
  const Type& value,
...
template<class Type>
void mousse::fvMatrix<Type>::relax(const scalar alpha)
{
  if (alpha <= 0) {
    return;
  }
  if (debug) {
    INFO_IN("fvMatrix<Type>::relax(const scalar alpha)")
...
template<class Type>
void mousse::fvMatrix<Type>::relax()
{
  word name = psi_.select
  (
    psi_.mesh().data::template lookupOrDefault<bool>("finalIteration", false)
  );
...
template<class Type>
void mousse::fvMatrix<Type>::boundaryManipulate
(
  typename GeometricField<Type, fvPatchField, volMesh>::
    GeometricBoundaryField& bFields
)
{
  FOR_ALL(bFields, patchI) {
...
template<class Type>
mousse::tmp<mousse::scalarField> mousse::fvMatrix<Type>::D() const
{
...
template<class Type>
mousse::tmp<mousse::Field<Type>> mousse::fvMatrix<Type>::DD() const
{
...
template<class Type>
mousse::tmp<mousse::volScalarField> mousse::fvMatrix<Type>::A() const
{
...
template<class Type>
mousse::tmp<mousse::volScalarField> mousse::fvMatrix<Type>::H1() const
{
...
mousse::fvMatrix<Type>::
flux() const
{
  if (!psi_.mesh().fluxRequired(psi_.name())) {
    FATAL_ERROR_IN("fvMatrix<Type>::flux()")
  ...
  FOR_ALL(InternalContrib, patchI) {
  ...
  FOR_ALL(NeighbourContrib, patchI) {
  ...
  FOR_ALL(fieldFlux.boundaryField(), patchI) {
...
// Member Operators 
template<class Type>
void mousse::fvMatrix<Type>::operator=(const fvMatrix<Type>& fvmv)
{
  if (this == &fvmv) {
    FATAL_ERROR_IN("fvMatrix<Type>::operator=(const fvMatrix<Type>&)")
...
template<class Type>
void mousse::fvMatrix<Type>::operator=(const tmp<fvMatrix<Type>>& tfvmv)
{
  operator=(tfvmv());
  tfvmv.clear();
}
...
template<class Type>
void mousse::fvMatrix<Type>::negate()
{
  lduMatrix::negate();
  source_.negate();
...
template<class Type>
void mousse::fvMatrix<Type>::operator+=
(
...
)
{
  ...
  source_ *= dsf.field();
  FOR_ALL(boundaryCoeffs_, patchI) {
    scalarField pisf
    {
...
template<class Type>
mousse::solverPerformance mousse::solve
(
  fvMatrix<Type>& fvm,
...
template<class Type>
mousse::solverPerformance mousse::solve(fvMatrix<Type>& fvm)
{
  return fvm.solve();
}
...
template<class Type>
mousse::solverPerformance mousse::solve(const tmp<fvMatrix<Type>>& tfvm)
{
  solverPerformance solverPerf =
...
template<class Type>
mousse::tmp<mousse::fvMatrix<Type>> mousse::correction
(
  const fvMatrix<Type>& A
)
{
  tmp<mousse::fvMatrix<Type>> tAcorr = A - (A & A.psi());
  if ((A.hasUpper() || A.hasLower())
...
{
  tmp<mousse::fvMatrix<Type>> tAcorr = tA - (tA() & tA().psi());
...
// Global Operators 
template<class Type>
mousse::tmp<mousse::fvMatrix<Type>> mousse::operator==
(
  const fvMatrix<Type>& A,
...
// IOstream Operators 
template<class Type>
mousse::Ostream& mousse::operator<<(Ostream& os, const fvMatrix<Type>& fvm)
{
