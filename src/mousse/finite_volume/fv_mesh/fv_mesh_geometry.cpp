...
{
  if (debug) {
    Info << "void fvMesh::makeSf() : assembling face areas" << endl;
  }
...
{
  if (debug) {
    Info << "void fvMesh::makeMagSf() : assembling mag face areas" << endl;
  }
  // It is an error to attempt to recalculate
  // if the pointer is already set
  if (magSfPtr_) {
...
{
  if (debug) {
    Info << "void fvMesh::makeC() : assembling cell centres" << endl;
  }
  // It is an error to attempt to recalculate
  // if the pointer is already set
  if (CPtr_) {
...
{
  if (debug) {
    Info<< "void fvMesh::makeCf() : assembling face centres" << endl;
  }
  // It is an error to attempt to recalculate
  // if the pointer is already set
  if (CfPtr_) {
...
const volScalarField::DimensionedInternalField& fvMesh::V00() const
{
  if (!V00Ptr_) {
    if (debug) {
      Info << "fvMesh::V00() const: constructing from V0" << endl;
    }
...
tmp<volScalarField::DimensionedInternalField> fvMesh::Vsc() const
{
  if (moving() && time().subCycling()) {
    ...
    scalar tFrac =
      (ts.value() - (ts0.value() - ts0.deltaTValue()))/ts0.deltaTValue();
    if (tFrac < (1 - SMALL)) {
...
tmp<volScalarField::DimensionedInternalField> fvMesh::Vsc0() const
{
  if (moving() && time().subCycling()) {
    ...
    const TimeState& ts0 = time().prevTimeState();
    scalar t0Frac =
      ((ts.value() - ts.deltaTValue()) - (ts0.value() - ts0.deltaTValue()))
      /ts0.deltaTValue();
    if (t0Frac > SMALL) {
