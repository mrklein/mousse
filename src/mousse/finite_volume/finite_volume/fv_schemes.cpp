...
// Static Data Members
int mousse::fvSchemes::debug{mousse::debug::debugSwitch("fvSchemes", false)};


// Private Member Functions 
void mousse::fvSchemes::clear()
{
  ddtSchemes_.clear();
...
void mousse::fvSchemes::read(const dictionary& dict)
{
  ...
  if (ddtSchemes_.found("default")
    ...
    steady_ =
    (
      word(defaultDdtScheme_) == fv::steadyStateDdtScheme<scalar>::typeName
    );
  }
  ...
  if (snGradSchemes_.found("default")
  ...
  if (laplacianSchemes_.found("default")
...
// Constructors 
mousse::fvSchemes::fvSchemes(const objectRegistry& obr)
:
  IOdictionary
  {
    {
...
const mousse::dictionary& mousse::fvSchemes::schemesDict() const
{
  if (found("select")) {
...
mousse::ITstream& mousse::fvSchemes::ddtScheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::d2dt2Scheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::interpolationScheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::divScheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::gradScheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::snGradScheme(const word& name) const
{
  if (debug) {
...
mousse::ITstream& mousse::fvSchemes::laplacianScheme(const word& name) const
{
  if (debug) {
...
void mousse::fvSchemes::setFluxRequired(const word& name) const
{
  if (debug) {
...
bool mousse::fvSchemes::fluxRequired(const word& name) const
{
  if (debug) {
