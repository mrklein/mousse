...
)
{
  ...
  {
    surf1.patches().size() + surf2.patches().size()
  };
  ...
  faceMap_{}
{
  ...
  {
    subSurf1.nPoints() + subSurf2.nPoints()
  ...
  faceMap_{}
{
  ...
  for (label pointI = 0; pointI < cutSurf2.nSurfacePoints(); pointI++) {
  ...
  FOR_ALL(cutSurf2, faceI) {
    ...
    FOR_ALL(tri, fp) {
      if (cutSurf2.isSurfacePoint(tri[fp])) {
        ...
        // Is intersection.
        combinedTri[fp] =
          tri[fp] - cutSurf2.nSurfacePoints() + cutSurf1.nSurfacePoints();
      }
    }
