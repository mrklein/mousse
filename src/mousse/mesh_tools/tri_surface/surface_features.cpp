...
)
{
  ...
  if (eHit.hit()) {
    ...
    // which one.
    if (mag(eHit.rawPoint() - start) < mag(eHit.rawPoint() - end)) {
      endPoint = 0;
    } else {
      endPoint = 1;
    }
  }
  return {eHit.hit(), eHit.rawPoint(), endPoint};
}
...
) const
{
  ...
  FOR_ALL(edgeFaces, edgeI) {
    ...
    if (eFaces.size() != 2) {
      ...
      if (!geometricTestOnly && surf_[face0].region() != surf_[face1].region()) {
        edgeStat[edgeI] = REGION;
      } else if (selectAll
        ...
        vector f0Tof1 =
          surf_[face1].centre(points) - surf_[face0].centre(points);
...
) const
{
  ...
  FOR_ALL(pEdges, i) {
    ...
    if (edgeI != prevEdgeI && edgeStat[edgeI] != NONE
...
)
{
  ...
  do {
    ...
    for (; startEdgeI < edgeStat.size(); startEdgeI++) {
      if (edgeStat[startEdgeI] != NONE && featLines[startEdgeI] == -1) {
    ...
    if ((leftPath.len_ + rightPath.len_
         + startEdge.mag(surf_.localPoints()) < minLen)
