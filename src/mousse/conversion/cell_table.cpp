...
mousse::Map<mousse::word> mousse::cellTable::selectType(const word& matl) const
{
  Map<word> lookup;
  FOR_ALL_CONST_ITER(Map<dictionary>, *this, iter) {
    if (matl == iter().lookupOrDefault<word>("MaterialType", defaultMaterial_)) {
      lookup.insert
      (
        iter.key(),
        iter().lookupOrDefault<word>
...
) const
{
  ...
  ioObj.note() = "persistent data for thirdParty mesh <-> mousse translation";
...
) const
{
  ...
  FOR_ALL(zoneUsed, zoneI) {
    ...
    (
      zoneI,
      new cellZone{zoneNames[origZoneI], zoneCells[origZoneI], zoneI, czMesh}
    );
  }
