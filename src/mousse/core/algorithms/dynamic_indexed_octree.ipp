#ifndef CORE_ALGORITHMS_DYNAMIC_INDEXED_OCTREE_IPP_
#define CORE_ALGORITHMS_DYNAMIC_INDEXED_OCTREE_IPP_
...
// Static Data Members
template<class Type>
mousse::scalar mousse::dynamicIndexedOctree<Type>::perturbTol_ = 10*SMALL;

// Private Member Functions 
// Does bb intersect a sphere around sample? Or is any corner point of bb
// closer than nearestDistSqr to sample.
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::overlaps
(
  const point& p0,
  const point& p1,
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::divide
(
...
template<class Type>
typename mousse::dynamicIndexedOctree<Type>::node
mousse::dynamicIndexedOctree<Type>::divide
(
  const treeBoundBox& bb,
...
)
{
  ...
  if (bb.min()[0] >= bb.max()[0]
      ...
      || bb.min()[2] >= bb.max()[2])
  {
    FATAL_ERROR_IN("dynamicIndexedOctree<Type>::divide(..)")
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::recursiveSubDivision
(
...
{
  if(contents_[contentI]().size() > minSize_ && nLevels < maxLevels_)
  {
...
template<class Type>
mousse::volumeType mousse::dynamicIndexedOctree<Type>::calcVolumeType
(
  const label nodeI
) const
{
...
template<class Type>
mousse::volumeType mousse::dynamicIndexedOctree<Type>::getVolumeType
(
  const label nodeI,
  const point& sample
) const
...
template<class Type>
mousse::volumeType mousse::dynamicIndexedOctree<Type>::getSide
(
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::findNearest
(
  const label nodeI,
  const point& sample,
...
template<class Type>
mousse::treeBoundBox mousse::dynamicIndexedOctree<Type>::subBbox
(
...
template<class Type>
mousse::point mousse::dynamicIndexedOctree<Type>::pushPoint
(
  const treeBoundBox& bb,
  const point& pt,
...
)
{
  ...
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("dynamicIndexedOctree<Type>::pushPoint(..)")
        << "pushed point:" << pt
        << " to:" << perturbedPt
...
template<class Type>
mousse::point mousse::dynamicIndexedOctree<Type>::pushPointIntoFace
(
  const treeBoundBox& bb,
...
)
{
  if (debug)
  {
    if (bb.posBits(pt) != 0)
    {
      FATAL_ERROR_IN("dynamicIndexedOctree<Type>::pushPointIntoFace(..)")
        << " bb:" << bb << endl
...
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::walkToParent
(
  const label nodeI,
  const direction octant,
  label& parentNodeI,
...
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::walkToNeighbour
(
...
) const
{
  ...
  */
  ...
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("dynamicIndexedOctree<Type>::walkToNeighbour(..)")
...
template<class Type>
mousse::word mousse::dynamicIndexedOctree<Type>::faceString
(
  const direction faceID
)
{
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::traverseNode
(
  const bool findAny,
...
) const
{
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("dynamicIndexedOctree<Type>::traverseNode(..)")
...
template<class Type>
mousse::pointIndexHit mousse::dynamicIndexedOctree<Type>::findLine
(
  const bool findAny,
  const point& treeStart,
  const point& treeEnd,
...
) const
{
  ...
  if (i == 100000)
  {
    ...
    if (debug)
    {
      FATAL_ERROR_IN("dynamicIndexedOctree<Type>::findLine(..)")
    ...
    {
      WARNING_IN("dynamicIndexedOctree<Type>::findLine(..)")
        << "Got stuck in loop raytracing from:" << treeStart
        << " to:" << treeEnd << endl
        << "inside top box:" << subBbox(startNodeI, startOctant)
        << endl;
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::findBox
(
  const label nodeI,
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::findSphere
(
  const label nodeI,
  const point& centre,
...
void mousse::dynamicIndexedOctree<Type>::findNear
(
...
template<class Type>
mousse::label mousse::dynamicIndexedOctree<Type>::countElements
(
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::writeOBJ
(
  const label nodeI,
  const direction octant
) const
{
  OFstream str
  {
    "node" + mousse::name(nodeI) + "_octant" + mousse::name(octant) + ".obj"
  };
  labelBits index = nodes_[nodeI].subNodes_[octant];
  treeBoundBox subBb;
  if (isNode(index))
...
// Constructors 
template<class Type>
mousse::dynamicIndexedOctree<Type>::dynamicIndexedOctree
(
  const Type& shapes,
  const treeBoundBox& bb,
...
// Member Functions 
template<class Type>
mousse::scalar& mousse::dynamicIndexedOctree<Type>::perturbTol()
{
  return perturbTol_;
}
...
template<class Type>
mousse::pointIndexHit mousse::dynamicIndexedOctree<Type>::findNearest
(
  const point& sample,
...
// Find nearest intersection
template<class Type>
mousse::pointIndexHit mousse::dynamicIndexedOctree<Type>::findLineAny
(
  const point& start,
  const point& end
) const
...
template<class Type>
mousse::labelList mousse::dynamicIndexedOctree<Type>::findBox
(
  const treeBoundBox& searchBox
) const
{
...
template<class Type>
mousse::labelList mousse::dynamicIndexedOctree<Type>::findSphere
(
  const point& centre,
  const scalar radiusSqr
) const
...
template<class Type>
mousse::labelBits mousse::dynamicIndexedOctree<Type>::findNode
(
  const label nodeI,
  const point& sample
) const
...
template<class Type>
mousse::label mousse::dynamicIndexedOctree<Type>::findInside
(
  const point& sample
) const
{
...
template<class Type>
const mousse::labelList& mousse::dynamicIndexedOctree<Type>::findIndices
(
  const point& sample
) const
{
  ...
  if (nodeTypes_.size() != 8*nodes_.size())
  {
    ...
    if (debug)
    {
      ...
      FOR_ALL(nodeTypes_, i)
      {
        ...
        {
          FATAL_ERROR_IN("getVolumeType") << abort(FatalError);
        }
      }
...
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::insert(label startIndex, label endIndex)
{
...
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::insertIndex
(
...
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::remove(const label index)
{
  if (nodes_.empty())
  {
    return false;
...
template<class Type>
mousse::label mousse::dynamicIndexedOctree<Type>::removeIndex
(
  const label nodIndex,
  const label index
)
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::print
(
  prefixOSstream& os,
...
template<class Type>
void mousse::dynamicIndexedOctree<Type>::writeTreeInfo() const
{
  label nEntries = 0;
  FOR_ALL(contents_, i)
  {
...
// Print contents of nodeI
template<class Type>
bool mousse::dynamicIndexedOctree<Type>::write(Ostream& os) const
{
  os << *this;
  return os.good();
}
...
template<class Type>
mousse::Ostream&
mousse::operator<<(Ostream& os, const dynamicIndexedOctree<Type>& t)
{
  os << t.bb() << token::SPACE << t.nodes() << endl;
  FOR_ALL(t.contents(), cI)
  {
