#ifndef CORE_ALGORITHMS_INDEXED_OCTREE_IPP_
#define CORE_ALGORITHMS_INDEXED_OCTREE_IPP_
...
template<class Type>
void mousse::indexedOctree<Type>::divide
(
...
template<class Type>
typename mousse::indexedOctree<Type>::node
mousse::indexedOctree<Type>::divide
(
  const treeBoundBox& bb,
...
) const
{
  ...
  if (bb.min()[0] >= bb.max()[0]
      ...
      || bb.min()[2] >= bb.max()[2])
  {
    FATAL_ERROR_IN("indexedOctree<Type>::divide(..)")
...
template<class Type>
void mousse::indexedOctree<Type>::splitNodes
(
  const label minSize,
...
// the compactLevel.
template<class Type>
mousse::label mousse::indexedOctree<Type>::compactContents
(
...
template<class Type>
mousse::volumeType mousse::indexedOctree<Type>::calcVolumeType
(
  const label nodeI
) const
{
...
template<class Type>
mousse::volumeType mousse::indexedOctree<Type>::getVolumeType
(
  const label nodeI,
  const point& sample
) const
...
template<class Type>
mousse::volumeType mousse::indexedOctree<Type>::getSide
(
...
void mousse::indexedOctree<Type>::findNearest
(
  const label nodeI,
  const point& sample,
...
template<class Type>
mousse::treeBoundBox mousse::indexedOctree<Type>::subBbox
(
...
template<class Type>
mousse::point mousse::indexedOctree<Type>::pushPoint
(
  const treeBoundBox& bb,
  const point& pt,
...
)
{
  ...
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("indexedOctree<Type>::pushPoint(..)")
        << "pushed point:" << pt
        << " to:" << perturbedPt
...
template<class Type>
mousse::point mousse::indexedOctree<Type>::pushPointIntoFace
(
  const treeBoundBox& bb,
...
)
{
  if (debug)
  {
    if (bb.posBits(pt) != 0)
    {
      FATAL_ERROR_IN("indexedOctree<Type>::pushPointIntoFace(..)")
        << " bb:" << bb << endl
...
template<class Type>
bool mousse::indexedOctree<Type>::walkToParent
(
  const label nodeI,
  const direction octant,
  label& parentNodeI,
...
template<class Type>
bool mousse::indexedOctree<Type>::walkToNeighbour
(
...
) const
{
  ...
  */
  ...
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("indexedOctree<Type>::walkToNeighbour(..)")
...
template<class Type>
mousse::word mousse::indexedOctree<Type>::faceString
(
  const direction faceID
)
{
...
void mousse::indexedOctree<Type>::traverseNode
(
  const bool findAny,
...
) const
{
  if (debug)
  {
    ...
    {
      FATAL_ERROR_IN("indexedOctree<Type>::traverseNode(..)")
...
template<class Type>
template<class FindIntersectOp>
mousse::pointIndexHit mousse::indexedOctree<Type>::findLine
(
  const bool findAny,
  const point& treeStart,
  const point& treeEnd,
...
) const
{
  ...
  if (i == 100000)
  {
    ...
    if (debug)
    {
      FATAL_ERROR_IN("indexedOctree<Type>::findLine(..)")
    ...
    {
      WARNING_IN("indexedOctree<Type>::findLine(..)")
        << "Got stuck in loop raytracing from:" << treeStart
        << " to:" << treeEnd << endl
        << "inside top box:" << subBbox(startNodeI, startOctant)
        << endl;
...
template<class Type>
void mousse::indexedOctree<Type>::findBox
(
  const label nodeI,
...
template<class Type>
void mousse::indexedOctree<Type>::findSphere
(
  const label nodeI,
  const point& centre,
...
void mousse::indexedOctree<Type>::findNear
(
...
template<class Type>
mousse::label mousse::indexedOctree<Type>::countElements
(
...
template<class Type>
void mousse::indexedOctree<Type>::writeOBJ
(
  const label nodeI,
  const direction octant
) const
...
// Constructors 
template<class Type>
mousse::indexedOctree<Type>::indexedOctree(const Type& shapes)
:
  shapes_{shapes},
  nodes_{0},
  contents_{0},
...
template<class Type>
mousse::indexedOctree<Type>::indexedOctree
(
  const Type& shapes,
  ...
  nodeTypes_{0}
{
  ...
  for (; nLevels < maxLevels; nLevels++)
  {
    ...
    if (nEntries > maxDuplicity*shapes.size())
    {
      break;
    }
...
// Member Functions 
template<class Type>
mousse::scalar& mousse::indexedOctree<Type>::perturbTol()
{
  return perturbTol_;
}
...
template<class Type>
mousse::pointIndexHit mousse::indexedOctree<Type>::findNearest
(
  const point& sample,
...
// Find nearest intersection
template<class Type>
mousse::pointIndexHit mousse::indexedOctree<Type>::findLineAny
(
  const point& start,
  const point& end
) const
...
template<class Type>
mousse::labelList mousse::indexedOctree<Type>::findBox
(
  const treeBoundBox& searchBox
) const
{
...
template<class Type>
mousse::labelList mousse::indexedOctree<Type>::findSphere
(
  const point& centre,
  const scalar radiusSqr
) const
...
template<class Type>
mousse::labelBits mousse::indexedOctree<Type>::findNode
(
  const label nodeI,
  const point& sample
) const
...
template<class Type>
mousse::label mousse::indexedOctree<Type>::findInside(const point& sample) const
{
...
template<class Type>
const mousse::labelList& mousse::indexedOctree<Type>::findIndices
(
  const point& sample
) const
{
...
template<class Type>
void mousse::indexedOctree<Type>::print
(
  prefixOSstream& os,
...
// Print contents of nodeI
template<class Type>
bool mousse::indexedOctree<Type>::write(Ostream& os) const
{
  os << *this;
  return os.good();
}
...
template<class Type>
mousse::Ostream& mousse::operator<<(Ostream& os, const indexedOctree<Type>& t)
{
  return
    os << t.bb() << token::SPACE << t.nodes() << token::SPACE << t.contents();
}
