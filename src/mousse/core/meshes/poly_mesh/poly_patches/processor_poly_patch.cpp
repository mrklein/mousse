...
ADD_TO_RUN_TIME_SELECTION_TABLE(polyPatch, processorPolyPatch, dictionary);
...
mousse::label mousse::processorPolyPatch::matchFace
(
  const face& a,
...
bool mousse::processorPolyPatch::order
(
  PstreamBuffers& pBufs,
  const primitivePatch& pp,
  labelList& faceMap,
    ...
    if (transform() == COINCIDENTFULLMATCH) {
      ...
      FOR_ALL(pp, lFaceI) {
        ...
        if (closestFaceRotation != -1 && closestMatchDistSqr == 0) {
          ...
          FATAL_ERROR_IN("mousse::processorPolyPatch::order(...) const")
