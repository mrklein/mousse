...
#include "old_cyclic_poly_patch.hpp"
...
DEFINE_TYPE_NAME_AND_DEBUG(oldCyclicPolyPatch, 0);
ADD_TO_RUN_TIME_SELECTION_TABLE(polyPatch, oldCyclicPolyPatch, word);
ADD_TO_RUN_TIME_SELECTION_TABLE(polyPatch, oldCyclicPolyPatch, dictionary);

}


mousse::pointField mousse::oldCyclicPolyPatch::calcFaceCentres
(
...
mousse::pointField mousse::oldCyclicPolyPatch::getAnchorPoints
(
  const UList<face>& faces,
  const pointField& points
)
...
mousse::label mousse::oldCyclicPolyPatch::findMaxArea
(
  const pointField& points,
...
bool mousse::oldCyclicPolyPatch::getGeometricHalves
(
  ...
  if (debug) {
    ...
    for (label zoneI = 0; zoneI < ppZones.nZones(); zoneI++) {
      OFstream stream
      {
        ...
        /name()+"_zone_"+mousse::name(zoneI)+".obj"
      };
...
// right ones.
void mousse::oldCyclicPolyPatch::getCentresAndAnchors
(
  const primitivePatch& pp,
  ...
  switch (transform()) {
    case ROTATIONAL:
    {
      ...
      FOR_ALL(half0Ctrs, faceI) {
        half0Ctrs[faceI] = mousse::transform(reverseT, half0Ctrs[faceI]);
        anchors0[faceI] = mousse::transform(reverseT, anchors0[faceI]);
      }
      ppPoints = mousse::transform(reverseT, pp.points());
      break;
    }
    default:
    {
...
bool mousse::oldCyclicPolyPatch::matchAnchors
(
  const bool report,
  const primitivePatch& pp,
...
) const
{
  ...
  // faces last.
  FOR_ALL(half0ToPatch, half0FaceI) {
...
mousse::label mousse::oldCyclicPolyPatch::getConsistentRotationFace
(
...
// Constructors 
mousse::oldCyclicPolyPatch::oldCyclicPolyPatch
(
  const word& name,
  const label size,
  const label start,
...
// Member Functions 
void mousse::oldCyclicPolyPatch::initGeometry(PstreamBuffers& pBufs)
{
  polyPatch::initGeometry(pBufs);
}


void mousse::oldCyclicPolyPatch::calcGeometry
(
...
void mousse::oldCyclicPolyPatch::calcGeometry(PstreamBuffers& /*pBufs*/)
{}


void mousse::oldCyclicPolyPatch::initMovePoints
(
...
void mousse::oldCyclicPolyPatch::movePoints
(
  PstreamBuffers& pBufs,
  const pointField& p
)
...
void mousse::oldCyclicPolyPatch::initUpdateMesh(PstreamBuffers& pBufs)
{
  polyPatch::initUpdateMesh(pBufs);
}


void mousse::oldCyclicPolyPatch::updateMesh(PstreamBuffers& pBufs)
{
  polyPatch::updateMesh(pBufs);
}


void mousse::oldCyclicPolyPatch::initOrder
(
  PstreamBuffers&,
...
bool mousse::oldCyclicPolyPatch::order
(
  PstreamBuffers&,
  const primitivePatch& pp,
  labelList& faceMap,
...
) const
{
  ...
  if (pp.size()&1) {
    FATAL_ERROR_IN("oldCyclicPolyPatch::order(..)")
  ...
  if (!matchedAll) {
    ...
    if (debug) {
      ...
      OFstream ccStr
      {
        boundaryMesh().mesh().time().path()/"match2_"+name()+"_faceCentres.obj"
      };
      Pout << "oldCyclicPolyPatch::order : "
        << "Dumping currently found cyclic match as lines between"
        << " corresponding face centres to file " << ccStr.name()
...
void mousse::oldCyclicPolyPatch::write(Ostream& os) const
{
  ...
  WARNING_IN("oldCyclicPolyPatch::write(Ostream& os) const")
