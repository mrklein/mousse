...
:
  ...
  mesh_{pm}
{
  if ((this->readOpt() == IOobject::READ_IF_PRESENT && this->headerOk())
    ...
    FOR_ALL(*this, patchI) {
      ...
      (
        ...
        if (fnd == pointsToEdge.end()) {
          ...
          neighbourEdges[patchI][edgei - pp.nInternalEdges()] = edgeInfo;
...
mousse::label mousse::polyBoundaryMesh::whichPatch(const label faceIndex) const
{
  ...
  FOR_ALL(*this, patchI) {
    ...
    if (faceIndex >= bp.start() && faceIndex < bp.start() + bp.size()) {
      return patchI;
    }
  }
...
) const
{
  ...
  for
  (
    HashTable<labelList,word>::const_iterator iter = groupPatchIDs.begin();
