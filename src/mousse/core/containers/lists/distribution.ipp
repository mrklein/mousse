...
Type mousse::Distribution<Type>::mean() const
{
  ...
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    ...
    FOR_ALL(theKeys, k) {
      label key = theKeys[k];
      setComponent(meanValue, cmpt) += (0.5 + scalar(key))
        *component(binWidth_, cmpt)*cmptDistribution[k]/totalCmptWeight;
    }
  }
  return meanValue;
}
  ...
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    ...
    if (normDist.size()) {
      if (normDist.size() == 1) {
        ...
        scalar xk = normDist[0].first() + 0.5*component(binWidth_, cmpt);
        scalar xkm1 = normDist[0].first() - 0.5*component(binWidth_, cmpt);
        ...
        FOR_ALL(normDist, nD) {
          if (cumulative
              + (normDist[nD].second()*component(binWidth_, cmpt)) > 0.5) {
            scalar xk = normDist[nD].first() + 0.5*component(binWidth_, cmpt);
            scalar xkm1 = normDist[previousNonZeroIndex].first()
              + 0.5*component(binWidth_, cmpt);
            scalar Sk = cumulative
            ...
            cumulative += normDist[nD].second()*component(binWidth_, cmpt);
...
)
{
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    List<scalar>& cmptDistribution = (*this)[cmpt];
    label n = label(component(valueToAdd, cmpt)/component(binWidth_, cmpt))
...
Distribution<Type>::normalised() const
{
  ...
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    ...
    for (label k = limits.first(), i = 0; k <= limits.second(); k++, i++) {
      label key = cmptKeys[k];
      normDist[i].first() = (0.5 + scalar(key))*component(binWidth_, cmpt);
      normDist[i].second() = cmptDistribution[k]/totalCmptWeight
...
Distribution<Type>::cumulativeNormalised() const
{
  ...
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    const List<Pair<scalar>>& normalisedCmpt = normalisedDistribution[cmpt];
    List<Pair<scalar>>& cumNormalisedCmpt =
      cumulativeNormalisedDistribution[cmpt];
    scalar sum = 0.0;
    FOR_ALL(normalisedCmpt, i) {
      cumNormalisedCmpt[i].first() = normalisedCmpt[i].first()
        + 0.5*component(binWidth_, cmpt);
...
Distribution<Type>::cumulativeRaw() const
{
  ...
  for (direction cmpt = 0; cmpt < pTraits<Type>::nComponents; cmpt++) {
    ...
    scalar sum = 0.0;
    FOR_ALL(rawCmpt, i) {
      cumRawCmpt[i].first() = rawCmpt[i].first()
        + 0.5*component(binWidth_, cmpt);
