...
)
{
  if (currToken.isWord()) {
    ...
    if ( disableFunctionEntries || w.size() == 1
    ...
    if (disableFunctionEntries || w.size() <= 3
        || !(w[0] == '$' && w[1] == token::BEGIN_BLOCK
...
bool mousse::primitiveEntry::read(const dictionary& dict, Istream& is)
{
  ...
  label blockCount = 0;
  token currToken;
  if (!is.read(currToken).bad() && currToken.good()
    ...
    while (!is.read(currToken).bad()
           && currToken.good()
           && !(currToken == token::END_STATEMENT && blockCount == 0)) {
      if (currToken == token::BEGIN_BLOCK || currToken == token::BEGIN_LIST) {
        blockCount++;
      } else if (currToken == token::END_BLOCK || currToken == token::END_LIST) {
        blockCount--;
      }
      append(currToken, dict, is);
    }
