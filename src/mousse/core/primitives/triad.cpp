...
template<> const char* const triad::Vector<vector>::typeName = "triad";
template<>
...
{
  {ROOTVGREAT, ROOTVGREAT, ROOTVGREAT},
  {ROOTVGREAT, ROOTVGREAT, ROOTVGREAT},
  {ROOTVGREAT, ROOTVGREAT, ROOTVGREAT}
};
...
void mousse::triad::align(const vector& v)
{
  if (set()) {
    ...
    scalar mav;
    if (mostAligned.x() > mostAligned.y() && mostAligned.x() > mostAligned.z()) {
      mav = mostAligned.x();
...
mousse::triad mousse::triad::sortxyz() const
{
  ...
  triad t;
  if (mag(operator[](0).x()) > mag(operator[](1).x())
  ...
  } else if (mag(operator[](1).x()) > mag(operator[](2).x())) {
    t[0] = operator[](1);
