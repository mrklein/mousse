...
void mousse::wallBoundedParticle::patchInteraction
(
  TrackData& td,
...
template<class TrackData>
mousse::scalar mousse::wallBoundedParticle::trackToEdge
(
  TrackData& td,
...
)
{
  ...
  if (!td.isWallPatch_[tetFace()]) {
    ...
    if (doTrack) {
      ...
      const mousse::face& f = mesh_.faces()[ti.face()];
      ...
      if (triEdgeI == 0) {
        ...
        if (ti.facePtB() == f.rcIndex(fp0)) {
          ...
          //Note: should not happen since boundary face so owner
          FATAL_ERROR_IN("shold not happend") << info() << abort(FatalError);
          diagEdge_ = -1;
          meshEdgeStart_ = fp0;
          crossEdgeConnectedFace(currentEdge());
          patchInteraction(td, trackFraction);
...
)
{
  ...
  // f.size()-edgeStart_-1.
  const mousse::face& f = mesh_.faces()[face()];
  if (meshEdgeStart_ != -1) {
...
void mousse::wallBoundedParticle::readFields(CloudType& c)
{
  if (!c.size()) {
    return;
  }
...
template<class CloudType>
void mousse::wallBoundedParticle::writeFields(const CloudType& c)
{
