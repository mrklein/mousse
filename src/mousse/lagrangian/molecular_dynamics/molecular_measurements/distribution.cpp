...
mousse::scalar mousse::distribution::approxTotalEntries() const
{
  ...
  FOR_ALL_CONST_ITER(Map<label>, *this, iter) {
    sumOfEntries += static_cast<scalar>(iter());
  }
  return sumOfEntries;
}
...
mousse::scalar mousse::distribution::median()
{
  ...
  if (normDist.size()) {
    ...
    } else if (normDist.size() > 1 && normDist[0].second()*binWidth_ > 0.5) {
...
void mousse::distribution::add(const scalar valueToAdd)
{
  ...
  label n = static_cast<label>(valueToAdd/binWidth_)
    - static_cast<label>(neg(valueToAdd/binWidth_));
  iter = find(n);
  if (iter == this->end()) {
    this->insert(n,1);
  } else {
...
)
{
  ...
  label lowestNewKey =
    static_cast<label>(lowestOldBin + 0.5*sign(lowestOldBin));
  ...
  FOR_ALL(oldDist,u) {
    ...
    if (interpolationStartDirection < 0) {
      ...
      if (u == oldDist.size() - 1) {
        newDist[u].second() =
          (0.5 + scalar(newKey))*-oldDist[u].second()
          + oldDist[u].second()*(oldDist[u].first() + binWidth_)/binWidth_;
      } else {
        newDist[u].second() =
          (0.5 + scalar(newKey))
          *(oldDist[u+1].second() - oldDist[u].second())
          + (oldDist[u].second()*oldDist[u+1].first()
             - oldDist[u+1].second()*oldDist[u].first())/binWidth_;
      }
    }
    newKey++;
  }
...
{
  os << d.binWidth_ << static_cast<const Map<label>&>(d);
