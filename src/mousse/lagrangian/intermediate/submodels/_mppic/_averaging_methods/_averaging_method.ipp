...
bool mousse::AveragingMethod<Type>::write() const
{
  ...
  GeometricField<TypeGrad, fvPatchField, volMesh> cellGrad
  {
    ...
    mesh_,
    {"zero", dimless, pTraits<TypeGrad>::zero}
  };
  ...
  FOR_ALL(mesh_.C(), cellI) {
    ...
    FOR_ALL(cellTets, tetI) {
      ...
      FOR_ALL(vertices, vertexI) {
        ...
        pointValue[pointI] += v*interpolate(mesh_.points()[pointI], tetIs);
        pointGrad[pointI] += v*interpolateGrad(mesh_.points()[pointI], tetIs);
      }
    }
  }
  // average
