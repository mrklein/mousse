...
void mousse::ParticleCollector<CloudType>::makeLogFile
(
...
template<class CloudType>
void mousse::ParticleCollector<CloudType>::initPolygons
(
...
template<class CloudType>
void mousse::ParticleCollector<CloudType>::initConcentricCircles()
{
...
template<class CloudType>
void mousse::ParticleCollector<CloudType>::collectParcelPolygon
(
  const point& p1,
  const point& p2
) const
{
  ...
  FOR_ALL(faces_, faceI) {
    ...
    FOR_ALL(tris, triI) {
      ...
      triPointRef t{points_[tri[0]], points_[tri[1]], points_[tri[2]]};
...
template<class CloudType>
void mousse::ParticleCollector<CloudType>::collectParcelConcentricCircles
(
  const point& p1,
  const point& p2
) const
{
  ...
  if (r < radius_.last()) {
    ...
    if (nSector_ == 1) {
      ...
      secI = nSector_*radI + floor(scalar(nSector_)*theta/twoPi);
    }
  }
...
template<class CloudType>
void mousse::ParticleCollector<CloudType>::write()
{
