...
)
{
  ...
  label maxIterations = max(1, (10*volumeAccumulator_)/Vm);
  label iterationNo = 0;
  ...
  while (!generationCells_.empty() && volumeAccumulator_ > 0) {
    ...
    label cI =
      generationCells_[rnd.position(label(0), generationCells_.size() - 1)];
    ...
    if (cellOccupancy[cI].empty()) {
      ...
      if (pPtr != NULL) {
        ...
        // is to be deleted
        volumeAccumulator_ +=
          CloudType::parcelType::volume(pPtr->dTarget());
  ...
  if (Pstream::parRun()) {
    List<List<vectorPairScalarPair>> gatheredNewParticles{Pstream::nProcs()};
