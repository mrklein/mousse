...
template<class CloudType>
void mousse::FreeStream<CloudType>::inflow()
{
  CloudType& cloud = this->owner();
  const polyMesh& mesh = cloud.mesh();
  ...
  FOR_ALL(patches_, p) {
    ...
    FOR_ALL(pFA, i) {
      ...
      {
        cloud.maxwellianMostProbableSpeed(boundaryT[patchi], mass)
      };
    ...
    FOR_ALL(patch, pFI) {
      ...
      FOR_ALL(faceTets, triI) {
        ...
        cTriAFracs[triI] =
          faceTetIs.faceTri(mesh).mag()/fA + previousCummulativeSum;
      ...
      FOR_ALL(pFA, i) {
        ...
        for (label i = 0; i < nI; i++) {
          ...
          scalar uNormProbCoeffB =
            0.5*(1.0 + sCosTheta*(sCosTheta - sqrt(sqr(sCosTheta) + 2.0)));
          ...
          do {
            uNormalThermal = randomScaling*(2.0*rndGen.scalar01() - 1);
          ...
          vector U =
            ...
            *(rndGen.GaussNormal()*t1 + rndGen.GaussNormal()*t2)
            + (t1 & faceVelocity)*t1
            + (t2 & faceVelocity)*t2
