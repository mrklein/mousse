  ...
  SMDCalcMethod_{this->coeffDict().lookup("SMDCalculationMethod")}
{
  ...
  FOR_ALL(rrd_, n) {
    scalar xx = 0.12*(n + 1);
    rrd_[n] = (1.0 - exp(-xx)*(1.0 + xx + sqr(xx)/2.0 + pow3(xx)/6.0))*rrd100;
  }
...
template<class CloudType>
mousse::TAB<CloudType>::TAB(const TAB<CloudType>& bum)
:
    ...
    if (a + Wetmp > 1.0) {
      ...
      if (dt > tb) {
        ...
        switch (SMDMethod_) {
          case method1:
          {
            #include "tabsmd_calc_method1.inc"
            break;
          }
          case method2:
          {
            #include "tabsmd_calc_method2.inc"
            break;
          }
        }
        if (rNew < r) {
