...
mousse::LISAAtomization<CloudType>::LISAAtomization
(
...
// Member Functions 
template<class CloudType>
mousse::scalar mousse::LISAAtomization<CloudType>::initLiquidCore() const
{
  return 1.0;
}
...
template<class CloudType>
bool mousse::LISAAtomization<CloudType>::calcChi() const
{
  return true;
}
...
template<class CloudType>
void mousse::LISAAtomization<CloudType>::update
(
  const scalar dt,
  scalar& d,
  scalar& liquidCore,
...
) const
{
  ...
  if (We > 27.0/16.0) {
    ...
    scalar derivNeg =
      (8.0*sqr(nu)*pow3(kNeg) + Q*sqr(Urel)*kNeg - 3.0*sigma/2.0/rho*sqr(kNeg))
      /sqrt(4.0*sqr(nu)*pow4(kNeg) + Q*sqr(Urel)*sqr(kNeg)
            - sigma*pow3(kNeg)/rho)
      - 4.0*nu*kNeg;
    scalar kOld = 0.0;
    for (label i=0; i<40; i++) {
      ...
      scalar derivk =
        (8.0*sqr(nu)*pow3(k) + Q*sqr(Urel)*k - 3.0*sigma/2.0/rho*sqr(k))
        /sqrt(4.0*sqr(nu)*pow4(k) + Q*sqr(Urel)*sqr(k) - sigma*pow3(k)/rho)
        - 4.0*nu*k;
      if (derivk > 0) {
        derivPos = derivk;
        kPos = k;
  ...
  if (pWalk > lBU) {
    ...
    switch (SMDMethod_) {
      case method1:
      {
        #include "lisasmd_calc_method1.inc"
        break;
      }
      case method2:
      {
        #include "lisasmd_calc_method2.inc"
        break;
      }
    }
