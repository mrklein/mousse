...
)
{
  FOR_ALL(featDicts, featI) {
    ...
    if (!fName.empty() && extendedEdgeMesh::canRead(fName)) {
      autoPtr<extendedEdgeMesh> eMeshPtr =
        extendedEdgeMesh::New(fName);
      ...
      FOR_ALL(edges, edgeI) {
        ...
        newEdges[edgeI] = edge(oldToNew[e[0]], oldToNew[e[1]]);
      }
    ...
    if (dict.found("levels")) {
      ...
      FOR_ALL(distLevels, j) {
        ...
        if (j > 0) {
          if ((distances_[featI][j] <= distances_[featI][j - 1])
...
) const
{
  ...
  FOR_ALL(candidates, candidateI) {
    nearInfo[candidateI] =
      tree.findNearest(candidates[candidateI], candidateDistSqr[candidateI]);
  }
  // Update maxLevel
...
) const
{
  ...
  nearNormal.setSize(samples.size());
  nearNormal = vector::zero;
  const PtrList<indexedOctree<treeDataEdge>>& regionTrees = regionEdgeTrees();
