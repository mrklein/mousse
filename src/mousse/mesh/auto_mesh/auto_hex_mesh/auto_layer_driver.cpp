...
#include "zero_fixed_value_point_patch_fields.hpp"
...
void mousse::autoLayerDriver::dumpDisplacement
(
...
mousse::tmp<mousse::scalarField> mousse::autoLayerDriver::avgPointData
(
  const indirectPrimitivePatch& pp,
...
// points in pointSet.
void mousse::autoLayerDriver::checkManifold
(
...
void mousse::autoLayerDriver::checkMeshManifold() const
{
  ...
  pointSet nonManifoldPoints{mesh, "nonManifoldPoints", mesh.nPoints() / 100};
...
bool mousse::autoLayerDriver::unmarkExtrusion
(
...
)
{
  ...
  FOR_ALL(lF, fp) {
    if (unmarkExtrusion(lF[fp], pD, pNL, eS)) {
      unextruded = true;
    }
  }
  return unextruded;
...
void mousse::autoLayerDriver::handleNonManifolds
(
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  FOR_ALL(edgeGlobalFaces, edgeI) {
    if (pp.edgeFaces()[edgeI].size() == 1 && edgeGlobalFaces[edgeI].size() > 2) {
  ...
  if (nNonManif > 0) {
    ...
    FOR_ALL(meshPoints, patchPointI) {
      if (nonManifoldPoints.found(meshPoints[patchPointI])) {
        unmarkExtrusion(patchPointI, patchDisp, patchNLayers, extrudeStatus);
      }
    }
  }
...
void mousse::autoLayerDriver::handleFeatureAngle
(
  const indirectPrimitivePatch& pp,
  const labelList& meshEdges,
  const scalar minCos,
  ...
  if (minCos < 1-SMALL) {
    ...
    FOR_ALL(edgeFaces, edgeI) {
      ...
      if (n != point::max) {
        ...
        if (cos < minCos) {
          const edge& e = pp.edges()[edgeI];
          unmarkExtrusion(e[0], patchDisp, patchNLayers, extrudeStatus);
          unmarkExtrusion(e[1], patchDisp, patchNLayers, extrudeStatus);
          nFeats++;
          if (str.valid()) {
...
// the face.
void mousse::autoLayerDriver::handleWarpedFaces
(
  const indirectPrimitivePatch& pp,
...
void mousse::autoLayerDriver::setNumLayers
(
...
// layers
mousse::tmp<mousse::pointVectorField>
mousse::autoLayerDriver::makeLayerDisplacementField
(
...
void mousse::autoLayerDriver::growNoExtrusion
(
  const indirectPrimitivePatch& pp,
  pointField& patchDisp,
  labelList& patchNLayers,
...
void mousse::autoLayerDriver::determineSidePatches
(
...
void mousse::autoLayerDriver::calculateLayerThickness
(
  const indirectPrimitivePatch& pp,
  const labelList& patchIDs,
...
) const
{
  ...
  if (layerParams.relativeSizes()) {
    if (min(layerParams.minThickness()) < 0
  ...
  FOR_ALL(firstLayerThickness, pointI) {
    ...
    expansionRatio[pointI] =
      layerParams.layerExpansionRatio
      (
        patchNLayers[pointI],
        firstLayerThickness[pointI],
        finalLayerThickness[pointI],
  ...
  // Print a bit
  {
    ...
    FOR_ALL(patchIDs, i) {
      ...
      FOR_ALL(meshPoints, patchPointI) {
        ...
        if (isMasterPoint[meshPointI]) {
          ...
          sumNearWallThickness +=
            layerParams.firstLayerThickness
            (
      ...
      if (totNPoints > 0) {
        avgThickness =
          returnReduce(sumThickness, sumOp<scalar>())/totNPoints;
        avgNearWallThickness =
          returnReduce(sumNearWallThickness, sumOp<scalar>())/totNPoints;
      }
...
void mousse::autoLayerDriver::syncPatchDisplacement
(
  const indirectPrimitivePatch& pp,
  ...
  while (true) {
    ...
    FOR_ALL(patchDisp, i) {
      if (mag(patchDisp[i]) < minThickness[i]) {
        if (unmarkExtrusion(i, patchDisp, patchNLayers, extrudeStatus)) {
          nChanged++;
        }
      }
    }
...
// patch point.
void mousse::autoLayerDriver::getPatchDisplacement
(
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  FOR_ALL(extrudeStatus, patchPointI) {
    ...
    patchDisp[patchPointI] = avg/nPoints - localPoints[patchPointI];
    nExtrudeRemove++;
  }
...
bool mousse::autoLayerDriver::sameEdgeNeighbour
(
...
void mousse::autoLayerDriver::getVertexString
(
  const indirectPrimitivePatch& pp,
  const labelListList& globalEdgeFaces,
  const label faceI,
...
mousse::label mousse::autoLayerDriver::truncateDisplacement
(
  const globalIndex& globalFaces,
  const labelListList& edgeGlobalFaces,
  const indirectPrimitivePatch& pp,
  ...
  while (true) {
    ...
    FOR_ALL(localFaces, i) {
      ...
      if (nTrans > 1) {
        ...
        if (unmarkExtrusion(localF, patchDisp, patchNLayers, extrudeStatus)) {
          nPinched++;
          nChanged++;
        }
      }
    ...
    label nButterFly = 0;
    {
      ...
      FOR_ALL(pp.edges(), edgeI) {
        ...
        if (globFaces.size() == 2) {
          ...
          label myGlobalFaceI =
            globalFaces.toGlobal(pp.addressing()[myFaceI]);
          label nbrGlobalFaceI =
              globFaces[0] != myGlobalFaceI ? globFaces[0] : globFaces[1];
          getVertexString
          (
            pp,
            edgeGlobalFaces,
          ...
          if (extrudeStatus[stringedVerts[0]] != NOEXTRUDE
...
void mousse::autoLayerDriver::setupLayerInfoTruncation
(
  const indirectPrimitivePatch& pp,
  const labelList& patchNLayers,
  const List<extrudeMode>& extrudeStatus,
      ...
      for (label ibuffer = 0; ibuffer < nBuffer + 1; ibuffer++) {
        ...
        FOR_ALL(pp.meshPoints(), patchPointI) {
          FOR_ALL(pointFaces[patchPointI], pointFaceI) {
            ...
            if (nPatchFaceLayers[faceI] != -1 && maxLevel[faceI] > 0) {
...
bool mousse::autoLayerDriver::cellsUseFace
(
  const polyMesh& mesh,
...
mousse::label mousse::autoLayerDriver::checkAndUnmark
(
    ...
    if (nChangedTotal > nReportMax) {
      nReportLocal =
        ...
        (
          ...
          min(nChanged, max(nReportMax / Pstream::nProcs(), 1))
        );
    }
    if (nReportLocal) {
...
mousse::label mousse::autoLayerDriver::countExtrusion
(
  const indirectPrimitivePatch& pp,
  const List<extrudeMode>& extrudeStatus
)
...
void mousse::autoLayerDriver::getLayerCellsFaces
(
  const polyMesh& mesh,
  const addPatchCellLayer& addLayer,
...
void mousse::autoLayerDriver::printLayerData
(
  const fvMesh& mesh,
  const labelList& patchIDs,
...
) const
{
  ...
  FOR_ALL(patchIDs, i) {
    ...
    // Thickness
    scalarField::subField patchWanted =
      pbm[patchI].patchSlice
      (
        faceWantedThickness
      );
    scalarField::subField patchReal =
      pbm[patchI].patchSlice
      (
        faceRealThickness
      );
...
bool mousse::autoLayerDriver::writeLayerData
(
  const fvMesh& mesh,
  const labelList& patchIDs,
  const labelList& cellNLayers,
  ...
  if (meshRefinement::writeLevel() & meshRefinement::WRITELAYERFIELDS) {
...

    {
      ...
      FOR_ALL(patchIDs, i) {
        label patchI = patchIDs[i];
        scalarField::subField patchWanted =
          pbm[patchI].patchSlice(faceWantedThickness);
        scalarField::subField patchReal =
...
// Constructors 
mousse::autoLayerDriver::autoLayerDriver
(
...
// Member Functions 
void mousse::autoLayerDriver::mergePatchFacesUndo
(
  const layerParameters& layerParams,
...
)
{
  ...
  scalar minCos = mousse::cos(degToRad(planarAngle));
  scalar concaveCos = mousse::cos(degToRad(layerParams.concaveAngle()));
  Info << nl
...
void mousse::autoLayerDriver::addLayers
(
  const layerParameters& layerParams,
  const dictionary& motionDict,
  const labelList& patchIDs,
...
)
{
  ...
  if (debug&meshRefinement::MESH) {
    ...
    (
      ...
      (
        meshRefinement::writeLevel() | meshRefinement::WRITEMESH
      ),
  ...
  for (label iteration = 0; iteration < layerParams.nLayerIter(); iteration++) {
    ...
    FOR_ALL(nPatchPointLayers, i) {
      scalar ratio =
        layerParams.finalLayerThicknessRatio
        (
          nPatchPointLayers[i],
          expansionRatio[i]
        );
