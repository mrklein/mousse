...
bool mousse::autoSnapDriver::isFeaturePoint
(
...
void mousse::autoSnapDriver::smoothAndConstrain
(
...
) const
{
  ...
  for (label avgIter = 0; avgIter < 20; avgIter++) {
    ...
    // Constraints
    FOR_ALL(constraints, pointI) {
      if (dispCount[pointI] > 0) {
        // Mix my displacement with neighbours' displacement
        disp[pointI] = 0.5*(disp[pointI] + dispSum[pointI]/dispCount[pointI]);
      }
    }
  }
}
...
void mousse::autoSnapDriver::calcNearestFace
(
  const label iter,
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  FOR_ALL(zonedSurfaces, i) {
    ...
    FOR_ALL(hitInfo, hitI) {
      if (hitInfo[hitI].hit()) {
        ...
        faceSurfaceGlobalRegion[faceI] =
          surfaces.globalRegion(hitSurface[hitI], hitRegion[hitI]);
      } else {
        WARNING_IN
        (
          "autoSnapDriver::calcNearestFace(..)"
  ...
  FOR_ALL(faceRotation, faceI) {
    // Note: extend to >180 degrees checking
    faceRotation[faceI] =
      pp.faceNormals()[faceI] ^ faceSurfaceNormal[faceI];
  }
  if (debug&meshRefinement::ATTRACTION) {
    dumpMove
      (
        mesh.time().path()/"faceDisp_" + name(iter) + ".obj",
        pp.faceCentres(),
        pp.faceCentres() + faceDisp
      );
    dumpMove
      (
        mesh.time().path()/"faceRotation_" + name(iter) + ".obj",
        pp.faceCentres(),
...
void mousse::autoSnapDriver::calcNearestFacePointProperties
(
  const label /*iter*/,
  const indirectPrimitivePatch& pp,
...
void mousse::autoSnapDriver::correctAttraction
(
...
) const
{
  ...
  labelList order;
  mousse::sortedOrder(surfaceCounts, order);
  if (order[0] < order[1]) {
    ...
    point linePt = edgePt + ((1.0-fraction)*tang2 + fraction*tang)*edgeNormal;
    edgeOffset = linePt-pt;
  }
}


mousse::pointIndexHit mousse::autoSnapDriver::findMultiPatchPoint
(
  const point& pt,
...
mousse::label mousse::autoSnapDriver::findNormal
(
  const scalar featureCos,
  const vector& n,
...
{
  label index = -1;
  FOR_ALL(surfaceNormals, j) {
    scalar cosAngle = (n & surfaceNormals[j]);
    if ((cosAngle >= featureCos) || (cosAngle < (-1+0.001))) {
      index = j;
      break;
    }
  }
...
void mousse::autoSnapDriver::featureAttractionUsingReconstruction
(
  const label /*iter*/,
  const scalar featureCos,
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  FOR_ALL(pfSurfNormals, i) {
    ...
    if (magSqr(fDisp) < sqr(snapDist[pointI]) && mag(fSNormal) > VSMALL) {
      ...
      faceToNormalBin[i] = findNormal(featureCos, fSNormal, surfaceNormals);
      if (faceToNormalBin[i] != -1) {
        // Same normal
      } else {
  ...
  if (surfaceNormals.size() == 1) {
    ...
    vector d = ((surfacePoints[0] - pt) & surfaceNormals[0])*surfaceNormals[0];
    // Trim to snap distance
    if (magSqr(d) > sqr(snapDist[pointI])) {
      d *= mousse::sqrt(sqr(snapDist[pointI])/magSqr(d));
    }
    patchAttraction = d;
    // Store constraints
...
) const
{
  ...
  if (debug&meshRefinement::ATTRACTION) {
    ...
    (
      new OBJstream
      {
        meshRefiner_.mesh().time().path()/"implicitFeatureEdge_"
        + name(iter) + ".obj"
      }
    );
  ...
  FOR_ALL(pp.localPoints(), pointI) {
    ...
    if ((constraint.first() > patchConstraints[pointI].first())
...
void mousse::autoSnapDriver::stringFeatureEdges
(
  const label /*iter*/,
  const scalar featureCos,
  const indirectPrimitivePatch& pp,
          ...
          FOR_ALL(pEdges, pEdgeI) {
            ...
            if (patchConstraints[nbrPointI].first() <= 1
              ...
              if (mag(featVec&nbrFeatVec) > featureCos) {
                ...
                scalar d2 = magSqr(rawPatchAttraction[nbrPointI]);
                const point featPt =
                  pp.localPoints()[nbrPointI] + rawPatchAttraction[nbrPointI];
                const scalar cosAngle = (featVec & (featPt - pt));
                if (cosAngle > 0) {
...
void mousse::autoSnapDriver::releasePointsNextToMultiPatch
(
  const label iter,
  const scalar /*featureCos*/,
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  FOR_ALL(pointFacePatchID, pointI) {
  ...
  FOR_ALL(isMultiPatchPoint, pointI) {
  ...
  FOR_ALL(pp.localFaces(), faceI) {
    ...
    FOR_ALL(f, fp) {
      label pointI = f[fp];
      if (isMultiPatchPoint[pointI] && patchConstraints[pointI].first() > 1) {
        nMultiPatchPoints++;
      }
    }
...
mousse::labelPair mousse::autoSnapDriver::findDiagonalAttraction
(
  const indirectPrimitivePatch& pp,
...
void mousse::autoSnapDriver::avoidDiagonalAttraction
(
  const label /*iter*/,
  const scalar featureCos,
  const indirectPrimitivePatch& pp,
  ...
  FOR_ALL(pp.localFaces(), faceI) {
    ...
    if (diag[0] != -1 && diag[1] != -1) {
      ...
      const scalar cosAngle =
        mag(patchConstraints[i0].second() & patchConstraints[i1].second());
...
mousse::Tuple2<mousse::label, mousse::pointIndexHit>
mousse::autoSnapDriver::findNearFeatureEdge
(
...
) const
{
  ...
  if (nearInfo.hit()) {
    ...
    edgeAttractors[featI][nearInfo.index()].append(nearInfo.hitPoint());
...
mousse::Tuple2<mousse::label, mousse::pointIndexHit>
mousse::autoSnapDriver::findNearFeaturePoint
(
  ...
  FOR_ALL(pp.localPoints(), pointI) {
    ...
    if ((constraint.first() > patchConstraints[pointI].first())
      ...
      if (patchConstraints[pointI].first() == 1) {
        ...
        const pointIndexHit& info = nearInfo.second();
        if (info.hit()) {
          if (patchConstraints[pointI].first() == 3
...
void mousse::autoSnapDriver::determineBaffleFeatures
(
  const label iter,
  const scalar featureCos,
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  // to be less than 90.
  const scalar baffleFeatureCos = mousse::cos(degToRad(91));
  ...
  FOR_ALL(pointStatus, pointI) {
    ...
    if (pointStatus[pointI] == 0) {  // baffle edge
      ...
      if (featI != -1) {
        ...
        label oldPointI = pointAttractor[featI][featPointI];
        if (oldPointI == -1
            || (distSqr < magSqr(featPt-pp.localPoints()[oldPointI]))) {
          pointAttractor[featI][featPointI] = pointI;
          pointConstraints[featI][featPointI].first() = 3;
...
void mousse::autoSnapDriver::reverseAttractMeshPoints
(
  const label /*iter*/,
  const indirectPrimitivePatch& pp,
  const scalarField& /*snapDist*/,
...
) const
{
...

  {
    ...
    label nFeats = 0;
    FOR_ALL(rawPatchConstraints, pointI) {
    ...
    FOR_ALL(isFeatureEdgeOrPoint, pointI) {
  ...
  FOR_ALL(pointAttractor, featI) {
    ...
    FOR_ALL(pointAttr, featPointI) {
      if (pointAttr[featPointI] != -1) {
        ...
        if (nearInfo.hit()) {
          label pointI = ppTree.shapes().pointLabels()[nearInfo.index()];
          const point& pt = pp.localPoints()[pointI];
          ...
          if (patchConstraints[pointI].first() <= 1) {
            ...
            // Only if nearer
            if (magSqr(attraction) < magSqr(patchAttraction[pointI])) {
              patchAttraction[pointI] = attraction;
              patchConstraints[pointI] =
...
void mousse::autoSnapDriver::featureAttractionUsingFeatureEdges
(
  const label iter,
  const bool avoidSnapProblems,
  const scalar featureCos,
...
) const
{
  ...
  if (debug&meshRefinement::ATTRACTION) {
    ...
    {
      meshRefiner_.mesh().time().path()/"edgeAttractors_" + name(iter) + ".obj"
    };
    ...
    Info << "Dumping feature-point attraction to "
      << featurePointStr.name() << endl;
    FOR_ALL(patchConstraints, pointI) {
      const point& pt = pp.localPoints()[pointI];
...
void mousse::autoSnapDriver::preventFaceSqueeze
(
  const label /*iter*/,
  const scalar /*featureCos*/,
  const indirectPrimitivePatch& pp,
...
mousse::vectorField mousse::autoSnapDriver::calcNearestSurfaceFeature
(
...
) const
{
  ...
  {
    vector avgPatchDisp =
      meshRefinement::gAverage(isPatchMasterPoint, patchDisp);
    vector avgPatchAttr =
      meshRefinement::gAverage(isPatchMasterPoint, patchAttraction);
  ...
  if (featureAttract < 1-0.001) {
    ...
    if (debug&meshRefinement::ATTRACTION) {
      dumpMove
        (
          mesh.time().path()/"tangPatchDispConstrained_" + name(iter) + ".obj",
          pp.localPoints(),
          pp.localPoints() + tangPatchDisp
        );
    }
    patchDisp =
      (1.0 - featureAttract)*smoothedPatchDisp + featureAttract*tangPatchDisp;
  }
