...
)
{
  ...
  Info << "Found " << nNonManifoldPoints << " non-manifold point(s)." << endl;
  // Average points
  // ~~~~~~~~~~~~~~
  ...
  FOR_ALL(pointFaces, i) {
    ...
    if (!nonManifoldPoint.get(i)) {
      ...
      scalar blend = 0.1;
      point avgPos =
        internalBlend*avgInternal[i] + (1 - internalBlend)*avgBoundary[i];
      newPos = (1-blend)*avgPos + blend*currentPos;
    }
...
)
{
  ...
  FOR_ALL(fZone, i) {
    ...
    FOR_ALL(f, fp) {
      ...
      Map<label>::const_iterator iter = pp.meshPointMap().find(meshPointI);
...
{
  const labelListList& pointFaces = pp.pointFaces();
  tmp<pointField> tavgBoundary{new pointField{pointFaces.size(), vector::zero}};
...
) const
{
  ...
  if (debug&meshRefinement::ATTRACTION) {
    ...
    (
      new OBJstream
      {
        mesh.time().path()/"detectNearSurfaces_"
  ...
  if (returnReduce(localPoints.size(), sumOp<label>()) > 0) {
    ...
    // 1. All points to non-interface surfaces
    {
      ...
      FOR_ALL(hitInfo, pointI) {
        if (hitInfo[pointI].hit()) {
          patchDisp[pointI] =
            hitInfo[pointI].hitPoint() - localPoints[pointI];
    ...
    FOR_ALL(zonedSurfaces, i) {
      ...
      FOR_ALL(hitInfo, i) {
        ...
        if (hitInfo[i].hit()) {
          patchDisp[pointI] =
            hitInfo[i].hitPoint() - localPoints[pointI];
          minSnapDist[pointI] =
            min(minSnapDist[pointI], mag(patchDisp[pointI]));
...
) const
{
  ...
  if (debug&meshRefinement::MESH) {
    ...
    dumpMove
      (
        mesh.time().path()/"actualPatchDisplacement_"
        + meshRefiner_.timeName() + ".obj",
        pp.localPoints(),
...

  {
    ...
    FOR_ALL(pp, i) {
      ...
      if (hitSurface[i] != -1 && !isZonedFace.get(faceI)) {
        const label hSi = hitSurface[i];
        const label hRi = hitRegion[i];
        closestPatch[i] =
          globalToMasterPatch_[surfaces.globalRegion(hSi, hRi)];
      }
    }
  }
  ...
  FOR_ALL(localFaces, faceI) {
    ...
    if (f.size() >= 4) {
      ...
      for (label startFp = 0; startFp < f.size()-2; startFp++) {
        ...
        for (label endFp = f.fcIndex(f.fcIndex(startFp));
  ...
  // or merge across them.
  {
    ...
    FOR_ALL(surfZones, surfI) {
      ...
      if (faceZoneName.size()) {
        ...
        if (faceType == surfaceZonesInfo::BAFFLE
          ...
          Info
            << "Surface : " << surfaces.names()[surfI] << nl
    ...
    FOR_ALL(baffles, i) {
      const labelPair& b = baffles[i];
      if (filterFace[b.first()] == -1 && filterFace[b.second()] == -1) {
        newBaffles.append(b);
      }
    }
...

  {
    ...
    for (label iter = 0; iter < nFeatIter; iter++) {
      ...
      if (debug&meshRefinement::ATTRACTION) {
        dumpMove
          (
            mesh.time().path()/"patchDisplacement_" + name(iter) + ".obj",
            pp.localPoints(),
            pp.localPoints() + disp
          );
      }
