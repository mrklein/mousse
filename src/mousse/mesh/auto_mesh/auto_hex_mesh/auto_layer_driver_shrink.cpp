...
void mousse::autoLayerDriver::sumWeights
(
  ...
  FOR_ALL(edges, edgeI) {
    if (isMasterEdge.get(meshEdges[edgeI]) == 1) {
      ...
      scalar eMag =
        ...
        (
          VSMALL,
          mag(pts[meshPoints[e[1]]] - pts[meshPoints[e[0]]])
        );
...
void mousse::autoLayerDriver::smoothField
(
...
void mousse::autoLayerDriver::smoothPatchNormals
(
  const motionSmoother& meshMover,
  const PackedBoolList& isMasterPoint,
  const PackedBoolList& isMasterEdge,
...
void mousse::autoLayerDriver::smoothNormals
(
  const label nSmoothDisp,
  const PackedBoolList& isMasterPoint,
  const PackedBoolList& isMasterEdge,
...
bool mousse::autoLayerDriver::isMaxEdge
(
...
) const
{
  ...
  return (pointWallDist[e[0]].v() & pointWallDist[e[1]].v()) < minCos;
}


// Stop layer growth where mesh wraps around edge with a
// large feature angle
void mousse::autoLayerDriver::handleFeatureAngleLayerTerminations
(
  const scalar minCos,
  const PackedBoolList& isMasterPoint,
...
) const
{
  ...
  FOR_ALL(edgeFaceNormals, edgeI) {
    ...
    if (eFaceNormals.size() == 2) {
      ...
      label v0 = e[0];
      label v1 = e[1];
      if (extrudeStatus[v0] != NOEXTRUDE || extrudeStatus[v1] != NOEXTRUDE) {
        if (!eFaceExtrude[0] || !eFaceExtrude[1]) {
          const vector& n0 = eFaceNormals[0];
          const vector& n1 = eFaceNormals[1];
          if ((n0 & n1) < minCos) {
            if (unmarkExtrusion(v0, patchDisp, patchNLayers, extrudeStatus)) {
              if (isMasterPoint[meshPoints[v0]]) {
                nPointCounter++;
              }
            }
            if (unmarkExtrusion(v1, patchDisp, patchNLayers, extrudeStatus)) {
...
void mousse::autoLayerDriver::findIsolatedRegions
(
...
void mousse::autoLayerDriver::medialAxisSmoothingInfo
(
  const motionSmoother& meshMover,
  ...
  FOR_ALL(medialRatio, pointI) {
    if (!pointWallDist[pointI].valid(dummyTrackData)) {
      ...
      if (wDist2 < sqr(SMALL) && mDist < SMALL) {
        ...
        medialRatio[pointI] = mDist / (mousse::sqrt(wDist2) + mDist);
      }
    }
  }
  if (debug&meshRefinement::MESH || debug&meshRefinement::LAYERINFO) {
...
void mousse::autoLayerDriver::shrinkMeshMedialDistance
(
...
) const
{
  ...
  if (debug) {
    ...
    (
      new OBJstream
      {
        mesh.time().path()/"thicknessRatioExcludePoints_"
  ...
  if (debug) {
    ...
    (
      new OBJstream
      {
        mesh.time().path()/"thicknessRatioExcludeMedialVec_"
        + meshRefiner_.timeName() + ".obj"
      }
    );
    Info << "Writing points with too large an extrusion distance to "
  ...
  FOR_ALL(displacement, pointI) {
    if (!pointWallDist[pointI].valid(dummyTrackData)) {
      ...
      displacement[pointI] = -medialRatio[pointI]*pointWallDist[pointI].s()
        *dispVec[pointI];
    }
  }
    ...
    for (label iter = 0; iter < nSmoothDisplacement; iter++) {
      ...
      FOR_ALL(displacement, i) {
        if (medialRatio[i] > SMALL && medialRatio[i] < 1-SMALL) {
          displacement[i] =
            (1 - lambda)*displacement[i] + lambda*average[i];
        }
      }
      // Calculate average of field
      averageNeighbours
