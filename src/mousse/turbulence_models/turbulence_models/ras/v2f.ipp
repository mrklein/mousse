...
{
  return
    CL_*max(pow(k_, 1.5)/epsilon_, Ceta_*pow025(pow3(this->nu())/epsilon_));
}
...
:
  ...
  v2Min_{"v2Min", v2_.dimensions(), SMALL},
  fMin_{"fMin", f_.dimensions(), 0.0}
{
  bound(k_, this->kMin_);
...
void v2f<BasicTurbulenceModel>::correct()
{
  ...
  {
    type() + ":alpha", 1.0/Ts*((C1_ - N)*v2_ - 2.0/3.0*k_*(C1_ - 1.0))
  };
  const volScalarField Ceps1
  {
    "Ceps1", 1.4*(1.0 + 0.05*min(sqrt(k_/v2_), scalar(100.0)))
  };
