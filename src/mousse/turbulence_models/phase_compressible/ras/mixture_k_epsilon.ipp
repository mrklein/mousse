...
{
  typedef inletOutletFvPatchScalarField iOPSF;
  volScalarField::GeometricBoundaryField& bf = vsf.boundaryField();
  const volScalarField::GeometricBoundaryField& refBf = refVsf.boundaryField();
  FOR_ALL(bf, patchi) {
    if (isA<iOPSF>(bf[patchi]) && isA<iOPSF>(refBf[patchi])) {
      refCast<iOPSF>(bf[patchi]).refValue() =
        refCast<const iOPSF>(refBf[patchi]).refValue();
    }
  }
}
...
{
  const mixtureKEpsilon<BasicTurbulenceModel>&
...
tmp<volScalarField> mixtureKEpsilon<BasicTurbulenceModel>::rhogEff() const
{
  ...
  const twoPhaseSystem& fluid = refCast<const twoPhaseSystem>(gas.fluid());
  return
    gas.rho() + fluid.virtualMass(gas).Cvm()*fluid.otherPhase(gas).rho();
}
