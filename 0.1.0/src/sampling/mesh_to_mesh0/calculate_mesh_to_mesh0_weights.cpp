  ...
  FOR_ALL(cellAddressing_, celli) {
    if (cellAddressing_[celli] != -1) {
      ...
      if (directCelli != -1) {
        ...
        V_ += invDistCoeffs[celli][0]*fromMesh_.V()[cellAddressing_[celli]];
        for (label i = 1; i < invDistCoeffs[celli].size(); i++) {
          V_ += invDistCoeffs[celli][i]*fromMesh_.V()[neighbours[i-1]];
        }
      }
    }
  }
  ...
  FOR_ALL(cellToCell, celli) {
    ...
    if (overlapCells.size() > 0) {
      ...
      FOR_ALL(overlapCells, j) {
        ...
        {
          pointField{fromMesh_.points(), fromMesh_.cellPoints()[cellFrom]}
        };
        scalar v =
          overlapEngine.cellCellOverlapVolumeMinDecomp(toMesh_, celli,
                                                       fromMesh_, cellFrom,
                                                       bbFromMesh);
