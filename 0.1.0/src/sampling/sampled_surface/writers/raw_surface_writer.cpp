...
MAKE_SURFACE_WRITER_TYPE(rawSurfaceWriter);
...
{
  os << values.size() << nl << "#  x  y  z  " << fieldName << nl;
}
...
{
  os << values.size() << nl << "#  x  y  z  " << fieldName << "_x  "
    << fieldName << "_y  " << fieldName << "_z  " << endl;
}
...
{
  os << values.size() << nl << "#  ii  " << fieldName << "_ii" << nl;
}
...
{
  os << values.size() << nl << "#  xx  xy  xz  yy  yz ";
  for (int i=0; i<6; ++i) {
    os << fieldName << "_" << i << "  ";
  }
  os << endl;
...
{
  os << values.size() << nl << "#  xx  xy  xz  yx  yy  yz  zx  zy  zz";
  for (int i=0; i<9; ++i) {
    os << fieldName << "_" << i << "  ";
  }
  os << nl;
...
) const
{
  ...
  // header
  os << "# geometry NO_DATA " << faces.size() << nl << "#  x  y  z" << nl;
  // Write faces centres
  FOR_ALL(faces, elemI) {
    writeLocation(os, points, faces, elemI);
    os << nl;
...
DEFINE_SURFACE_WRITER_WRITE_FIELDS(mousse::rawSurfaceWriter);
