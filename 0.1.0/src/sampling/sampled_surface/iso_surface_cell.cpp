...
DEFINE_TYPE_NAME_AND_DEBUG(isoSurfaceCell, 0);

}


// Private Member Functions 
mousse::scalar mousse::isoSurfaceCell::isoFraction
(
  const scalar s0,
  const scalar s1
) const
...
bool mousse::isoSurfaceCell::isTriCut
(
  const triFace& tri,
...
mousse::isoSurfaceCell::cellCutType mousse::isoSurfaceCell::calcCutType
(
...
void mousse::isoSurfaceCell::calcCutTypes
(
  const PackedBoolList& isTet,
...
mousse::labelPair mousse::isoSurfaceCell::findCommonPoints
(
...
mousse::point mousse::isoSurfaceCell::calcCentre(const triSurface& s)
{
...
mousse::pointIndexHit mousse::isoSurfaceCell::collapseSurface
(
  const label /*cellI*/,
  ...
  if (localTris.size() == 1) {
    ...
    if (shared[0] != -1) {
      ...
      if ((n0 & n1) < 0) {
        ...
        info.setPoint(0.5*(tri0.centre(localPoints) + tri1.centre(localPoints)));
        info.setHit();
      }
    }
    ...
    labelList faceZone;
    label nZones = surf.markZones(boolList(surf.nEdges(), false), faceZone);
    if (nZones == 1) {
...
void mousse::isoSurfaceCell::calcSnappedCc
(
  const PackedBoolList& isTet,
  const scalarField& cVals,
  const scalarField& pVals,
      ...
      if (localPoints.size() == 1) {
        ...
        FOR_ALL(cFaces, cFaceI) {
          ...
          for (label i = 2; i < f.size(); i++) {
            ...
            if ((s[0] >= 0.0 && s[0] <= 0.5) && (s[1] >= 0.0 && s[1] <= 0.5)
                && (s[2] >= 0.0 && s[2] <= 0.5)) {
              if ((mesh_.faceOwner()[faceI] == cellI)
                  == (cVal >= pVals[tri[0]])) {
                localTris.append
                (
                  labelledTri{pointToLocal[tri[1]], pointToLocal[tri[0]],
                              pointToLocal[tri[2]], 0}
                );
              } else {
                localTris.append
                (
                  labelledTri{pointToLocal[tri[0]], pointToLocal[tri[1]],
                              pointToLocal[tri[2]], 0}
                );
              }
            }
        ...
        pointIndexHit info = collapseSurface(cellI, surfPoints, localTris);
        if (info.hit()) {
          snappedCc[cellI] = snappedPoints.size();
...
void mousse::isoSurfaceCell::genPointTris
(
  const scalarField& cellValues,
  const scalarField& pointValues,
  const label pointI,
...
void mousse::isoSurfaceCell::calcSnappedPoint
(
  const PackedBoolList& isTet,
  const scalarField& cVals,
  const scalarField& pVals,
...
) const
{
  ...
  FOR_ALL(mesh_.pointFaces(), pointI) {
    ...
    FOR_ALL(pFaces, i) {
      ...
      if (cellCutType_[mesh_.faceOwner()[faceI]] == CUT
    ...
    FOR_ALL(pFaces, pFaceI) {
      ...
      if (isTet.get(own) == 1) {
        ...
        genPointTris(cVals, pVals, pointI, faceI, own, localTriPoints);
      }
      if (mesh_.isInternalFace(faceI))
      {
        ...
        {
          genPointTris(cVals, pVals, pointI, faceI, nei, localTriPoints);
        }
      }
    }
...
mousse::triSurface mousse::isoSurfaceCell::stitchTriPoints
(
...
) const
{
  ...
  if ((triPoints.size() % 3) != 0) {
    FATAL_ERROR_IN("isoSurfaceCell::stitchTriPoints(..)")
      << "Problem: number of points " << triPoints.size()
      << " not a multiple of 3." << abort(FatalError);
  }
  pointField newPoints;
  mergePoints(triPoints, mergeDistance_, false, triPointReverseMap, newPoints);
  ...
  if (debug) {
    ...
    labelList oldToNew;
    bool hasMerged = mergePoints(newPoints, mergeDistance_, true, oldToNew,
                                 newNewPoints);
    if (hasMerged) {
      FATAL_ERROR_IN("isoSurfaceCell::stitchTriPoints(..)")
  ...
  if (checkDuplicates) {
    ...
    bool hasMerged = mergePoints(centres, mergeDistance_, false, oldToMerged,
                                 mergedCentres);
    if (debug) {
  ...
  return {tris, geometricSurfacePatchList(0), newPoints, true};
}


// Does face use valid vertices?
bool mousse::isoSurfaceCell::validTri(const triSurface& surf, const label faceI)
{
  ...
  FOR_ALL(fFaces, i) {
    ...
    if (((f[0] == nbrF[0]) || (f[0] == nbrF[1]) || (f[0] == nbrF[2]))
...
void mousse::isoSurfaceCell::calcAddressing
(
...
) const
{
  ...
  pointField mergedCentres;
  labelList oldToMerged;
  bool hasMerged = mergePoints(edgeCentres, mergeDistance_, false, oldToMerged,
                               mergedCentres);
  if (debug) {
    Pout << "isoSurfaceCell : detected "
      << mergedCentres.size()
...
bool mousse::isoSurfaceCell::danglingTriangle
(
...
mousse::label mousse::isoSurfaceCell::markDanglingTriangles
(
...
mousse::triSurface mousse::isoSurfaceCell::subsetMesh
(
  const triSurface& s,
...
// Constructors 
mousse::isoSurfaceCell::isoSurfaceCell
(
  const polyMesh& mesh,
  const scalarField& cVals,
  const scalarField& pVals,
  ...
  mergeDistance_{mergeTol*mesh.bounds().mag()}
{
  ...
  labelList snappedCc;
  if (regularise) {
    calcSnappedCc(isTet, cVals, pVals, snappedPoints, snappedCc);
  } else {
    snappedCc.setSize(mesh_.nCells());
    snappedCc = -1;
  }
  ...
  labelList snappedPoint;
  if (regularise) {
    calcSnappedPoint(isTet, cVals, pVals, snappedPoints, snappedPoint);
  } else {
    snappedPoint.setSize(mesh_.nPoints());
    snappedPoint = -1;
  }
  ...
  generateTriPoints(cVals, pVals, mesh_.cellCentres(), mesh_.points(),
                    snappedPoints, snappedCc, snappedPoint, triPoints,
                    triMeshCells);
  if (debug) {
  ...
  if (regularise) {
    ...
    while (true) {
      ...
      boolList keepTriangles;
      label nDangling = markDanglingTriangles(faceEdges, edgeFace0, edgeFace1,
                                              edgeFacesRest,
                                              keepTriangles);
      if (debug) {
