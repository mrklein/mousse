...
) const
{
  ...
  FOR_ALL(patchStart(), i) {
    ...
    label end =
      i < patchStart().size()-1 ? patchStart()[i+1] : patchFaceLabels().size();
...
) const
{
  ...
  FOR_ALL(patchIDs(), i) {
    ...
    FOR_ALL(samples, cellI) {
      if (samples[cellI] != point::max) {
        patchVals[cellI] =
          interpolator.interpolate(samples[cellI], cellI);
      }
    }
  ...
  return
    PrimitivePatchInterpolation<indirectPrimitivePatch>(allPatches)
