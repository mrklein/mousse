...
void mousse::linearValveLayersFvMesh::addZonesAndModifiers()
{
  ...
  fz[2] = new faceZone{"cutFaceZone", labelList{0}, boolList{0, false}, 2,
                       faceZones()};
  // Add face zone for layer addition
  const word layerPatchName{motionDict_.subDict("layer").lookup("patch")};
  const polyPatch& layerPatch = boundaryMesh()[layerPatchName];
  labelList lpf{layerPatch.size()};
  FOR_ALL(lpf, i) {
    lpf[i] = layerPatch.start() + i;
  }
  fz[3] = new faceZone{"valveLayerZone", lpf,
                       boolList{layerPatch.size(), true}, 0, faceZones()};
  ...
  tm[1] =
    new layerAdditionRemoval
    {
      ...
      topoChanger_,
      "valveLayerZone",
      readScalar(motionDict_.subDict("layer").lookup("minThickness")),
      readScalar(motionDict_.subDict("layer").lookup("maxThickness"))
    };
...
mousse::tmp<mousse::pointField> mousse::linearValveLayersFvMesh::newPoints() const
{
  ...
  const vector vel{motionDict_.lookup("pistonVelocity")};
