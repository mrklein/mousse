...
void mousse::mixerFvMesh::addZonesAndModifiers()
{
  ...
  fz[2] = new faceZone{"cutFaceZone", labelList(0),
                       boolList{0, false}, 2, faceZones()};
  List<cellZone*> cz{1};
  ...
  cz[0] = new cellZone{"movingCells", movingCells, 0, cellZones()};
...
void mousse::mixerFvMesh::calcMovingMasks() const
{
  if (debug) {
  ...
  {
    word(motionDict_.subDict("slider").lookup("inside")) + "Zone"
  };
  ...
  {
    word(motionDict_.subDict("slider").lookup("outside")) + "Zone"
  };
...
mousse::mixerFvMesh::mixerFvMesh
(
  const IOobject& io
)
:
  ...
  csPtr_
  {
    coordinateSystem::New
      (
        "coordinateSystem", motionDict_.subDict("coordinateSystem")
      )
  },
...
const mousse::scalarField& mousse::mixerFvMesh::movingPointsMask() const
{
  if (!movingPointsMaskPtr_) {
    calcMovingMasks();
  }
...
bool mousse::mixerFvMesh::update()
{
  ...
  (
    ...
    (
      ...
      + vector(0, rpm_*360.0*time().deltaTValue()/60.0, 0)*movingPointsMask()
    )
  );
