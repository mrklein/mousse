...
mousse::autoRefineDriver::autoRefineDriver
(
...
// Member Functions 
mousse::label mousse::autoRefineDriver::featureEdgeRefine
(
...
)
{
  ...
  if (meshRefiner_.features().size() && maxIter > 0) {
    for (; iter < maxIter; iter++) {
      ...
      if (returnReduce((mesh.nCells() >= refineParams.maxLocalCells()),
                       orOp<bool>())) {
...
mousse::label mousse::autoRefineDriver::surfaceOnlyRefine
(
  const refinementParameters& refineParams,
  const label maxIter
)
...
mousse::label mousse::autoRefineDriver::gapOnlyRefine
(
  const refinementParameters& refineParams,
  const label maxIter
)
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    // Grow by one layer to make sure we're covering the gap
    {
      ...
      for (label i=0; i<1; i++) {
        ...
        // Boundary faces
        for (label faceI = mesh.nInternalFaces();
             faceI < mesh.nFaces();
             faceI++) {
          label own = mesh.faceOwner()[faceI];
...
mousse::label mousse::autoRefineDriver::danglingCellRefine
(
  const refinementParameters& refineParams,
  const label nFaces,
  const label maxIter
)
{
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    // few cells
    if (nCellsToRefine == 0
        || (iter >= 1 && nCellsToRefine <= refineParams.minRefineCells())) {
      Info << "Stopping refining since too few cells selected."
        << nl << endl;
      break;
    }
...
void mousse::autoRefineDriver::removeInsideCells
(
  const refinementParameters& refineParams,
...
mousse::label mousse::autoRefineDriver::shellRefine
(
  const refinementParameters& refineParams,
  const label maxIter
)
{
  ...
  meshRefiner_.userFaceData()[0].second() =
    createWithValues<labelList>
    (
      mesh.nFaces(),
      -1,
  ...
  for (iter = 0; iter < maxIter; iter++) {
    ...
    if (refineParams.nBufferLayers() <= 2) {
      cellsToRefine =
        meshRefiner_.meshCutter().consistentSlowRefinement
        (
    ...
    if (nCellsToRefine == 0
        || (iter >= overallMaxShellLevel
            && nCellsToRefine <= refineParams.minRefineCells())) {
      Info << "Stopping refining since too few cells selected." << nl << endl;
      break;
    }
    if (debug) {
      const_cast<Time&>(mesh.time())++;
...
void mousse::autoRefineDriver::baffleAndSplitMesh
(
  const refinementParameters& refineParams,
...
void mousse::autoRefineDriver::zonify
(
  const refinementParameters& refineParams
)
{
...
void mousse::autoRefineDriver::splitAndMergeBaffles
(
  const refinementParameters& refineParams,
  const snapParameters& snapParams,
  const bool handleSnapProblems,
...
)
{
  ...
  meshRefiner_.mergeEdgesUndo(mousse::cos(degToRad(45.0)), motionDict);
  if (debug) {
    meshRefiner_.checkData();
  }
}
