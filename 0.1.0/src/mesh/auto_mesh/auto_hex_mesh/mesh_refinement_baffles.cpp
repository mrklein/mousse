...
mousse::label mousse::meshRefinement::createBaffle
(
  const label faceI,
...
) const
{
  ...
  if (debug&OBJINTERSECTIONS) {
    ...
    str.reset
    (
      new OFstream{mesh_.time().path()/timeName()/"intersections.obj"}
    );
  ...
  FOR_ALL(testFaces, i) {
    ...
    if (hit1[i].hit() && hit2[i].hit()) {
      ...
      // Pick up the patches
      ownPatch[faceI] =
        globalToMasterPatch[surfaces_.globalRegion(surface1[i], region1[i])];
      neiPatch[faceI] =
        globalToMasterPatch[surfaces_.globalRegion(surface2[i], region2[i])];
...
mousse::Map<mousse::labelPair> mousse::meshRefinement::getZoneBafflePatches
(
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::createBaffles
(
  const labelList& ownPatch,
  const labelList& neiPatch
)
{
  if (ownPatch.size() != mesh_.nFaces() || neiPatch.size() != mesh_.nFaces()) {
    FATAL_ERROR_IN_FUNCTION
      << "Illegal size :"
  ...
  if (debug) {
    ...
    FOR_ALL(syncedOwnPatch, faceI) {
      if ((ownPatch[faceI] == -1 && syncedOwnPatch[faceI] != -1)
...
void mousse::meshRefinement::checkZoneFaces() const
{
  const faceZoneMesh& fZones = mesh_.faceZones();
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::createZoneBaffles
(
  const labelList& globalToMasterPatch,
  const labelList& globalToSlavePatch,
...
)
{
  ...
  if (zonedSurfaces.size()) {
    ...
    {
      getZoneBafflePatches(false, globalToMasterPatch, globalToSlavePatch)
    };
    ...
    if (nZoneFaces > 0) {
      ...
      FOR_ALL(faceMap, faceI) {
        ...
        Map<labelPair>::const_iterator iter =
          faceToPatch.find
          (
            oldFaceI
          );
...
mousse::List<mousse::labelPair> mousse::meshRefinement::freeStandingBaffles
(
...
) const
{
  ...
  FOR_ALL(couples, i) {
    ...
    if (patches.whichPatch(couple.first())
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::mergeBaffles
(
  const List<labelPair>& couples
)
{
...
void mousse::meshRefinement::findCellZoneGeometric
(
  const pointField& neiCc,
...
void mousse::meshRefinement::findCellZoneInsideWalk
(
...
bool mousse::meshRefinement::calcRegionToZone
(
...
void mousse::meshRefinement::findCellZoneTopo
(
...
void mousse::meshRefinement::makeConsistentFaceIndex
(
...
void mousse::meshRefinement::handleSnapProblems
(
...
mousse::labelList mousse::meshRefinement::freeStandingBaffleFaces
(
...
void mousse::meshRefinement::calcPatchNumMasterFaces
(
...
mousse::label mousse::meshRefinement::markPatchZones
(
  const indirectPrimitivePatch& patch,
...
) const
{
  ...
  const scalar tol =
    PatchEdgeFaceWave<indirectPrimitivePatch, patchEdgeFaceRegion>::propagationTol();
  int dummyTrackData;
  ...
  while (true) {
    ...
    // Walk
    PatchEdgeFaceWave<indirectPrimitivePatch, patchEdgeFaceRegion> calc
    {
      mesh_,
      patch,
      changedEdges,
...
void mousse::meshRefinement::consistentOrientation
(
  const PackedBoolList& isMasterFace,
  const indirectPrimitivePatch& patch,
  const labelList& nMasterFacesPerEdge,
...
) const
{
  ...
  const scalar tol =
    PatchEdgeFaceWave<indirectPrimitivePatch, patchFaceOrientation>::propagationTol();
  int dummyTrackData;
...
// Member Functions 
void mousse::meshRefinement::baffleAndSplitMesh
(
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::splitMesh
(
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::dupNonManifoldPoints
(
...
mousse::autoPtr<mousse::mapPolyMesh> mousse::meshRefinement::zonify
(
  const point& keepPoint,
...

  {
    ...
    FOR_ALL(testFaces, i) {
      ...
      if (surface1[i] != -1) {
        ...
        if (surface2[i] != -1
            && (magSqr(hit2[i].hitPoint()) < magSqr(hit1[i].hitPoint()))) {
...

  {
    ...
    if (nFreeStanding > 0) {
      ...
      FOR_ALL(patch.addressing(), faceI) {
        ...
        if (isMasterFace[meshFaceI]) {
          label n = 1;
          if (bool(posOrientation[meshFaceI]) == meshFlipMap[meshFaceI]) {
            n = -1;
          }
  ...
  for (label faceI = 0; faceI < mesh_.nInternalFaces(); faceI++) {
    ...
    if (faceZoneI != -1) {
      ...
      if (ownZone == neiZone) {
        ...
        flip = ownZone == -1 || (neiZone != -1 && ownZone > neiZone);
      }
      meshMod.setAction
        (
          polyModifyFace
