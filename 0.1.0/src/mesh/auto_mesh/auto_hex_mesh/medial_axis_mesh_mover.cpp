...
mousse::labelList mousse::medialAxisMeshMover::getFixedValueBCs
(
...
mousse::medialAxisMeshMover::getPatch
(
  const polyMesh& mesh,
...
void mousse::medialAxisMeshMover::smoothPatchNormals
(
...
) const
{
  ...
  for (label iter = 0; iter < nSmoothDisp; iter++) {
    ...
    if ((iter % 10) == 0) {
      scalar resid =
        meshRefinement::gAverage(isPatchMasterPoint, mag(normals - average)());
...
void mousse::medialAxisMeshMover::smoothNormals
(
  const label nSmoothDisp,
...
) const
{
  ...
  for (label iter = 0; iter < nSmoothDisp; iter++) {
    ...
    if ((iter % 10) == 0) {
      scalar resid =
        meshRefinement::gAverage(isMeshMasterPoint, mag(normals - average)());
      Info << "    Iteration " << iter << "   residual " << resid << endl;
    }
    // Transfer to normals vector field
    FOR_ALL(average, pointI) {
...
bool mousse::medialAxisMeshMover::isMaxEdge
(
...
void mousse::medialAxisMeshMover::update(const dictionary& coeffDict)
{
  Info << typeName
  ...
  scalar minMedialAxisAngleCos =
    mousse::cos(degToRad(readScalar(coeffDict.lookup(angleKey))));
  ...
  const label nMedialAxisIter =
    coeffDict.lookupOrDefault<label>("nMedialAxisIter",
  ...
  const PackedBoolList isPatchMasterPoint
  {
    meshRefinement::getMasterPoints(mesh(), meshPoints)
  };
  const PackedBoolList isPatchMasterEdge
  {
    meshRefinement::getMasterEdges(mesh(), meshEdges)
  };
  ...
  // 1. Calculate distance to points where displacement is specified.
  {
    ...
    label nUnvisit =
      returnReduce(wallDistCalc.getUnsetPoints(), sumOp<label>());
    if (nUnvisit > 0) {
  ...
  //    wall.
  {
    ...
    FOR_ALL(edges, edgeI) {
      ...
      if (!pointWallDist[e[0]].valid(dummyTrackData)
    ...
    FOR_ALL(patches, patchI) {
      ...
      if (!pp.coupled() && !isA<emptyPolyPatch>(pp)
        ...
        if (pvf.fixesValue()) {
          ...
          scalar slipFeatureAngleCos = mousse::cos(degToRad(slipFeatureAngle));
          pointField pointNormals(PatchTools::pointNormals(mesh(), pp));
          FOR_ALL(meshPoints, i) {
            label pointI = meshPoints[i];
            if (pointWallDist[pointI].valid(dummyTrackData)
              ...
              scalar cosAngle = (-pointWallDist[pointI].v() & pointNormals[i]);
    ...
    FOR_ALL(pointMedialDist, pointI) {
      if (pointMedialDist[pointI].valid(dummyTrackData)) {
        medialDist_[pointI] =
          mousse::sqrt(pointMedialDist[pointI].distSqr());
  ...
  FOR_ALL(medialRatio_, pointI) {
    if (!pointWallDist[pointI].valid(dummyTrackData)) {
      ...
      //)
        ...
        medialRatio_[pointI] = mDist / (mousse::sqrt(wDist2) + mDist);
      }
    }
  }
  if (debug) {
...
bool mousse::medialAxisMeshMover::unmarkExtrusion
(
...
void mousse::medialAxisMeshMover::syncPatchDisplacement
(
...
void mousse::medialAxisMeshMover::minSmoothField
(
  const label nSmoothDisp,
  const PackedBoolList& isPatchMasterPoint,
  const PackedBoolList& isPatchMasterEdge,
...
) const
{
  ...
  for (label iter = 0; iter < nSmoothDisp; iter++) {
    ...
    FOR_ALL(field, pointI) {
      ...
      if (average[pointI] < field[pointI]
    ...
    if ((iter % 10) == 0) {
      scalar resid =
        meshRefinement::gAverage(isPatchMasterPoint, mag(field - average)());
      Info << "    Iteration " << iter << "   residual " << resid << endl;
    }
  }
}
...
// large feature angle
void mousse::medialAxisMeshMover::handleFeatureAngleLayerTerminations
(
  const scalar minCos,
  const PackedBoolList& isPatchMasterPoint,
...
void mousse::medialAxisMeshMover::findIsolatedRegions
(
...
) const
{
  ...
  if (debug) {
    ...
    (
      ...
      {
        mesh().time().path()
        /"islandExcludePoints_" + mesh().time().timeName() + ".obj"
      }
    );
    Info << typeName
...
void mousse::medialAxisMeshMover::smoothLambdaMuDisplacement
(
  const label nSmoothDisp,
  const PackedBoolList& isMeshMasterPoint,
  const PackedBoolList& isMeshMasterEdge,
...
) const
{
  ...
  for (label iter = 0; iter < nSmoothDisp; iter++) {
    ...
    if ((iter % 10) == 0) {
      scalar resid =
        meshRefinement::gAverage(isMeshMasterPoint,
...
// Constructors 
mousse::medialAxisMeshMover::medialAxisMeshMover
(
...
:
  ...
  medialVec_
  {
    ...
    pMesh(),
    {"medialVec", dimLength, vector::zero}
  }
{
  update(dict);
}
...
// Member Functions 
void mousse::medialAxisMeshMover::calculateDisplacement
(
...
)
{
  ...
  const scalar minCosLayerTermination =
    mousse::cos(degToRad(0.5*featureAngle));
  //- Smoothing wanted patch thickness
  const label nSmoothPatchThickness =
    readLabel(coeffDict.lookup("nSmoothThickness"));
  //- Number of edges walking out
  const label nMedialAxisIter =
    coeffDict.lookupOrDefault<label>
    (
      "nMedialAxisIter",
      mesh().globalData().nTotalPoints()
    );
  //- Use strict extrusionIsland detection
  const Switch detectExtrusionIsland =
    coeffDict.lookupOrDefault<Switch>("detectExtrusionIsland", false);
  ...
  if (debug)
  {
    ...
    (
      ...
      {
        mesh().time().path()
        /"thicknessRatioExcludePoints_" + mesh().time().timeName() + ".obj"
      }
    );
    Info << typeName
  ...
  if (debug) {
    ...
    (
      ...
      {
        mesh().time().path()
        /"thicknessRatioExcludeMedialVec_" + mesh().time().timeName() + ".obj"
      }
    );
    Info << typeName
  ...
  FOR_ALL(meshPoints, patchPointI) {
    if (extrudeStatus[patchPointI] != autoLayerDriver::NOEXTRUDE) {
      ...
      vector n =
        patchDisp[patchPointI]/(mag(patchDisp[patchPointI]) + VSMALL);
      ...
      if (thicknessRatio > maxThicknessToMedialRatio) {
        ...
        if (medialVecStr.valid()) {
          ...
          medialVecStr().write
            (
              linePointRef{pt, medialVec_[pointI]}
            );
        }
      }
    }
  ...
  FOR_ALL(displacement, pointI) {
    if (!pointWallDist[pointI].valid(dummyTrackData)) {
      ...
      displacement[pointI] = -medialRatio_[pointI]*pointWallDist[pointI].s()
        *dispVec_[pointI];
    }
  }
...
bool mousse::medialAxisMeshMover::shrinkMesh
(
...
bool mousse::medialAxisMeshMover::move
(
...
void mousse::medialAxisMeshMover::movePoints(const pointField& p)
{
