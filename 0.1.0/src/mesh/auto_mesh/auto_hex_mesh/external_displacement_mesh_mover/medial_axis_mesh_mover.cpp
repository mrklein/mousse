...
void mousse::medialAxisMeshMover::update(const dictionary& coeffDict)
{
  ...
  scalar minMedialAxisAngleCos = mousse::cos
  (
  ...
  //    wall.
  {
    ...
    forAll(patches, patchI)
    {
      ...
      )
      {
        ...
        else
        {
          ...
          scalar slipFeatureAngleCos = mousse::cos
          (
    ...
    forAll(pointMedialDist, pointI)
    {
      if (pointMedialDist[pointI].valid(dummyTrackData))
      {
        medialDist_[pointI] = mousse::sqrt
        (
...
// large feature angle
void mousse::medialAxisMeshMover::
...
)
{
  ...
  const scalar minCosLayerTermination = mousse::cos
  (
