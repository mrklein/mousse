...
void mousse::blockMesh::createPoints() const
{
  ...
  FOR_ALL(blocks, blockI) {
    ...
    FOR_ALL(blockPoints, blockPointI) {
      points_[mergeList_[blockOffsets_[blockI] + blockPointI]] =
    ...
    FOR_ALL(blockCells, blockCellI) {
      ...
      FOR_ALL(cellPoints, cellPointI) {
        cellPoints[cellPointI] =
          mergeList_[blockCells[blockCellI][cellPointI] + blockOffsets_[blockI]];
      }
      ...
      if (blockFaces[blockFaceLabel]
        ...
        FOR_ALL(blockPatchFaces, blockFaceLabel) {
          ...
          quadFace[0] =
            mergeList_
            [
              blockPatchFaces[blockFaceLabel][0] + blockOffsets_[blockI]
            ];
          label nUnique = 1;
          for (label facePointLabel = 1;
               facePointLabel < 4;
               facePointLabel++) {
            quadFace[nUnique] =
              mergeList_
          ...
          if (nUnique == 4) {
            ...
            patchFaces[faceLabel++] =
              face(labelList::subList(quadFace, 3));
          }
