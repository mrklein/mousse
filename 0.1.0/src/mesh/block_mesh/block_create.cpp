...
void mousse::block::createPoints() const
{
      ...
      for (label i = 0; i <= ni; i++) {
        ...
        vertices_[vertexNo] =
          (edgex1 + edgex2 + edgex3 + edgex4
           ...
           + edgez1 + edgez2 + edgez3 + edgez4)/3.0;
        vertices_[vertexNo] +=
...
void mousse::block::createCells() const
{
  const label ni = meshDensity().x();
  const label nj = meshDensity().y();
  const label nk = meshDensity().z();
...
void mousse::block::createBoundary() const
{
  const label ni = meshDensity().x();
  const label nj = meshDensity().y();
  const label nk = meshDensity().z();
