...
:
  ...
  pressure_
  {
    ptf.pressure_.valid() ? ptf.pressure_().clone().ptr() : NULL
  }
{}
...
)
{
  ...
  if (psiName_ == "none" && rhoName_ == "none") {
    ...
    if (gamma_ > 1.0) {
      ...
      operator==
      (
        p0/pow((1.0 + 0.5*psip*gM1ByG*(1.0 - pos(phip))*magSqr(Up)),
               1.0/gM1ByG)
      );
    } else {
    ...
    FATAL_ERROR_IN("uniformTotalPressureFvPatchScalarField::updateCoeffs()")
