...
void mousse::fvPatchMapper::calcAddressing() const
{
  ...
  if (direct()) {
    ...
    // internal
    FOR_ALL(addr, faceI) {
      if (addr[faceI] >= oldPatchStart && addr[faceI] < oldPatchEnd) {
    ...
    interpolationAddrPtr_ =
      new labelListList{patch_.patchSlice(faceMap_.addressing())};
    labelListList& addr = *interpolationAddrPtr_;
    weightsPtr_ =
      new scalarListList{patch_.patchSlice(faceMap_.weights())};
    ...
    FOR_ALL(addr, faceI) {
      ...
      if (min(curAddr) >= oldPatchStart && max(curAddr) < oldPatchEnd) {
        ...
        label nActive = 0;
        FOR_ALL(curAddr, lfI) {
          if (curAddr[lfI] >= oldPatchStart && curAddr[lfI] < oldPatchEnd) {
