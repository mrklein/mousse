...
) const
{
  ...
  FOR_ALL(sfCorr, facei) {
    point pi = w[owner[facei]]*C[owner[facei]]
    ...
    scalar sumAt = at;
    Type sumPsip = at*(sfCorr[facei] + pvf[f[0]] + pvf[f[f.size()-1]])/3.0;
    for (label pointi=1; pointi<f.size(); pointi++) {
      ...
      sumAt += at;
      sumPsip += at*(sfCorr[facei] + pvf[f[pointi]] + pvf[f[pointi-1]])/3.0;
    }
  ...
  FOR_ALL(bSfCorr, patchi) {
    ...
    if (pSfCorr.coupled()) {
      ...
      FOR_ALL(pOwner, facei) {
        ...
        point pi = pWghts[facei]*C[own] + (1.0 - pWghts[facei])*pNbrC[facei];
        ...
        scalar sumAt = at;
        Type sumPsip = at*(pSfCorr[facei] + pvf[f[0]] + pvf[f[f.size()-1]])/3.0;
        for (label pointi=1; pointi<f.size(); pointi++) {
          at = triangle<point, const point&>
          (
            pi,
          ...
          sumAt += at;
          sumPsip += at*(pSfCorr[facei] + pvf[f[pointi]] + pvf[f[pointi-1]])/3.0;
        }
