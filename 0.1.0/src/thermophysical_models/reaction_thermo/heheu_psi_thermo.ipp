  ...
  FOR_ALL(this->T_.boundaryField(), patchi) {
    ...
    if (pT.fixesValue()) {
      ...
      FOR_ALL(pT, facei) {
        ...
        palpha_[facei] = mixture_.alphah(pp[facei], pT[facei]);
        pTu[facei] =
          this->patchFaceReactants(patchi, facei).THE
          (
            pheu[facei],
            pp[facei],
            pTu[facei]
...
mousse::heheuPsiThermo<BasicPsiThermo, MixtureType>::Tb() const
{
  ...
  FOR_ALL(Tb_.boundaryField(), patchi) {
    ...
    FOR_ALL(pTb, facei) {
      pTb[facei] =
        this->patchFaceProducts(patchi, facei).THE
        (
          ph[facei],
          pp[facei],
          pT[facei]
...
mousse::heheuPsiThermo<BasicPsiThermo, MixtureType>::psiu() const
{
  ...
  FOR_ALL(psiu.boundaryField(), patchi) {
    ...
    FOR_ALL(ppsiu, facei) {
      ppsiu[facei] =
        this->patchFaceReactants(patchi, facei).psi(pp[facei], pTu[facei]);
    }
  }
  return tpsiu;
}
...
mousse::heheuPsiThermo<BasicPsiThermo, MixtureType>::muu() const
{
  ...
  FOR_ALL(muuCells, celli) {
    muuCells[celli] =
      this->cellReactants(celli).mu(pCells[celli], TuCells[celli]);
  }
  FOR_ALL(muu_.boundaryField(), patchi) {
    ...
    FOR_ALL(pMuu, facei) {
      pMuu[facei] =
        this->patchFaceReactants(patchi, facei).mu(pp[facei], pTu[facei]);
    }
  }
  return tmuu;
}
...
mousse::heheuPsiThermo<BasicPsiThermo, MixtureType>::mub() const
{
  ...
  FOR_ALL(mubCells, celli) {
    mubCells[celli] =
      this->cellProducts(celli).mu(pCells[celli], TbCells[celli]);
  }
  FOR_ALL(mub_.boundaryField(), patchi) {
    ...
    FOR_ALL(pMub, facei) {
      pMub[facei] =
        this->patchFaceProducts(patchi, facei).mu(pp[facei], pTb[facei]);
    }
  }
  return tmub;
}
