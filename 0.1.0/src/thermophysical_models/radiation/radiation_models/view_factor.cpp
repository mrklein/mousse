...
ADD_TO_RADIATION_RUN_TIME_SELECTION_TABLES(viewFactor);
...
void mousse::radiation::viewFactor::calculate()
{
  ...
  FOR_ALL(selectedPatches_, i) {
    ...
    greyDiffusiveViewFactorFixedValueFvPatchScalarField& Qrp =
      refCast<greyDiffusiveViewFactorFixedValueFvPatchScalarField>(QrPatch);
  ...
  SubList<label>{compactGlobalIds, nLocalCoarseFaces_}.assign(localGlobalIds);
    ...
    if (!constEmissivity_) {
      ...
      for (label i=0; i<totalNCoarseFaces_; i++) {
        for (label j=0; j<totalNCoarseFaces_; j++) {
          ...
          scalar sigmaT4 = physicoChemical::sigma.value()*pow(T[j], 4.0);
          if (i==j) {
      ...
      for (label i=0; i<totalNCoarseFaces_; i++) {
        for (label j=0; j<totalNCoarseFaces_; j++) {
          scalar sigmaT4 =
            constant::physicoChemical::sigma.value()*pow(T[j], 4.0);
          if (i==j) {
            q[i] += (Fmatrix_()[i][j] - 1.0)*sigmaT4  - QrExt[j];
          } else {
            q[i] += Fmatrix_()[i][j]*sigmaT4;
  ...
  return
    tmp<volScalarField>
    {
      new volScalarField
      {
        ...
        mesh_,
        {"zero", dimMass/pow3(dimTime)/dimLength/pow4(dimTemperature), 0.0}
      }
    };
}
