...
// Synchronise edges
void mousse::createShellMesh::syncEdges
(
...
)
{
  ...
  labelPairList
    cppEdgeData{map.constructSize(), labelPair(labelMax, labelMax)};
...
void mousse::createShellMesh::calcPointRegions
(
  const globalMeshData& globalData,
...
)
{
  ...
  PatchTools::matchEdges(cpp, patch, coupledEdges, patchEdges,
                         sameEdgeOrientation);
  ...
  FOR_ALL(patch.edgeFaces(), edgeI) {
    if (!nonManifoldEdge[edgeI]) {
      ...
      allEdgeData[edgeI] = labelPair(pointGlobalRegions[faceI][fp0],
  ...
  syncEdges(globalData, patchEdges, coupledEdges, sameEdgeOrientation,
            syncNonCollocated, isChangedEdge, changedEdges, allEdgeData);
    ...
    FOR_ALL(changedFaces, i) {
      ...
      FOR_ALL(fEdges, fp) {
        ...
        if (!nonManifoldEdge[edgeI]) {
          ...
          if ((allEdgeData[edgeI][0] > region0)
...
// Constructors 
mousse::createShellMesh::createShellMesh
(
  const primitiveFacePatch& patch,
...
// Member Functions 
void mousse::createShellMesh::setRefinement
(
...
)
{
  ...
  if (topPatchID.size() != patch_.size()
  ...
  DynamicList<label>
    faceToFaceMap{(nLayers+1)*(patch_.size()+patch_.nEdges())};
  ...
  FOR_ALL(patch_, faceI) {
    for (label layerI = 0; layerI < nLayers; layerI++) {
      addedCells[nLayers*faceI+layerI] =
        meshMod.addCell
        (
          -1,                     // masterPointID
          -1,                     // masterEdgeID
  ...
  FOR_ALL(regionPoints_, regionI) {
    ...
    for (label layerI = 0; layerI < nLayers; layerI++) {
      pt += disp;
      addedPoints[nLayers*regionI+layerI] =
        meshMod.addPoint
        (
          pt,                     // point
...
void mousse::createShellMesh::updateMesh(const mapPolyMesh& map)
{
