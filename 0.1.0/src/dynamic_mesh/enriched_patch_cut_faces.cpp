...
const mousse::label mousse::enrichedPatch::maxFaceSizeDebug_ = 100;
...
// l - left turn
// r - right turn
void mousse::enrichedPatch::calcCutFaces() const
{
  ...
  HashSet<edge, Hash<edge>>
    edgesUsedTwice(pp.size()*primitiveMesh::edgesPerPoint_);
  FOR_ALL(lf, faceI) {
      ...
      do {
        ...
        FOR_ALL(nextPoints, nextI) {
          ...
          if (nextPoints[nextI] != prevPointLabel) {
            ...
            newDir /= magNewDir;
            scalar curAtanTurn = atan2(newDir & right, newDir & ahead);
        ...
        if (mp[bestAtanPoint] == cutFaceGlobalPoints[0]) {
          ...
          completedCutFace = true;
          if (debug) {
            Pout << " local: " << cutFaceLocalPoints << " one side: " << faceI;
          }
          // Append the face
          face cutFaceGlobal;
          ...
          if (faceI < slavePatch_.size()) {
            ...
            if (mpfAddrIter != masterPointFaceAddr.end()) {
              ...
              for (label pointI = 1; pointI < cutFaceGlobal.size(); pointI++) {
                Map<labelList>::const_iterator
                  mpfAddrPointIter =
                    masterPointFaceAddr.find(cutFaceGlobal[pointI]);
                if (mpfAddrPointIter == masterPointFaceAddr.end()) {
                  // Point is off the master patch. Skip
                  miss = true;
                  break;
                }
                const labelList& curMasterFaces = mpfAddrPointIter();
                // For every current face, try to find it in the
                // zero-list
                FOR_ALL(curMasterFaces, i) {
                  FOR_ALL(masterFacesOfPZero, j) {
                    if (curMasterFaces[i] == masterFacesOfPZero[j]) {
                      hits[j]++;
                      break;
                    }
                  }
              ...
              if (!miss) {
                FOR_ALL(hits, pointI) {
                  if (hits[pointI] == cutFaceGlobal.size()) {
                    // Found other side.
                    otherSideFound = true;
                    cfMaster.append(masterFacesOfPZero[pointI]);
          ...
          if (debug || cutFaceGlobalPoints.size() > faceSizeDebug) {
            ...
            FOR_ALL(cutFaceGlobalPoints, checkI) {
              for (label checkJ = checkI + 1;
                   checkJ < cutFaceGlobalPoints.size();
                   checkJ++) {
                if (cutFaceGlobalPoints[checkI] == cutFaceGlobalPoints[checkJ]) {
                  ...
                  if (faceI < slavePatch_.size()) {
                    ...
                    origFaceLocal = slavePatch_.localFaces()[faceI];
                  } else {
                    origFace = masterPatch_[faceI - slavePatch_.size()];
                    origFaceLocal =
...
void mousse::enrichedPatch::clearCutFaces()
{
...
// Member Functions 
const mousse::faceList& mousse::enrichedPatch::cutFaces() const
{
  if (!cutFacesPtr_) {
    calcCutFaces();
  }
...
const mousse::labelList& mousse::enrichedPatch::cutFaceMaster() const
{
...
const mousse::labelList& mousse::enrichedPatch::cutFaceSlave() const
{
  if (!cutFaceSlavePtr_) {
    calcCutFaces();
  }
