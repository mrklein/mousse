...
{
  Pout << nl << "Current coupling info:" << endl;
...
)
{
  ...
  {
    fvMeshAdder::findSharedPoints(mesh_, mergeTol_)
  };
  ...
  FOR_ALL(patches, patchI) {
    ...
    if (pp.coupled()) {
      ...
      SubList<label>(nbrNewNbrProc, pp.size(), offset)
        .assign(UIndirectList<label>{distribution, pp.faceCells()}());
    }
  }
  ...
  FOR_ALL(patches, patchI) {
    ...
    if (isA<processorPolyPatch>(pp)) {
      ...
      if (isA<processorCyclicPolyPatch>(pp)) {
        patchI =
          refCast<const processorCyclicPolyPatch>(pp).referPatchID();
      }
      FOR_ALL(pp, i) {
        label bndI = offset + i;
  ...
  FOR_ALL(sourceFace, bFaceI) {
    if (sourceProc[bFaceI] != -1 && sourcePatch[bFaceI] == -1) {
      ...
      if (iter != map.end()) {
        ...
        slaveCoupledFaces[coupledI] = domainMesh.nInternalFaces() + nbrBFaceI;
        coupledI++;
      }
    }
  }
      ...
      if (!procPatchID[procI].found(referPatchID[bFaceI])) {
        ...
        if (referPatchID[bFaceI] == -1) {
          ...
          const word patchName =
            "procBoundary" + name(Pstream::myProcNo()) + "to" + name(procI);
          processorPolyPatch pp
            {
              ...
              mesh_.boundaryMesh(),
              Pstream::myProcNo(),
              static_cast<int>(nbrProc[bFaceI])
            };
...
)
{
  ...
  FOR_ALL(pZonePtrs, i) {
    pZonePtrs[i] =
      new pointZone{pointZoneNames[i], zonePoints[i], i, domainMesh.pointZones()};
  }
  List<faceZone*> fZonePtrs{faceZoneNames.size()};
  FOR_ALL(fZonePtrs, i) {
    fZonePtrs[i] =
      new faceZone{faceZoneNames[i], zoneFaces[i], zoneFaceFlip[i], i,
                   domainMesh.faceZones()};
  }
  List<cellZone*> cZonePtrs{cellZoneNames.size()};
  FOR_ALL(cZonePtrs, i) {
    cZonePtrs[i] =
      new cellZone{cellZoneNames[i], zoneCells[i], i, domainMesh.cellZones()};
  }
...
)
{
  ...
  labelList sourceProc;
  labelList sourceNewNbrProc;
  getNeighbourData(distribution, sourceFace, sourceProc, sourcePatch,
                   sourceNewNbrProc);
  ...
  const wordList
    volSphereTensors{mesh_.names(volSphericalTensorField::typeName)};
  ...
  const wordList
    surfSphereTensors{mesh_.names(surfaceSphericalTensorField::typeName)};
  checkEqualWordList("surfaceSphericalTensorFields", surfSphereTensors);
  const wordList
    surfSymmTensors{mesh_.names(surfaceSymmTensorField::typeName)};
  ...
  // Subset the part that stays
  {
    ...
    {
      doRemoveCells(select(false, distribution, Pstream::myProcNo()),
                    oldInternalPatchI)
    };
    // Addressing from subsetted mesh
    subCellMap[Pstream::myProcNo()] = subMap().cellMap();
    subFaceMap[Pstream::myProcNo()] =
      renumber(repatchFaceMap, subMap().faceMap());
  ...
  FOR_ALL(nSendCells, sendProc) {
    ...
    if (sendProc != Pstream::myProcNo()
      ...
      UIPstream str{static_cast<int>(sendProc), pBufs};
      ...
      constructPatchMap[sendProc] = identity(domainMesh.boundaryMesh().size());
      // Print a bit.
      if (debug) {
      ...
      autoPtr<mapAddedPolyMesh> map =
        fvMeshAdder::add(mesh_, domainMesh, couples, false);
  ...
  // Get boundary faces to be repatched. Is -1 or new patchID
  labelList
    newPatchID{getBoundaryPatch(sourceNewNbrProc, sourcePatch, procPatchID)};
