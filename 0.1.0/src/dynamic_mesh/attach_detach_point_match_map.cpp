...
void mousse::attachDetach::calcPointMatchMap() const
{
  ...
  primitiveFacePatch reverseSlavePatch{faceList(slavePatch.size()),
                                       mesh.points()};
  ...
  FOR_ALL(masterLocalFaces, faceI) {
    ...
    FOR_ALL(curMasterPoints, pointI) {
      ...
      if (masterMeshPoints[curMasterPoints[pointI]]
          != slaveMeshPoints[curSlavePoints[pointI]]) {
        // Grab the addressing
        removedPointMap.insert(slaveMeshPoints[curSlavePoints[pointI]],
