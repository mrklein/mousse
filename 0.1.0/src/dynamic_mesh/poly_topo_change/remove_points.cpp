...
) const
{
  ...
  syncTools::syncPointList(mesh_, pointCanBeDeleted, andEqOp<bool>(), true);
  label nDeleted = 0;
    ...
    FOR_ALL(savedFaceLabels_, saveI) {
      ...
      if (keptFace != f) {
        FATAL_ERROR_IN("setRefinement")
          ...
          << "saved points:"
          << BiIndirectList<point>{mesh_.points(), savedPoints_, savedFace}()
          << nl
          << abort(FatalError);
      }
    }
...
) const
{
...

  {
    ...
    syncTools::syncBoundaryFaceList(mesh_, faceVertexRestore,
