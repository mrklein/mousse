...
void mousse::addPatchCellLayer::addVertex
(
  const label pointI,
  face& f,
  label& fp
...
// dealt with already)
bool mousse::addPatchCellLayer::sameEdgeNeighbour
(
...
) const
{
  ...
  return
    ...
    && (addedPoints_[e[0]].size() /* is extruded */ || addedPoints_[e[1]].size())
...
mousse::labelPair mousse::addPatchCellLayer::getEdgeString
(
  const indirectPrimitivePatch& pp,
  const labelListList& globalEdgeFaces,
  const boolList& doneEdge,
...
) const
{
  ...
  if (startFp != -1) {
    ...
    label nbrGlobalFaceI = nbrFace(globalEdgeFaces, fEdges[startFp],
                                   globalFaceI);
    if (nbrGlobalFaceI == -1) {
      ...
      while (true) {
        ...
        if (!sameEdgeNeighbour(pp, globalEdgeFaces, doneEdge, globalFaceI,
                               nbrGlobalFaceI, fEdges[prevFp])) {
          break;
        }
        startFp = prevFp;
      }
...
mousse::label mousse::addPatchCellLayer::addSideFace
(
  const indirectPrimitivePatch& pp,
...
) const
{
  ...
  if (nbrFaceI == -1) {
    ...
    FOR_ALL(meshFaces, k) {
      ...
      if ((faceI != meshFaceI) && (patches.whichPatch(faceI) == newPatchID)) {
    ...
    if (addedCells[ownFaceI].size() > addedCells[nbrFaceI].size()) {
      label offset = addedCells[ownFaceI].size() - addedCells[nbrFaceI].size();
      layerOwn = layerI;
      layerNbr = (layerI <= offset) ? 0 : layerI - offset;
    } else if (addedCells[nbrFaceI].size() > addedCells[ownFaceI].size()) {
      label offset = addedCells[nbrFaceI].size() - addedCells[ownFaceI].size();
      layerNbr = layerI;
      layerOwn = (layerI <= offset) ? 0 : layerI - offset;
    } else {
...
mousse::label mousse::addPatchCellLayer::findProcPatch
(
  const polyMesh& mesh,
...
)
{
  ...
  FOR_ALL(mesh.globalData().processorPatches(), i) {
    ...
    if (refCast<const processorPolyPatch>(patches[patchI]).neighbProcNo()
        == nbrProcID) {
      return patchI;
    }
  }
...
void mousse::addPatchCellLayer::setFaceProps
(
  const polyMesh& mesh,
  const label faceI,
  label& patchI,
...
// Constructors 
// Construct from mesh
mousse::addPatchCellLayer::addPatchCellLayer
(
  const polyMesh& mesh,
...
// Member Functions 
mousse::labelListList mousse::addPatchCellLayer::addedCells
(
  const polyMesh& mesh,
...
mousse::labelListList mousse::addPatchCellLayer::addedCells() const
{
...
mousse::labelListList mousse::addPatchCellLayer::globalEdgeFaces
(
  const polyMesh& mesh,
...
)
{
  ...
  syncTools::syncEdgeList(mesh, globalEdgeFaces, uniqueEqOp(), labelList{});
  // Extract pp part
  return labelListList{UIndirectList<labelList>{globalEdgeFaces, meshEdges}};
}


void mousse::addPatchCellLayer::calcSidePatch
(
  const polyMesh& mesh,
  const globalIndex& globalFaces,
  const labelListList& globalEdgeFaces,
...
)
{
  ...
  FOR_ALL(globalEdgeFaces, edgeI) {
    ...
    if (eGlobalFaces.size() == 2 && pp.edgeFaces()[edgeI].size() == 1) {
  ...
  FOR_ALL(edgeFaces, edgeI) {
    if (edgeFaces[edgeI].size() == 1 && sidePatchID[edgeI] == -1) {
      ...
      const labelList& meshFaces =
        mesh.edgeFaces(meshEdgeI, dynMeshEdgeFaces);
      FOR_ALL(meshFaces, k) {
        label faceI = meshFaces[k];
        if (faceI != myFaceI && !mesh.isInternalFace(faceI)) {
          setFaceProps(mesh, faceI, sidePatchID[edgeI], sideZoneID[edgeI],
                       sideFlip[edgeI]);
...
void mousse::addPatchCellLayer::setRefinement
(
  const globalIndex& globalFaces,
  const labelListList& globalEdgeFaces,
...
)
{
  ...
  if (pp.nPoints() != firstLayerDisp.size()
  ...
  if (debug) {
    // Check synchronisation
    {
      ...
      syncTools::syncPointList(mesh_, nFromFace, maxEqOp<label>(), label{0});
      FOR_ALL(nPointLayers, i) {
        label meshPointI = meshPoints[i];
        if (nPointLayers[i] > 0 && nPointLayers[i] != nFromFace[meshPointI]) {
          FATAL_ERROR_IN
          (
            "addPatchCellLayer::setRefinement"
            "(const scalar, const indirectPrimitivePatch&"
...

    {
      ...
      syncTools::syncPointList(mesh_, d, minEqOp<vector>(), vector::max);
      FOR_ALL(meshPoints, i) {
        label meshPointI = meshPoints[i];
  ...
  FOR_ALL(pp, patchFaceI) {
    if (nFaceLayers[patchFaceI] > 0) {
      ...
      label meshFaceI = pp.addressing()[patchFaceI];
      label ownZoneI =
        mesh_.cellZones().whichZone(mesh_.faceOwner()[meshFaceI]);
      ...
      FOR_ALL(addedCells[patchFaceI], i) {
        FOR_ALL(f, fp) {
          if (addedPoints_[f[fp]].empty()) {
            // Keep original point
            newFace[fp] =
              addToMesh_ ? meshPoints[f[fp]] : copiedPatchPoints[f[fp]];
          } else {
            // Get new outside point
            label offset =
              addedPoints_[f[fp]].size()- addedCells[patchFaceI].size();
      ...
      if ((nPointLayers[e[0]] == 0) && (nPointLayers[e[1]] == 0)) {
        ...
        FOR_ALL(eFaces, i) {
          meshEdgeLayers[meshEdgeI] =
            max(nFaceLayers[eFaces[i]], meshEdgeLayers[meshEdgeI]);
        }
      }
    }
    syncTools::syncEdgeList(mesh_, meshEdgeLayers, maxEqOp<label>(), label{0});
    FOR_ALL(meshEdges, edgeI) {
  ...
  FOR_ALL(pp, patchFaceI) {
    ...
    FOR_ALL(fEdges, fp) {
      ...
      labelPair indexPair
      {
        getEdgeString(pp, globalEdgeFaces, doneEdge, patchFaceI,
        ...
        for (label i = 0; i < numEdgeSideFaces; i++) {
          ...
          if (i == 0) {
            ...
            if (addedPoints_[vEnd].size()) {
              newFp += addedPoints_[vEnd].size() - numEdgeSideFaces;
            }
            if (addedPoints_[vStart].size()) {
              newFp += addedPoints_[vStart].size() - numEdgeSideFaces;
            }
          }
          face newFace{newFp};
          newFp = 0;
          ...
          if (i == 0) {
            FOR_ALL(stringedVerts, stringedI) {
              ...
              addVertex
                (
                  addToMesh_ ? meshPoints[v] : copiedPatchPoints[v],
                  newFace,
                  newFp
                );
            }
            ...
            FOR_ALL(stringedVerts, stringedI) {
              ...
              if (addedPoints_[v].size()) {
                label offset = addedPoints_[v].size() - numEdgeSideFaces;
                addVertex
                  (
          ...
          if (numEdgeSideFaces < addedPoints_[vEnd].size()) {
            if (i == 0 && addedPoints_[vEnd].size()) {
              label offset = addedPoints_[vEnd].size() - numEdgeSideFaces;
          ...
          if (numEdgeSideFaces < addedPoints_[vStart].size()) {
            if (i == 0 && addedPoints_[vStart].size()) {
              label offset = addedPoints_[vStart].size() - numEdgeSideFaces;
            ...
            if (debug) {
              ...
              FOR_ALL(newFace, fp) {
                if (!verts.insert(newFace[fp]))
                {
                  ...
                  << " at:"
                  << mesh_.edges()[meshEdgeI].line(mesh_.points())
                  << nl
                  << "string:" << stringedVerts
                  << "stringpoints:"
                  << UIndirectList<point>(pp.localPoints(), stringedVerts)
                  << nl
                  << "stringNLayers:"
                  << UIndirectList<label>(nPointLayers, stringedVerts)
                  << nl
                  << abort(FatalError);
                }
              }
            }
            label nbrFaceI = nbrFace(pp.edgeFaces(), startEdgeI, patchFaceI);
            const labelList& meshFaces = mesh_.edgeFaces(meshEdgeI, ef);
            addSideFace
              (
                pp,
...
void mousse::addPatchCellLayer::updateMesh
(
