#ifndef DYNAMIC_MESH_POLY_TOPO_CHANGE_POLY_TOPO_CHANGE_HEX_REF8_HPP_
#define DYNAMIC_MESH_POLY_TOPO_CHANGE_POLY_TOPO_CHANGE_HEX_REF8_HPP_
...
public:
  ...
  // Member Functions
    // Access
      ...
      const labelIOList& cellLevel() const { return cellLevel_; }
      const labelIOList& pointLevel() const { return pointLevel_; }
      const refinementHistory& history() const { return history_; }
      //- Typical edge length between unrefined points
      scalar level0EdgeLength() const { return level0Edge_.value(); }
    // Refinement
