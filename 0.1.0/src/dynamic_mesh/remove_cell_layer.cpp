...
bool mousse::layerAdditionRemoval::validCollapse() const
{
  ...
  FOR_ALL(mf, faceI) {
    if (!mesh.isInternalFace(mf[faceI]) && !mesh.isInternalFace(ftc[faceI])) {
      nBoundaryHits++;
    }
  }
  if (debug) {
...
) const
{
  ...
  FOR_ALL(mc, cellI) {
    ...
    FOR_ALL(curCell, faceI) {
      ...
      if (mesh.faceZones().whichZone(curCell[faceI]) != faceZoneID_.index()) {
  ...
  FOR_ALL(ftm, faceI) {
    ...
    FOR_ALL(newFace, pointI) {
      Map<label>::iterator rpmIter = removedPointMap.find(newFace[pointI]);
