...
#include "mesh_cut_and_remove.hpp"
...
mousse::label mousse::meshCutAndRemove::firstCommon
(
...
bool mousse::meshCutAndRemove::isIn
(
  const edge& twoCuts,
...
mousse::label mousse::meshCutAndRemove::findCutCell
(
  const cellCuts& cuts,
...
//  none found.
mousse::label mousse::meshCutAndRemove::findInternalFacePoint
(
...
// to the patch
mousse::label mousse::meshCutAndRemove::findPatchFacePoint
(
  const face& f,
...
void mousse::meshCutAndRemove::faceCells
(
  const cellCuts& cuts,
  const label exposedPatchI,
  const label faceI,
...
void mousse::meshCutAndRemove::getZoneInfo
(
  const label faceI,
  label& zoneID,
  bool& zoneFlip
...
void mousse::meshCutAndRemove::addFace
(
...
void mousse::meshCutAndRemove::modFace
(
  polyTopoChange& meshMod,
  const label faceI,
  const face& newFace,
...
void mousse::meshCutAndRemove::copyFace
(
  const face& f,
  const label startFp,
  const label endFp,
...
// (addedPoints_).
void mousse::meshCutAndRemove::splitFace
(
  const face& f,
  const label v0,
  const label v1,
...
mousse::face mousse::meshCutAndRemove::addEdgeCutsToFace(const label faceI) const
{
  const face& f = mesh().faces()[faceI];
...
mousse::meshCutAndRemove::meshCutAndRemove(const polyMesh& mesh)
:
  edgeVertex(mesh),
  addedFaces_(),
  addedPoints_()
{}
// Member Functions 
void mousse::meshCutAndRemove::setRefinement
(
  const label exposedPatchI,
  const cellCuts& cuts,
...
void mousse::meshCutAndRemove::updateMesh(const mapPolyMesh& map)
{
