...
void extrudePatchMesh::extrudeMesh(const List<polyPatch*>& regionPatches)
{
  ...
  const primitiveFacePatch pp{extrudedPatch_, extrudedPatch_.points()};
...

  {
    ...
    FOR_ALL(pointLocalRegions, faceI) {
      ...
      FOR_ALL(pRegions, fp) {
        ...
        localSum[localRegionI] += extrudedPatch_.faceNormals()[faceI];
      }
    }
  ...
  // Extrude engine
  createShellMesh extruder{pp, pointLocalRegions, localRegionPoints};
  this->clearOut();
