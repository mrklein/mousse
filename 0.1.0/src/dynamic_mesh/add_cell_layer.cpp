...
mousse::tmp<mousse::vectorField> mousse::layerAdditionRemoval::extrusionDir() const
{
...
void mousse::layerAdditionRemoval::addCellLayer
(
  polyTopoChange& ref
) const
{
  ...
  FOR_ALL(mp, pointI) {
    ...
    addedPoints[pointI] =
      ...
      (
        polyAddPoint
        (
          points[mp[pointI]] + addDelta_*pointOffsets[pointI],
  ...
  const labelList& sc = mesh.faceZones()[faceZoneID_.index()].slaveCells();
  ...
  FOR_ALL(zoneFaces, faceI) {
    ...
    if (!mesh.isInternalFace(mf[faceI]) || mc[faceI] == nei[mf[faceI]]) {
      flipFaceFlux = true;
    }
    // Add the face
    ref.setAction
  ...
  for (label curEdgeID = nInternalEdges;
