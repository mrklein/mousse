...
tmp<fvVectorMatrix> contactAngleForce::correct(volVectorField& U)
{
  tmp<volVectorField> tForce
  {
    new volVectorField
    {
      ...
      owner_.regionMesh(),
      {"zero", dimForce/dimArea, vector::zero}
    }
  };
  ...
  FOR_ALL(nbr, faceI) {
    ...
    if (cellI != -1 && mask_[cellI] > 0.5) {
      ...
      const vector n = gradAlpha[cellI]/(mag(gradAlpha[cellI]) + ROOTVSMALL);
  ...
  FOR_ALL(alpha.boundaryField(), patchI) {
    ...
    FOR_ALL(alphaf, faceI)
    {
      ...
      if ((alpha[cellO] <= 0.5) || (alphaf[faceI] >= 0.5))
        continue;
      const vector n = gradAlpha[cellO]/(mag(gradAlpha[cellO]) + ROOTVSMALL);
      scalar theta = cos(degToRad(distribution_->sample()));
      force[cellO] += Ccf_*n*sigma[cellO]*(1.0 - theta)/invDx[faceI];
    }
  }
  force /= magSf;
  ...
  tmp<fvVectorMatrix> tfvm{new fvVectorMatrix{U, dimForce/dimArea*dimVolume}};
  tfvm() += tForce;
  return tfvm;
}
