...
template<class Type>
void mousse::uniformInterpolationTable<Type>::checkTable() const
{
  if (size() < 2) {
    FATAL_ERROR_IN("uniformInterpolationTable<Type>::checkTable()")
...
// Constructors 
template<class Type>
mousse::uniformInterpolationTable<Type>::uniformInterpolationTable
(
  const IOobject& io,
  bool readFields
)
...
// Member Functions 
template<class Type>
Type mousse::uniformInterpolationTable<Type>::interpolate(scalar x) const
{
  if (bound_) {
...
template<class Type>
Type mousse::uniformInterpolationTable<Type>::interpolateLog10
(
  scalar x
) const
{
...
template<class Type>
void mousse::uniformInterpolationTable<Type>::write() const
{
