...
) const
{
  ...
  {
    new Field<Type>{toPatch_.nPoints(), pTraits<Type>::zero}
  };
  ...
  FOR_ALL(result, pointI) {
    ...
    if (addr[pointI] > -1) {
      const labelList& hitFacePoints = fromPatchLocalFaces[addr[pointI]];
...
) const
{
  ...
  tmp<Field<Type>> tresult
  {
    new Field<Type>{toPatch_.size(), pTraits<Type>::zero}
  };
  Field<Type>& result = tresult();
