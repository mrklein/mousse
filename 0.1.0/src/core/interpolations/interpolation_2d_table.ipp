...
template<class Type>
void mousse::interpolation2DTable<Type>::readTable()
{
  ...
  if (this->empty()) {
    FATAL_ERROR_IN
    (
      "mousse::interpolation2DTable<Type>::readTable()"
    )
...
template<class Type>
mousse::interpolation2DTable<Type>::interpolation2DTable(const fileName& fName)
:
  List<Tuple2<scalar, List<Tuple2<scalar, Type>>>>{},
  boundsHandling_{interpolation2DTable::WARN},
  fileName_{fName},
...
template<class Type>
mousse::interpolation2DTable<Type>::interpolation2DTable(const dictionary& dict)
:
  List<Tuple2<scalar, List<Tuple2<scalar, Type>>>>{},
...
// Member Functions 
template<class Type>
Type mousse::interpolation2DTable<Type>::interpolateValue
(
...
mousse::label mousse::interpolation2DTable<Type>::Xi
(
  const BinaryOp& bop,
  const scalar valueX,
  const bool reverse
...
template<class Type>
mousse::word mousse::interpolation2DTable<Type>::boundsHandlingToWord
(
...
template<class Type>
typename mousse::interpolation2DTable<Type>::boundsHandling
...
template<class Type>
typename mousse::interpolation2DTable<Type>::boundsHandling
mousse::interpolation2DTable<Type>::outOfBounds
(
  const boundsHandling& bound
)
{
...
template<class Type>
void mousse::interpolation2DTable<Type>::checkOrder() const
{
  label n = this->size();
  const table& t = *this;
  ...
  for (label i=1; i<n; ++i) {
    ...
    if (currValue <= prevValue) {
      FATAL_ERROR_IN
      (
        "mousse::interpolation2DTable<Type>::checkOrder() const"
      )
      << "out-of-order value: "
...
template<class Type>
void mousse::interpolation2DTable<Type>::write(Ostream& os) const
{
  os.writeKeyword("fileName") << fileName_ << token::END_STATEMENT << nl;
