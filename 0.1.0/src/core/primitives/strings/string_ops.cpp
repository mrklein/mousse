...
mousse::string& mousse::stringOps::inplaceExpand
(
  string& s,
  const HashTable<string, word, string::hash>& mapping,
  const char sigil
...
mousse::string mousse::stringOps::trim(const string& original)
{
  return trimLeft(trimRight(original));
}


mousse::string& mousse::stringOps::inplaceTrim(string& s)
{
  inplaceTrimRight(s);
  inplaceTrimLeft(s);
  return s;
