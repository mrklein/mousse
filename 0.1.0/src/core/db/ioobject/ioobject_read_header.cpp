...
bool mousse::IOobject::readHeader(Istream& is)
{
  if (IOobject::debug) {
  ...
  if (!is.good()) {
    if (rOpt_ == MUST_READ || rOpt_ == MUST_READ_IF_MODIFIED) {
      FATAL_IO_ERROR_IN("IOobject::readHeader(Istream&)", is)
        << " stream not open for reading essential object from file "
        << is.name()
        << exit(FatalIOError);
    }
    if (IOobject::debug) {
      SERIOUS_IO_ERROR_IN("IOobject::readHeader(Istream&)", is)
  ...
  token firstToken{is};
  if (is.good() && firstToken.isWord() && firstToken.wordToken() == "FoamFile") {
    ...
    if (IOobject::debug && headerObject != name()) {
      IO_WARNING_IN("IOobject::readHeader(Istream&)", is)
        << " object renamed from "
