...
unsigned int mousse::PackedList<nBits>::count() const
{
  unsigned int c = 0;
  if (size_) {
...
template<unsigned nBits>
bool mousse::PackedList<nBits>::trim()
{
  if (!size_) {
    return false;
  }
...
template<unsigned nBits>
void mousse::PackedList<nBits>::flip()
{
  if (!size_) {
    return;
  }
...
template<unsigned nBits>
mousse::Xfer<mousse::labelList> mousse::PackedList<nBits>::values() const
{
...
template<unsigned nBits>
mousse::Ostream& mousse::PackedList<nBits>::iteratorBase::printInfo
(
  Ostream& os
) const
{
...
template<unsigned nBits>
mousse::Ostream& mousse::PackedList<nBits>::printBits
(
  Ostream& os,
...
template<unsigned nBits>
mousse::Ostream& mousse::PackedList<nBits>::printInfo
(
  Ostream& os,
  const bool fullOutput
) const
...
template<unsigned nBits>
mousse::Istream& mousse::PackedList<nBits>::read(Istream& is)
{
  ...
  if (firstTok.isLabel()) {
    ...
    if (firstTok.pToken() == token::BEGIN_LIST) {
      token nextTok(is);
      is.fatalCheck("PackedList<nBits>::read(Istream&)");
      while (!(nextTok.isPunctuation() && nextTok.pToken() == token::END_LIST)) {
        is.putBack(nextTok);
      ...
      token nextTok(is);
      is.fatalCheck("PackedList<nBits>::read(Istream&)");
      while (!(nextTok.isPunctuation() && nextTok.pToken() == token::END_BLOCK)) {
        is.putBack(nextTok);
        lst.setPair(is);
        is >> nextTok;
        is.fatalCheck("PackedList<nBits>::read(Istream&)");
...
template<unsigned nBits>
mousse::Ostream& mousse::PackedList<nBits>::write
(
  Ostream& os,
...
template<unsigned nBits>
void mousse::PackedList<nBits>::writeEntry(Ostream& os) const
{
  os << *this;
}
...
template<unsigned nBits>
void mousse::PackedList<nBits>::writeEntry
(
  const word& keyword,
  Ostream& os
) const
...
// Member Operators 
template<unsigned nBits>
mousse::PackedList<nBits>&
mousse::PackedList<nBits>::operator=(const PackedList<nBits>& lst)
{
...
template<unsigned nBits>
mousse::PackedList<nBits>&
mousse::PackedList<nBits>::operator=(const labelUList& lst)
{
...
template<unsigned nBits>
mousse::PackedList<nBits>&
mousse::PackedList<nBits>::operator=(const UIndirectList<label>& lst)
{
  setCapacity(lst.size());
  size_ = lst.size();
  FOR_ALL(lst, i) {
...
//  Friend Operators
template<unsigned nBits>
mousse::Istream& mousse::operator>>(Istream& is, PackedList<nBits>& lst)
{
  return lst.read(is);
}
...
template<unsigned nBits>
mousse::Ostream& mousse::operator<<(Ostream& os, const PackedList<nBits>& lst)
{
