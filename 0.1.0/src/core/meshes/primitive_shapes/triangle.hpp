...
) const
{
  ...
  if (d1 <= 0.0 && d2 <= 0.0) {
    ...
    nearType = POINT;
    nearLabel = 0;
    return {false, a_, mousse::mag(a_ - p), true};
  }
  ...
  if (d3 >= 0.0 && d4 <= d3) {
    ...
    nearType = POINT;
    nearLabel = 1;
    return {false, b_, mousse::mag(b_ - p), true};
  }
  ...
  if (vc <= 0.0 && d1 >= 0.0 && d3 <= 0.0) {
    ...
    nearType = EDGE;
    nearLabel = 0;
    return {false, nearPt, mousse::mag(nearPt - p), true};
  }
  ...
  if (d6 >= 0.0 && d5 <= d6) {
    ...
    nearType = POINT;
    nearLabel = 2;
    return {false, c_, mousse::mag(c_ - p), true};
  }
  ...
  if ((va + vb + vc) < ROOTVSMALL) {
    ...
    nearType = NONE,
    nearLabel = -1;
    return {true, nearPt, mousse::mag(nearPt - p), false};
  }
