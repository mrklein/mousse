...
)
{
  ...
  if (!is || nTris < 0 || nTris < (dataFileSize - headerSize)/50
...
)
{
  ...
  // Also compare the file size with that expected from the number of tris
  // If the comparison is not sensible then it may be an ASCII file
  if (!is || nTris < 0 || nTris < int(dataFileSize - headerSize)/50
...
)
{
  ...
  char header[headerSize];
  snprintf(header, headerSize, "STL binary file %u facets", nTris);
  // avoid trailing junk
