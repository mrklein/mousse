...
void mousse::forces::writeFileHeader(const label i)
{
  if (i == 0) {
    // force data
...
mousse::tmp<mousse::volSymmTensorField> mousse::forces::devRhoReff() const
{
...
mousse::tmp<mousse::volScalarField> mousse::forces::mu() const
{
...
mousse::tmp<mousse::volScalarField> mousse::forces::rho() const
{
  if (rhoName_ == "rhoInf") {
...
mousse::scalar mousse::forces::rho(const volScalarField& p) const
{
...
void mousse::forces::writeForces()
{
  if (log_) {
...
void mousse::forces::execute()
{
  // Do nothing - only valid on write
}


void mousse::forces::end()
{
  // Do nothing - only valid on write
}


void mousse::forces::timeSet()
{
  // Do nothing - only valid on write
}


void mousse::forces::write()
{
  calcForcesMoment();
  if (!active_) {
    return;
...
void mousse::forces::calcForcesMoment()
{
  initialise();
  if (!active_) {
    return;
  ...
  if (directForceDensity_) {
    ...
    FOR_ALL_CONST_ITER(labelHashSet, patchSet_, iter) {
      label patchI = iter.key();
      vectorField Md{mesh.C().boundaryField()[patchI] - coordSys_.origin()};
...
mousse::vector mousse::forces::forceEff() const
{
  return sum(force_[0]) + sum(force_[1]) + sum(force_[2]);
}


mousse::vector mousse::forces::momentEff() const
{
