...
mousse::COxidationHurtMitchell<CloudType>::COxidationHurtMitchell
(
...
// Member Functions 
template<class CloudType>
mousse::scalar mousse::COxidationHurtMitchell<CloudType>::calculate
(
  const scalar dt,
  const label cellI,
  const scalar d,
...
) const
{
  ...
  const scalar qCs = min(convSI*Rk*mousse::sqrt(ppO2/101325.0), qCsLim);
