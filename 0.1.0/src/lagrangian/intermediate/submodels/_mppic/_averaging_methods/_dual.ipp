...
template<class Type>
mousse::autoPtr<mousse::labelList> mousse::AveragingMethods::Dual<Type>::size
(
  const fvMesh& mesh
)
{
...
// Constructors 
template<class Type>
mousse::AveragingMethods::Dual<Type>::Dual
(
  const IOobject& io,
...
template<class Type>
void mousse::AveragingMethods::Dual<Type>::tetGeometry
(
...
template<class Type>
void mousse::AveragingMethods::Dual<Type>::syncDualData()
{
...
// Member Functions 
template<class Type>
void mousse::AveragingMethods::Dual<Type>::add
(
  const point position,
  const tetIndices& tetIs,
  const Type& value
)
{
  ...
  dataCell_[tetIs.cell()] +=
    tetCoordinates_[0]*value/(0.25*volumeCell_[tetIs.cell()]);
  for(label i = 0; i < 3; i ++) {
    dataDual_[tetVertices_[i]] +=
      tetCoordinates_[i+1]*value/(0.25*volumeDual_[tetVertices_[i]]);
  }
}


template<class Type>
Type mousse::AveragingMethods::Dual<Type>::interpolate
(
  const point position,
  const tetIndices& tetIs
) const
...
template<class Type>
typename mousse::AveragingMethods::Dual<Type>::TypeGrad
mousse::AveragingMethods::Dual<Type>::interpolateGrad
(
  const point position,
  const tetIndices& tetIs
) const
...
template<class Type>
void mousse::AveragingMethods::Dual<Type>::average()
{
  syncDualData();
...
mousse::AveragingMethods::Dual<Type>::internalField() const
{
