...
void mousse::PairSpringSliderDashpot<CloudType>::findMinMaxProperties
(
  scalar& RMin,
  scalar& rhoMax,
  scalar& UMagMax
...
// Constructors 
template<class CloudType>
mousse::PairSpringSliderDashpot<CloudType>::PairSpringSliderDashpot
(
...
// Member Functions 
template<class CloudType>
bool mousse::PairSpringSliderDashpot<CloudType>::controlsTimestep() const
{
  return true;
}
...
template<class CloudType>
mousse::label mousse::PairSpringSliderDashpot<CloudType>::nSubCycles() const
{
  if (!(this->owner().size())) {
    return 1;
  }
  ...
  const scalar P = pow(rhoMax/(Estar_*sqrt(UMagMax) + VSMALL), 0.4);
  scalar minCollisionDeltaT = 5.429675*RMin*P/collisionResolutionSteps_;
  return ceil(this->owner().time().deltaTValue()/minCollisionDeltaT);
}


template<class CloudType>
void mousse::PairSpringSliderDashpot<CloudType>::evaluatePair
(
...
) const
{
  ...
  if (normalOverlapMag > 0) {
    ...
    // Normal force
    vector fN_AB =
      rHat_AB*(kN*pow(normalOverlapMag, b_) - etaN*(U_AB & rHat_AB));
