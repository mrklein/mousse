...
mousse::scalar mousse::pairPotential::force(const scalar r) const
{
  ...
  scalar f =
    (k_rIJ - k)*forceLookup_[k+1] + (k + 1 - k_rIJ)*forceLookup_[k];
  return f;
}
...
mousse::scalar mousse::pairPotential::energy(const scalar r) const
{
  ...
  scalar e =
    (k_rIJ - k)*energyLookup_[k+1] + (k + 1 - k_rIJ)*energyLookup_[k];
  return e;
}
...
) const
{
  ...
  scalar a1 =
    (rb*rb*(Ea - Ef) + ra*ra*(Ef - Eb) + rf*rf*(Eb - Ea))/denominator;
  scalar a2 =
    (rb*(Ef - Ea) + rf*(Ea - Eb) + ra*(Eb - Ef))/ denominator;
  return a1 + 2.0*a2*r;
}
