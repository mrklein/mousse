...
mousse::ReitzKHRT<CloudType>::ReitzKHRT
(
...
template<class CloudType>
mousse::ReitzKHRT<CloudType>::ReitzKHRT(const ReitzKHRT<CloudType>& bum)
:
...
// Member Functions 
template<class CloudType>
bool mousse::ReitzKHRT<CloudType>::update
(
  const scalar dt,
  const vector& g,
  scalar& d,
...
)
{
  ...
  scalar omegaRT =
    sqrt(2.0*pow(helpVariable, 1.5)/(3.0*sqrt(3.0*sigma)*(rhoc + rho)));
  // RT wave number
