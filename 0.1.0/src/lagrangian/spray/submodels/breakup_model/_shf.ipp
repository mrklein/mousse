...
template<class CloudType>
mousse::SHF<CloudType>::SHF(const SHF<CloudType>& bum)
:
...
// Member Functions 
template<class CloudType>
bool mousse::SHF<CloudType>::update
(
  const scalar dt,
  const vector& /*g*/,
  scalar& d,
  ...
  if (tc > tBreakUP) {
    ...
    if (bag || multimode) {
      ...
      for(;;) {
        ...
        scalar p = x/(2.0*sqrt(twoPi)*sigma_)*exp(-0.5*sqr((x - mu_)/sigma_));
        if (yGuess < p) {
          break;
        }
      }
