...
#include "starcd_mesh_reader.hpp"
...
// Static Data Members
const char* const mousse::meshReaders::STARCD::defaultBoundaryName =
  "Default_Boundary_Region";
const char* const mousse::meshReaders::STARCD::defaultSolidBoundaryName =
  "Default_Boundary_Solid";
bool mousse::meshReaders::STARCD::keepSolids = false;
const int mousse::meshReaders::STARCD::starToFoamFaceAddr[4][6] =
{
...
void mousse::meshReaders::STARCD::readToNewline(IFstream& is)
{
  char ch = '\n';
  do {
    (is).get(ch);
...
bool mousse::meshReaders::STARCD::readHeader(IFstream& is, word fileSignature)
{
  if (!is.good()) {
...
void mousse::meshReaders::STARCD::readAux(const objectRegistry& registry)
{
...
//
void mousse::meshReaders::STARCD::readPoints
(
...
//
void mousse::meshReaders::STARCD::readCells(const fileName& inputName)
{
    ...
    while ((is >> lineLabel).good()) {
      ...
      if (curModelPtr) {
        ...
        for (label i=0; i < nFaces; ++i) {
          ...
          face f{SubList<label>{starLabels, n, beg}};
          f.collapse();
          // valid faces only
          if (f.size() >= 3) {
            faces[faceI++] = f;
        ...
        face f{SubList<label>{starLabels, nLabels}};
        f.collapse();
        // valid faces only
        if (f.size() >= 3) {
...
//
void mousse::meshReaders::STARCD::readBoundary(const fileName& inputName)
{
  ...
  // - this makes it easier to transfer them to an adjacent patch if reqd
  {
    ...
    if (is.good()) {
      ...
      while ((is >> lineLabel).good()) {
        nFaces++;
        is >> starCellId >> cellFaceId >> starRegion >> configNumber
          >> patchType;
...
void mousse::meshReaders::STARCD::cullPoints()
{
...
bool mousse::meshReaders::STARCD::readGeometry(const scalar scaleFactor)
{
...
// Constructors 
mousse::meshReaders::STARCD::STARCD
(
