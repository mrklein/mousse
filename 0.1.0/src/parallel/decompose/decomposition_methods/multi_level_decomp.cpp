...
)
{
  labelList dist
  {
    methods_[levelI].decompose(pointPoints, points, pointWeights)
  };
  FOR_ALL(pointMap, i) {
  ...
  if (levelI != methods_.size()-1) {
    ...
    if (debug) {
      ...
      if (debug && Pstream::master()) {
        Pout << "Reference decomposition with " << myDict << " :" << endl;
      }
      ...
      labelList dist
      {
        method0().decompose(pointPoints, points, pointWeights)
      };
