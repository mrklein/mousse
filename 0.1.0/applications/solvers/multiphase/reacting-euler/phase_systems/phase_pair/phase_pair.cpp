...
{
  return mag(dispersed().rho() - continuous().rho())*mag(g())
    *sqr(d)/sigma();
}
// Constructors 
...
{
  return continuous().nu()*continuous().thermo().Cpv()*continuous().rho()
    /continuous().kappa();
}
...
mousse::tmp<mousse::volScalarField> mousse::phasePair::sigma() const
{
  return
...
mousse::tmp<mousse::volScalarField> mousse::phasePair::Mo() const
{
  return
    ...
    *pow3
    (
      continuous().nu()*continuous().rho()/sigma()
    );
}
