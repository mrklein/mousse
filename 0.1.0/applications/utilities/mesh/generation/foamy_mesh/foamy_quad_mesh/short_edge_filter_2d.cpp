...
void mousse::shortEdgeFilter2D::assignBoundaryPointRegions
(
...
void mousse::shortEdgeFilter2D::updateEdgeRegionMap
(
...
) const
{
  ...
  FOR_ALL(edges, edgeI) {
    ...
    if (startPtRegions.size() > 1 && endPtRegions.size() > 1) {
      ...
      region = startPtRegions.size() > 1 ? endPtRegions[0] : startPtRegions[0];
    } else if (startPtRegions[0] == endPtRegions[0]
               && startPtRegions[0] != -1) {
      region = startPtRegions[0];
    }
    if (region != -1) {
...
// Constructors 
mousse::shortEdgeFilter2D::shortEdgeFilter2D
(
  const mousse::CV2D& cv2Dmesh,
...
// Member Functions 
void
mousse::shortEdgeFilter2D::filter()
{
  ...
  FOR_ALL(edges, edgeI) {
    ...
    FOR_ALL(boundaryPoints, bPoint) {
      if (boundaryPoints[bPoint] == startVertex
  ...
  FOR_ALL(edges, edgeI) {
    ...
    const label endVertex = e.end();
    scalar edgeLength =
      mag(points[meshPoints[startVertex]] - points[meshPoints[endVertex]]);
    ...
    FOR_ALL(psEdges, psEdgeI) {
      ...
      shortEdgeFilterValue +=
        mag(points[meshPoints[psE.start()]] - points[meshPoints[psE.end()]]);
    }
    FOR_ALL(peEdges, peEdgeI) {
      ...
      if (edgeI == peEdges[peEdgeI])
        continue;
      shortEdgeFilterValue +=
        mag(points[meshPoints[peE.start()]] - points[meshPoints[peE.end()]]);
    }
    shortEdgeFilterValue *=
      shortEdgeFilterFactor_/(psEdges.size() + peEdges.size() - 2);
    edge lookupInPatchEdge{meshPoints[startVertex], meshPoints[endVertex]};
    ...
    if (pointsToRemove[meshPoints[startVertex]] == -1
  ...
  FOR_ALL(faces, faceI) {
    ...
    FOR_ALL(f, fp) {
      ...
      if (pointsToRemove[pointI] == -1) {
        ...
        // collapsed to.
        if (f.nextLabel(fp) != newPointI && f.prevLabel(fp) != newPointI) {
...
void mousse::shortEdgeFilter2D::writeInfo(Ostream& os)
{
