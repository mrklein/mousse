...
<
  ...
  class PointField
>
mousse::Xfer<mousse::pointField> mousse::extrudedMesh::extrudedPoints
(
...
mousse::Xfer<mousse::faceList> mousse::extrudedMesh::extrudedFaces
(
  const PrimitivePatch<Face, FaceList, PointField>& extrudePatch,
  const extrudeModel& model
)
{
  ...
  for (label layer=0; layer<nLayers; layer++) {
    ...
    if (layer < nLayers-1) {
      FOR_ALL(surfaceFaces, i) {
        eFaces[facei++] = face{surfaceFaces[i] + nextLayerOffset};
      }
    }
  }
  // External side faces
  ...
  FOR_ALL(surfaceFaces, i) {
    eFaces[facei++] =
      face{surfaceFaces[i] + nLayers*surfacePoints.size()};
  }
  // return points for transferring
...
template<class Face, template<class> class FaceList, class PointField>
mousse::Xfer<mousse::cellList> mousse::extrudedMesh::extrudedCells
(
  const PrimitivePatch<Face, FaceList, PointField>& extrudePatch,
  const extrudeModel& model
)
...
<
  ...
  class PointField
>
mousse::extrudedMesh::extrudedMesh
(
  const IOobject& io,
  const PrimitivePatch<Face, FaceList, PointField>& extrudePatch,
  const extrudeModel& model
